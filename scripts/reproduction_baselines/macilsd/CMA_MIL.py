...
def CMAL(mmil_logits, audio_logits, visual_logits, seq_len, audio_rep, visual_rep):
    """Cross-modal contrastive instance loss. Verbatim upstream but for M3/M6.

    Selection is *self-guided*: the branch is chosen by the model's own bag
    score `mmil_logits[i] > 0.5`, not by the video label. That is upstream
    behaviour and it is kept.
    """
    # PORT PATCH (patch M3): the six accumulators take their device from the
    # incoming logits instead of being allocated with .cuda().
    device = audio_logits.device
    audio_abn = torch.zeros(0, device=device)  # tensor([])
    visual_abn = torch.zeros(0, device=device)  # tensor([])
    audio_bgd = torch.zeros(0, device=device)  # tensor([])
    visual_bgd = torch.zeros(0, device=device)  # tensor([])
    audio_nor = torch.zeros(0, device=device)
    visual_nor = torch.zeros(0, device=device)
