...
def attention(query, key, value, masksize, dropout=None):
    ...
    if masksize != 1:
        ...
        # PORT PATCH (patch M1): upstream allocates the mask with .cuda(), so
        # the module cannot run on cpu at all. The device is taken from the
        # scores tensor instead; the values are unchanged. Note that MACIL-SD
        # never leaves masksize at anything but its default 1, so this branch
        # is dead in every published configuration -- it is patched only so the
        # file imports and runs device-agnostically.
        mask = torch.ones(scores.size(), device=scores.device)
