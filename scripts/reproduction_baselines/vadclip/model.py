"""VadCLIP (AAAI'24) model, ported for binary hateful-video localisation.

Vendored from https://github.com/nwpu-zxr/VadCLIP @ c41067f (src/model.py).
...
# PORT PATCH (patch V1): upstream runs with src/ as cwd, so it imports
# `clip` and `utils.layers` as top-level modules. This port lives in a
# package, so the two imports are repointed at hate_common/.
