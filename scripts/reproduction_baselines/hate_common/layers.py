...
class GraphAttentionLayer(nn.Module):
    ...
    def __init__(self, in_features, out_features, dropout, alpha, concat=True):
        ...
        # PORT PATCH (patch L2): nn.init.xavier_uniform and torch.cuda.FloatTensor
        # are removed in torch 2.x. GraphAttentionLayer is dead code in both
        # upstream repos (nothing constructs it); the initialiser is rewritten
        # only so that importing this module does not fail.
        self.W = nn.Parameter(nn.init.xavier_uniform_(torch.empty(in_features, out_features), gain=np.sqrt(2.0)), requires_grad=True)
        self.a = nn.Parameter(nn.init.xavier_uniform_(torch.empty(2*out_features, 1), gain=np.sqrt(2.0)), requires_grad=True)
...
class DistanceAdj(Module):
    # PORT PATCH (see scripts/reproduction_baselines/PATCHES.md, patch L1):
    # upstream hard-codes .to('cuda') twice, which makes the module unusable on
    # CPU. The matrix is a pure function of max_seqlen, so it is now built once
    # per (device, max_seqlen) and cached. Numerically identical to upstream.
    ...
    def forward(self, batch_size, max_seqlen):
        ...
        if self._cache_key != key:
            ...
            dist = torch.exp(-dist / torch.exp(torch.tensor(1.0, device=device)))
            self._cache_key = key
            self._cache = dist
        return torch.unsqueeze(self._cache, 0).repeat(batch_size, 1, 1)
