"""DSANet (AAAI'26) model, ported for binary hateful-video localisation.

Vendored from https://github.com/lessiYin/DSANet @ eb335b2 (src/model.py).
Every difference from upstream is marked "PORT PATCH" and listed in
scripts/reproduction_baselines/PATCHES.md.
"""
...
# PORT PATCH (patch V1): upstream runs with src/ as cwd and imports `clip`
# and `utils.*` as top-level modules. Repointed at this port's package layout.
from hate_common.clip import clip
from hate_common.layers import GraphConvolution, DistanceAdj
from .adapter_modules import SimpleAdapter, SimpleProj
from .descriptions import DESCRIPTIONS_HATE
from .dnp_vision_transformer import Aggregation_Block, Prototype_Block
...
class DSANet(nn.Module):
    def __init__(self,
        ...
        # PORT PATCH (patch V2): clip_download_root added so the frozen CLIP
        # checkpoint can be pointed at a shared cache instead of ~/.cache/clip.
        super().__init__()
    ...
    def train(self, mode: bool = True):
        # PORT PATCH (patch D4): get_text_features caches the class embeddings
        # the first time it runs in eval mode and never invalidates them. The
        # text adapter is trainable, so after another epoch of training the
        # cached embeddings are stale, and upstream's per-epoch evaluation
        # scores every later epoch with the first epoch's text features. This
        # port drops the cache on every train/eval switch; within one eval pass
    ...
    def get_text_features(self, text):
        ...
        # PORT PATCH (patch D2): upstream picks the class-description table by
        # `len(text)` (14 -> UCF-Crime, else XD-Violence) and ignores the
        # prompt list it is handed. This port has exactly one class table, the
        # binary one, so the dispatch is removed. The `text` argument is kept in
        # the signature (callers still pass it) but, as upstream, unused.
        category_features = []
        DESCRIPTIONS = DESCRIPTIONS_HATE
