#!/usr/bin/env python
"""Persistent single-allocation GPU ledger for C04-A0T-SMALL-v1 impl-v8.
...
# `NO_SEAL_SENTINEL` is IMPORTED, not redefined.  v7 defined it independently in
# this module and in the common module; the v7 round-5 reviewer recorded that as
# a non-blocking observation with an explicit instruction -- "any v8 must import
# it" -- because a one-sided edit would resurface the round-4 High (an
# unsatisfiable intersection of code and schema) only after the GPU is spent.
#
# The wrapper timeout is anchored to /proc/uptime while sacct measures from job
# start, so a terminal elapsed can legitimately land a little above the tranche
...
# later allocation.  The slack is the same 600 s v7 carried over its own cap.
TERMINAL_SECONDS_HARD_MAX = SMALL_TRANCHE_CAP_GPU_SECONDS + 600
...
def validate_ledger(ledger: dict[str, Any], cfg: dict[str, Any]) -> None:
    ...
    assert_equal(ledger["schema_version"], "c04_gpu_ledger_v8", "ledger schema")
    assert_equal(ledger["run_id"], RUN_ID, "ledger run id")
    assert_equal(
        ledger["implementation_version"], "v8_prospective", "ledger implementation"
    )
...
def verify_reconciliation_lineage(cfg: dict[str, Any]) -> dict[str, str]:
    ...
    assert_equal(claim["schema_version"], "c04_allocation_claim_v8", "claim schema")
    ...
    assert_equal(
        ...
        cfg["resources"]["small_cap_gpu_seconds"],
        "claim the full tranche reservation",
    )
    assert_equal(
    ...
    assert_equal(marker["schema_version"], "c04_allocation_entry_marker_v8", "marker schema")
    ...
    if not seal_published:
        ...
        # holding the whole tranche reservation forever.
...
def create_entry_marker(cfg: dict[str, Any], job_id: str, start_uptime: int) -> None:
    ...
    marker = {
        "schema_version": "c04_allocation_entry_marker_v8",
        "run_id": RUN_ID,
        "slurm_job_id": job_id,
...
def claim(cfg: dict[str, Any], start_uptime: int) -> int:
    ...
    with lock_path.open("a+b") as lock_handle:
        ...
        if not ledger["jobs"] and claim_path.exists():
            ...
            if not consumption_path.exists():
                exclusive_publish_json(cfg["paths"]["resource_consumption"], {
                    "schema_version": "c04_resource_ticket_consumption_v8",
                    "run_id": RUN_ID,
                    "slurm_job_id": job_id,
        ...
        assert_equal(ticket["schema_version"], "c04_resource_ticket_v8", "ticket schema")
        ...
        claim_record = {
            "schema_version": "c04_allocation_claim_v8",
            "run_id": RUN_ID,
            "slurm_job_id": job_id,
            "allocation_entry_uptime_seconds": start_uptime,
...
def publish_or_verify_resource_final_state(
    ...
    job = ledger["jobs"][0]
    immutable = {
        "schema_version": "c04_resource_final_state_v8",
        "run_id": RUN_ID,
    ...
    with lock_path.open("a+b") as lock_handle:
        ...
        if ledger_file_sha == reviewed_pre_sha:
            ...
            if terminal["elapsed_gpu_seconds"] > ledger["cap_gpu_seconds"]:
                ...
                # standing full-tranche reservation and no publishable final state,
