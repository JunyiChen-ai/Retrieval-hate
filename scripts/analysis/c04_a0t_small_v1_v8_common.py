...
SCHEMA_VERSION = "c04_a0t_small_v1_impl_v8"
...
NUM_FRAMES = 8
# ---------------------------------------------------------------------------
# The deployed visual-input cap.  v7 built the processor with NO max_pixels, so
# frames entered the vision tower at native resolution.  Job 13852 died on the
# first MHC_zh item (1080x1920) because Qwen2.5-VL vision attention runs on
# PRE-merge patch tokens and materialises its score tensor in fp32:
# (4,138,78) = 43,056 tokens, 43,056^2 x 16 heads x 4 bytes = 110.50 GiB, which
# is exactly the allocation the job reported.
#
# 151200 = 360*420 is NOT a convenience constant chosen to make the run fit.  It
# is the cap every deployed Qwen2.5-VL entrypoint in this repository already
# uses -- `generate_c02_density_view_text_embedding_HF.py:157`,
# `predict_target_qwen.py:299`, `generate_vision_summary.py:69`,
# `p10_score_segments.py:147`, `p10c_score_segments.py:137`,
# `score_segments_mllm.py:87`, `role3/arbitrate_qwen.py:363` all default to
# `360*420`, and `generate_VideoMLLM_embedding_bidir_textpool_HF.py:29` calls
# 151200 "the deployed max_pixels".  Adopting it makes the teacher's visual
# input identical in kind to every other Qwen2.5-VL call in the project; NOT
# adopting it is what made v7's teacher input unlike the deployed pipeline.
#
# This IS a teacher-input change and is recorded as one: 178/200 HateMM items
# and the MHC_zh items above the cap are down-sampled relative to v7.  That is
# also why no v7 record may be salvaged into a v8 bank -- teacher input must be
# uniform across the sealed tranche.
TEACHER_MAX_PIXELS = 360 * 420
# Fail-closed runtime ceiling on PRE-merge visual patch tokens per item.  It is
# checked after the processor and BEFORE `model.generate`, so an item whose
# geometry escapes the cap halts instead of reaching the vision tower.  At 4096
# tokens the worst-case fp32 vision-SDPA score tensor is
# 4096^2 x 16 x 4 bytes = 1.00 GiB, against 79.27 GiB of device memory; the
# measured worst case over all 400 sealed items must sit strictly under it.
VISUAL_PATCH_TOKEN_HARD_CEILING = 4096
VISUAL_TOKEN_HALT = "HALT_VISUAL_TOKEN_CEILING"
TRANSCRIPT_CAP = 2048
...
ARTIFACT_ROOT = Path("artifacts/c04/a0t_small_v1_impl_v8")
CONFIG_RELATIVE = "configs/c04/c04_a0t_small_v1_v8.json"
TRAIN_ASR_RELATIVE = {
...
PROMPT_HASH_FREEZE_SCHEMA = "c04_prompt_hash_freeze_v8"
...
# strict key equality.  v8's first draft added three fields on the writer side
...
PROMPT_RECORD_SCHEMA_RELATIVE = (
    "schemas/c04/c04_a0t_small_v1_v8_prompt_record.schema.json"
)
CANONICAL_RECORD_SCHEMA_RELATIVE = (
    "schemas/c04/c04_a0t_small_v1_v8_canonical_record.schema.json"
)
...
def verify_preflight_manifest(
    ...
    if manifest["run_id"] != RUN_ID or manifest["implementation_version"] != "v8_prospective":
...
def teacher_visible_containment_fixtures() -> list[tuple[str, bool]]:
    ...
    def with_benign_template_tamper() -> None:
        # v7 round-5 non-blocking observation 2: the only template-tamper
        # fixture injected "hate_video_3", which is itself a banned token, so
        # deleting the template-equality check turned no fixture red -- the ban
        # scan caught it instead.  This injection contains NO banned token and
        # NO identifier, so it is rejected by the template-equality clause
        # alone.  Deleting that clause now goes red here.
        payload = messages_of(clean_transcript, "A")
    ...
    def benign_tamper_carries_no_banned_token() -> bool:
        payload = messages_of(clean_transcript, "A")
        payload[1]["content"][1]["text"] += "\nAdditional operator note."
        haystacks = []
        for text in teacher_visible_texts(payload):
            normalized = unicodedata.normalize("NFKC", text)
            haystacks.extend((normalized, normalized.casefold()))
        return not any(
            variant in haystack
            for token in forbidden
            for variant in _containment_variants(token)
    ...
    return [
        ...
        (
            "teacher_visible_benign_template_tamper_rejected",
            _raises_runtime_error(with_benign_template_tamper),
        ),
        (
            "teacher_visible_benign_tamper_is_not_caught_by_the_ban_scan",
            benign_tamper_carries_no_banned_token(),
        ),
...
# ---------------------------------------------------------------------------
# Frame packs, and the visual geometry the teacher will actually see.
#
# In v7 the eight PNG frames were decoded and written INSIDE the GPU allocation.
# Measured on job 13852 that cost roughly 0.54 s per HateMM item (0.11 s decode
# + 0.43 s PNG encode) and 3.4 s per MHC_zh item (0.72 s + 2.66 s at
# 1080x1920) -- of the order of 700 s of pure CPU work with the A100 idle,
# against a tranche whose remaining ceiling is now 5222 s.  In v8 the CPU
# preflight materialises every frame pack and hash-pins it in the preflight
# manifest, and the producer may only LOAD one.  Three consequences, all
# intended:
#
#   * ~700 s of CPU work leaves the GPU allocation;
#   * the frames become auditable BEFORE the payload review rather than being
#     created by the very job the review authorises;
#   * the producer can no longer decode a video at all, so an unexpected decode
#     cost cannot appear inside the budgeted window.
#
# The frame BYTES are unchanged: the same eight indices, the same decoder
# preference, the same black-frame rule, the same PNG settings.  Only the stage
# that writes them moves.
# ---------------------------------------------------------------------------
IMPLEMENTATION_VERSION = "v8_prospective"
FRAME_PACK_MANIFEST_SCHEMA = "c04_frame_pack_manifest_v8"
FRAME_PACK_HALT = "HALT_FRAME_PACK_CONTRACT"
FRAME_PACK_BINDING_KEYS = (
    "run_id",
    "implementation_version",
    "dataset",
    "video_id",
...
def requested_frame_indices(total: int) -> list[int]:
    """The frozen V2/V3 index rule: floor((i+0.5)*N/8), clamped, or [] at N==0."""
    if total <= 0:
        return []
    return [
...
def _pyav_decode_exact(path: Path) -> tuple[list[Any], int]:
    """Two-pass pyav fallback: count frames, then keep only the eight requested.

    v7's fallback materialised EVERY frame as a PIL image before selecting
    eight.  Measured on the one MHC_zh item whose decord open fails
    (`BV18N4y1B7qA`): 2879 frames at 1920x842 in 32.9 s, which as PIL RGB is
    of the order of 14 GiB resident.  The frames this returns are the same
    objects the one-pass version selected -- `decoded[i] for i in requested` --
    so the written pack is byte-identical; only the peak memory changes, from
    O(all frames) to O(8).
    """
    import av
    ...
    try:
        ...
        container.close()
    requested = requested_frame_indices(total)
    if total == 0:
        return [], 0
    ...
    try:
        for index, frame in enumerate(container.decode(video=0)):
            if index in wanted:
                keep[index] = frame.to_image().convert("RGB")
            if len(keep) == len(wanted):
                break
    finally:
        container.close()
    if set(keep) != wanted:
        raise RuntimeError(f"{FRAME_PACK_HALT}: pyav second pass lost a requested frame")
    return [keep[index] for index in requested], total


def decode_exact_frames(path: Path) -> dict[str, Any]:
    """Decode the frozen eight indices; a failed request is black, never substituted."""
    from PIL import Image
    ...
    try:
        ...
        requested = []
        try:
            decoded, total = _pyav_decode_exact(path)
            backend = "pyav"
            if total > 0:
                requested = requested_frame_indices(total)
                frames = decoded
...
def frame_pack_binding(
    cfg: dict[str, Any],
    dataset: str,
    video_id: str,
    transcript_sha256: str,
    video_sha256: str,
    code_resource_authorization_sha256: str,
) -> dict[str, Any]:
    """The identity a frame pack is bound to, using only preflight-time facts.

    v7 bound a pack to `payload_review_sha256`, `gpu_execution_authorization_
    sha256` and `allocation_claim_sha256`, none of which exist when the CPU
    preflight runs -- they are produced by the review that comes AFTER the
    preflight and by the allocation that comes after that.  Those three are
    replaced by the code/resource authorization, which is the review that
    authorises the preflight itself, plus the config contract.  Nothing is left
    unbound: every pack's bytes enter `staged_output_hashes` in the preflight
    manifest, the manifest is pinned by the payload review, and the payload
    review is pinned by the GPU execution authorization.  The chain is the same
    ...
    binding = {
        ...
        "config_contract_sha256": config_contract_sha256(cfg),
        "code_resource_authorization_sha256": code_resource_authorization_sha256,
    ...
    return require_exact_keys(binding, set(FRAME_PACK_BINDING_KEYS), "frame-pack binding")
...
def encode_frame_png(frame: Any) -> bytes:
    import io
...
def write_frame_pack(
    pack_root: Path,
    video_id: str,
    decoded: dict[str, Any],
    binding: dict[str, Any],
) -> dict[str, Any]:
    """Materialise one immutable frame pack atomically and return its manifest."""
    final_dir = pack_root / video_id
    if final_dir.exists():
        raise RuntimeError(f"{FRAME_PACK_HALT}: frame pack already exists for {video_id}")
    pack_root.mkdir(parents=True, exist_ok=True)
    temp_dir = Path(tempfile.mkdtemp(prefix=video_id + ".tmp.", dir=str(pack_root)))
    try:
        frame_rows = []
        for index, frame in enumerate(decoded["frames"]):
            payload = encode_frame_png(frame)
        ...
        manifest = {
            "schema_version": FRAME_PACK_MANIFEST_SCHEMA,
            **binding,
        ...
        _write_fsynced(temp_dir / "manifest.json", canonical_json_bytes(manifest) + b"\n")
...
def strict_validate_frame_pack(
    pack_root: Path,
    video_id: str,
    binding: dict[str, Any],
) -> tuple[dict[str, Any], list[Path], str]:
    """Validate one immutable frame pack and all decode metadata in one place."""
    final_dir = pack_root / video_id
    if final_dir.is_symlink() or not final_dir.is_dir():
        raise RuntimeError(f"{FRAME_PACK_HALT}: frame-pack directory invalid")
    pack_root_resolved = pack_root.resolve(strict=True)
    final_resolved = final_dir.resolve(strict=True)
    if final_resolved.parent != pack_root_resolved:
        raise RuntimeError(f"{FRAME_PACK_HALT}: frame-pack directory escapes root")
    manifest_path = final_dir / "manifest.json"
    if manifest_path.is_symlink() or not manifest_path.is_file():
        raise RuntimeError(f"{FRAME_PACK_HALT}: frame-pack manifest invalid")
    if manifest_path.resolve(strict=True).parent != final_resolved:
        raise RuntimeError(f"{FRAME_PACK_HALT}: frame-pack manifest escapes directory")
    ...
    require_exact_keys(manifest, manifest_keys, "frame-pack manifest")
    body = dict(manifest)
    claimed = body.pop("payload_sha256")
    if sha256_obj(body) != claimed:
        raise RuntimeError(f"{FRAME_PACK_HALT}: frame-pack manifest payload digest")
    if manifest["schema_version"] != FRAME_PACK_MANIFEST_SCHEMA:
        raise RuntimeError(f"{FRAME_PACK_HALT}: frame-pack schema version")
    for key, value in binding.items():
        if manifest[key] != value:
            raise RuntimeError(f"{FRAME_PACK_HALT}: frame-pack binding {key}")
    if manifest["frame_backend"] not in {"decord", "pyav", "none"}:
        raise RuntimeError(f"{FRAME_PACK_HALT}: frame-pack backend")
    total = manifest["total_frame_indices"]
    if not isinstance(total, int) or isinstance(total, bool) or total < 0:
        raise RuntimeError(f"{FRAME_PACK_HALT}: frame-pack total indices")
    if manifest["requested_indices"] != requested_frame_indices(total):
        raise RuntimeError(f"{FRAME_PACK_HALT}: frame-pack requested indices")
    failed = manifest["frame_decode_failed"]
    if (
        not isinstance(failed, list)
        or len(failed) != NUM_FRAMES
        or any(not isinstance(value, bool) for value in failed)
    ):
        raise RuntimeError(f"{FRAME_PACK_HALT}: frame-pack decode-failure vector")
    rows = manifest["frames"]
    if not isinstance(rows, list) or len(rows) != NUM_FRAMES:
        raise RuntimeError(f"{FRAME_PACK_HALT}: frame-pack frame row count")
    expected_names = {"manifest.json"} | {
        f"{index:02d}.png" for index in range(NUM_FRAMES)
    }
    if {path.name for path in final_dir.iterdir()} != expected_names:
        raise RuntimeError(f"{FRAME_PACK_HALT}: frame-pack directory entries")
    frame_paths: list[Path] = []
    for index, frame_row in enumerate(rows):
        require_exact_keys(
            frame_row, {"index", "filename", "size", "sha256"}, "frame-pack row"
        )
        expected_filename = f"{index:02d}.png"
        if frame_row["index"] != index or frame_row["filename"] != expected_filename:
            raise RuntimeError(f"{FRAME_PACK_HALT}: frame-pack row identity")
        if (
            not isinstance(frame_row["size"], int)
            or isinstance(frame_row["size"], bool)
            or frame_row["size"] <= 0
        ):
            raise RuntimeError(f"{FRAME_PACK_HALT}: frame-pack size")
        if (
            not isinstance(frame_row["sha256"], str)
            or len(frame_row["sha256"]) != 64
            or any(char not in "0123456789abcdef" for char in frame_row["sha256"])
        ):
            raise RuntimeError(f"{FRAME_PACK_HALT}: frame-pack frame SHA-256")
        path = final_dir / expected_filename
        if path.parent != final_dir or path.is_symlink() or not path.is_file():
            raise RuntimeError(f"{FRAME_PACK_HALT}: frame-pack frame path invalid")
        resolved = path.resolve(strict=True)
        if resolved.parent != final_resolved:
            raise RuntimeError(f"{FRAME_PACK_HALT}: frame-pack frame escapes directory")
        followed = path.stat()
        target = resolved.stat()
        if (followed.st_dev, followed.st_ino) != (target.st_dev, target.st_ino):
            raise RuntimeError(f"{FRAME_PACK_HALT}: frame-pack regular identity")
        if followed.st_size != frame_row["size"]:
            raise RuntimeError(f"{FRAME_PACK_HALT}: frame-pack frame size")
        if sha256_file(path) != frame_row["sha256"]:
            raise RuntimeError(f"{FRAME_PACK_HALT}: frame-pack frame hash")
        frame_paths.append(path)
    return manifest, frame_paths, sha256_file(manifest_path)


def load_frame_pack_images(frame_paths: list[Path]) -> list[Any]:
    from PIL import Image
...
def visual_patch_tokens(video_grid_thw: Any) -> int:
    """PRE-merge visual patch tokens for one item, from the processor's grid.

    This is the quantity vision attention is quadratic in.  The v7 OOM
    arithmetic was wrong by 4x precisely because it applied the 2x2 spatial
    merge first; vision SDPA runs before the merger.
    """
    grid = [int(value) for value in video_grid_thw]
    if len(grid) != 3 or any(value <= 0 for value in grid):
        raise RuntimeError(f"{VISUAL_TOKEN_HALT}: malformed video_grid_thw {grid!r}")
    return grid[0] * grid[1] * grid[2]


def assert_visual_token_ceiling(dataset: str, video_id: str, tokens: int) -> None:
    """Fail-closed on the geometry that killed v7, BEFORE the vision tower runs."""
    if not isinstance(tokens, int) or isinstance(tokens, bool) or tokens <= 0:
        raise RuntimeError(f"{VISUAL_TOKEN_HALT}: non-positive visual token count")
    if tokens > VISUAL_PATCH_TOKEN_HARD_CEILING:
        raise RuntimeError(
            f"{VISUAL_TOKEN_HALT}: {dataset}/{video_id} would enter the vision tower "
            f"with {tokens} pre-merge patch tokens against the "
            f"{VISUAL_PATCH_TOKEN_HARD_CEILING} ceiling; refusing before the forward"
        )


def vision_sdpa_fp32_bytes(tokens: int, heads: int = 16) -> int:
    """The score tensor Qwen2.5-VL vision SDPA materialises, in bytes.

    Pinned by the v7 forensic reconciliation: at 43,056 tokens this returns
    exactly the 110.50 GiB the OOM reported.
    """
    return int(tokens) * int(tokens) * int(heads) * 4
...
# The v7 allocation (job 13852) really ran for 1978 s of A100 time before it
# died on the vision OOM, and the campaign accumulator recorded that from sacct.
# The FIRST_TRANCHE phase ceiling is unchanged at 7200 s -- the amendment's
# "aggregate maximum of 2 GPU-hours across both datasets and all C04 jobs" is
# the same clause it always was.  What changes is this tranche's RESERVATION,
# which is the headroom left under that unchanged ceiling: 7200 - 1978 = 5222.
# Every stage that previously asserted `small_cap_gpu_seconds == 7200` now
# asserts this constant instead, and the campaign headroom check is what makes
# the number binding rather than decorative -- reserving 5223 s would be refused
# against the live ledger.
SMALL_TRANCHE_CAP_GPU_SECONDS = 5222
...
def verify_prompt_hash_freeze_payload(payload: dict[str, Any]) -> dict[str, str]:
    ...
    if body.get("run_id") != RUN_ID or body.get("implementation_version") != "v8_prospective":
...
def build_provisional_gpu_usage(
    ...
    record = {
        "schema_version": "c04_provisional_gpu_usage_v8",
        "run_id": RUN_ID,
        "implementation_version": "v8_prospective",
        "lineage": lineage,
...
STAGE_AUTHORIZATION_SCHEMA_RELATIVE = (
    "schemas/c04/c04_a0t_small_v1_v8_stage_authorization.schema.json"
)
...
def _fixture_reconciliation_authorization(provisional_sha256: str) -> dict[str, Any]:
    ...
    manifest = {
        "schema_version": "c04_stage_authorization_v8",
        "run_id": RUN_ID,
        "implementation_version": "v8_prospective",
        "stage": "CPU_POST_JOB_RECONCILIATION",
        "verdict": "GO",
...
def downstream_contract_fixtures() -> list[tuple[str, bool]]:
    ...
    The blind spot these close is the one that let both of v8's first-draft
...
def self_test_fixtures() -> list[tuple[str, bool]]:
    ...
    return (
        ...
        (
            "campaign_cap_strictly_exceeds_the_first_tranche_phase_cap",
            CAMPAIGN_AGGREGATE_CAP_GPU_SECONDS > CAMPAIGN_PHASE_CAPS["FIRST_TRANCHE"],
        ),
        (
            "first_tranche_phase_cap_is_the_amendment_two_gpu_hours",
            CAMPAIGN_PHASE_CAPS["FIRST_TRANCHE"] == 2 * 3600,
        ),
        # Non-vacuous by construction: this runs against the LIVE campaign
        # accumulator, so it asserts that 5222 s is exactly the headroom that
        # remains -- one second more is refused.  If a later job moves the
        # aggregate, this fixture goes red before anything is materialised.
        (
            "small_tranche_cap_is_exactly_the_live_campaign_headroom",
            (
                not _raises_runtime_error(
                    lambda: assert_campaign_aggregate_headroom(
                        SMALL_TRANCHE_CAP_GPU_SECONDS, "self-test headroom probe"
                    )
                )
                and _raises_runtime_error(
                    lambda: assert_campaign_aggregate_headroom(
                        SMALL_TRANCHE_CAP_GPU_SECONDS + 1, "self-test headroom probe"
                    )
                )
            ),
        ),
        (
            "teacher_max_pixels_is_the_deployed_project_cap",
            TEACHER_MAX_PIXELS == 360 * 420 and TEACHER_MAX_PIXELS == 151200,
        ),
        (
            "visual_token_ceiling_is_far_below_the_v7_oom_geometry",
            VISUAL_PATCH_TOKEN_HARD_CEILING == 4096
            # v7's offending item measured (4,138,78) = 43,056 pre-merge tokens.
            and 4 * 138 * 78 == 43056
            and VISUAL_PATCH_TOKEN_HARD_CEILING * 10 < 43056,
        ),
        (
            "visual_patch_tokens_reproduces_the_v7_oom_grid",
            visual_patch_tokens([4, 138, 78]) == 43056
            and visual_patch_tokens((4, 36, 20)) == 2880,
        ),
        (
            "visual_patch_tokens_rejects_a_malformed_grid",
            _raises_runtime_error(lambda: visual_patch_tokens([4, 138]))
            and _raises_runtime_error(lambda: visual_patch_tokens([4, 0, 78])),
        ),
        # The OOM arithmetic, pinned as a known answer.  110.50 GiB is the
        # figure job 13852 reported; this is the closed form that reproduces it.
        (
            "vision_sdpa_fp32_bytes_reproduces_the_110_50_gib_allocation",
            round(vision_sdpa_fp32_bytes(43056) / (1024 ** 3), 2) == 110.50,
        ),
        (
            "vision_sdpa_at_the_ceiling_fits_one_gibibyte",
            round(
                vision_sdpa_fp32_bytes(VISUAL_PATCH_TOKEN_HARD_CEILING) / (1024 ** 3), 2
            )
            == 1.00,
        ),
        (
            "visual_token_ceiling_accepts_measured_and_refuses_the_v7_geometry",
            not _raises_runtime_error(
                lambda: assert_visual_token_ceiling("HateMM", "x", 3072)
            )
            and _raises_runtime_error(
                lambda: assert_visual_token_ceiling("MHC_zh", "x", 43056)
            )
            and _raises_runtime_error(
                lambda: assert_visual_token_ceiling(
                    "HateMM", "x", VISUAL_PATCH_TOKEN_HARD_CEILING + 1
                )
            ),
        ),
        (
            "requested_frame_indices_is_the_frozen_rule",
            requested_frame_indices(0) == []
            and requested_frame_indices(8) == [0, 1, 2, 3, 4, 5, 6, 7]
            and requested_frame_indices(1) == [0] * NUM_FRAMES
            and requested_frame_indices(917)
            == [57, 171, 286, 401, 515, 630, 745, 859],
        ),
        (
            "frame_pack_binding_key_set_is_preflight_computable",
            set(FRAME_PACK_BINDING_KEYS).isdisjoint(
                {
                    "payload_review_sha256",
                    "gpu_execution_authorization_sha256",
                    "allocation_claim_sha256",
            ...
            and "code_resource_authorization_sha256" in FRAME_PACK_BINDING_KEYS
            and "teacher_max_pixels" in FRAME_PACK_BINDING_KEYS,
        ),
