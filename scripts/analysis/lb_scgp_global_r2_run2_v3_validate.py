...
def resource_and_run_check(cfg: dict[str, object], run_id: str) -> dict[str, object]:
    ...
    if cfg["run"]["artifact_path"] != "artifacts/lb_scgp_global/v3/m0/synth_kkt/manifest.json":
...
def main() -> int:
    ...
    result = {
        "schema_version": "lb_scgp_global_r2_run2_v3_validation_v1",
        "run_id": args.run_id,
        "status": status,
        "checks": checks,
        "validator_sha256": sha256_file("scripts/analysis/lb_scgp_global_r2_run2_v3_validate.py"),
