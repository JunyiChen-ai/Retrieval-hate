...
PAYLOAD_SCHEMA_ID = "scgp_global_synth_kkt_payload_v1"
CASE_SCHEMA_ID = "scgp_global_synth_kkt_case_v1"
...
def structural_forced_solution(
    q: np.ndarray,
    g0: np.ndarray,
    ...
    """Construct a moved feasible point with exact equality-KKT closure.

    The forcing direction is derived from the consensus structural target
    vech(M_Q(K_C))-vech(M_Q(G0)).  We choose nu along that direction, move G by
    the off-diagonal part of A^T nu, and set b so r=A(G*)-b=-nu/lambda.
    Diagonal equality normals close the unit-diagonal stationarity entries.
    """
    q_rank = int(q.shape[1])
    if q_rank == 0:
        r0 = np.zeros(0, dtype=np.float64)
        return g0.copy(), r0, vech(structural_moment(q, g0)), r0, np.zeros_like(g0)
    target_delta = vech(structural_moment(q, k_consensus)) - vech(structural_moment(q, g0))
    ...
    for _ in range(20):
        ...
        movement = adj - np.diag(np.diag(adj))
        g_star = g0 + movement
        g_star = 0.5 * (g_star + g_star.T)
        np.fill_diagonal(g_star, 1.0)
        eig_min = float(np.min(np.linalg.eigvalsh(g_star)))
        off_max = float(np.max(np.abs(g_star - np.eye(g_star.shape[0]))))
        if eig_min >= 1e-5 and off_max <= 1.0 - 1e-4:
            r_struct = -nu / float(lambda_struct)
            b_struct = vech(structural_moment(q, g_star)) - r_struct
            return g_star, r_struct, b_struct, nu, adj
        scale *= 0.5
    raise RuntimeError("could not construct feasible moved structural solution")
...
def verify_machine_run2(cfg: dict[str, Any], ledger: AccessLedger) -> dict[str, Any]:
    ...
    run = machine["runs"][1]
    assert_equal(machine["run_order"][1], RUN2, "machine run order[1]")
    assert_equal(run["run_id"], RUN2, "machine Run2 id")
    assert_equal(run["artifact_paths"], [cfg["run"]["artifact_path"]], "machine Run2 artifact path")
    assert_equal(run["artifact_schema_ids"], [PAYLOAD_SCHEMA_ID], "machine Run2 schema")
    assert_equal(run["slurm"], cfg["run"]["slurm"], "machine Run2 slurm")
    assert_equal(run["dependencies"], [RUN1], "machine Run2 dependency")
...
def build_source_manifest(cfg: dict[str, Any], ledger: AccessLedger) -> dict[str, Any]:
    ...
    cfg_hash = ledger.hash_file("configs/lb_scgp_global_r2/m0_synth_kkt_v1.json", "run2_config_hash", "schema_or_source")
    ...
    relevant_paths = cfg["implementation_files"] + [
        "AGENTS.md",
        "refine-logs/lb_scgp_global/FINAL_PROPOSAL.md",
        "refine-logs/lb_scgp_global/EXPERIMENT_PLAN.md",
        "refine-logs/lb_scgp_global/EXPERIMENT_PLAN.machine.json",
        "refine-logs/lb_scgp_global/EXPERIMENT_PLAN_REVIEW_R2.md",
        "refine-logs/lb_scgp_global/EXPERIMENT_PLAN_REVIEW_R2.machine.json",
        "refine-logs/lb_scgp_global/EXPERIMENT_TRACKER.md",
        "refine-logs/lb_scgp_global/M0_IMPLEMENTATION_HANDOFF.md",
        "refine-logs/lb_scgp_global/M0_CONTRACT_FREEZE_EXECUTION.md",
        "refine-logs/lb_scgp_global/M0_CONTRACT_FREEZE_INDEPENDENT_REVIEW.md",
    ]
    manifest = {
        "schema_version": "lb_scgp_global_r2_run2_source_manifest_v1",
        "run_id": RUN2,
...
def structural_case_from_replicas(
    ...
    if mode == "remove_null":
        ...
        gram, r_struct, b_struct, structural_nu, structural_adjoint_matrix = structural_forced_solution(
            q=q,
            g0=g0,
...
def top_level_kkt_from_case(case: dict[str, Any], cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    m = int(r_struct.size)
    movement = gram - g0
    diag_dual = np.diag(structural_adjoint_matrix)
    equality_normal_g = -structural_adjoint_matrix
    diagonal_normal_g = np.diag(diag_dual)
    normal_g = equality_normal_g + diagonal_normal_g
    ...
    return {
        ...
        "affine_normals": {
            ...
            "normal_G_fro_norm": floatify(normal_g_norm),
        ...
        "psd_normal": {
            "present": True,
            "S_psd": matrix_to_list(np.zeros_like(gram)),
            "normal_contribution_sign": "v_psd=-S_psd",
            "dual_lambda_min": 0.0,
            "complementarity_trace": 0.0,
        },
        "halfspace_normals": {
            "present": True,
            "robust_constraints_enabled": False,
