...
CELLS = [
    ("HateMM", {
        ...
        "mntp": "Qwen2.5-VL-7B-Instruct-LoRA-curric-bidir-mntp_HF",
    }),
    ("MHC_zh", {
        ...
        "mntp": "Qwen2.5-VL-7B-Instruct-LoRA-bidir-mntp_HF",
    }),
]
ARMS = ("causal", "bidir", "meanpool", "textpool", "mntp")
LABEL = {"causal": "causal", "bidir": "bidir-lasttoken",
         "meanpool": "S1 bidir-MEANPOOL", "textpool": "S1b bidir-TEXTPOOL", "mntp": "S2a bidir-MNTP"}
...
def run(ds, tags):
    ...
    b, tp = feats["bidir"], feats["mntp"]
    out["drift_mntp_vs_bidir"] = {
        "train_img": cos(tp[0], b[0]), "train_text": cos(tp[1], b[1]),
        "dev_img": cos(tp[2], b[2]), "dev_text": cos(tp[3], b[3]),
    }
    c = feats["causal"]
    out["drift_mntp_vs_causal"] = {
        "train_img": cos(tp[0], c[0]), "train_text": cos(tp[1], c[1]),
        "dev_img": cos(tp[2], c[2]), "dev_text": cos(tp[3], c[3]),
    }
    out["img_nullop_belt"] = {
        "train_cos": out["drift_mntp_vs_bidir"]["train_img"],
        "dev_cos": out["drift_mntp_vs_bidir"]["dev_img"],
        "bar": 0.9999,
        "pass": None,  # S2a: img is READ, not asserted (§6d.4 belt 3)
    }
    return out
...
if __name__ == "__main__":
    ...
    for ds, tags in CELLS:
        ...
        for name in ("img", "text", "concat"):
            ...
            d_b = r["mntp|{}".format(name)][0] - r["bidir|{}".format(name)][0]
            d_c = r["mntp|{}".format(name)][0] - r["causal|{}".format(name)][0]
            d_s1 = r["mntp|{}".format(name)][0] - r["textpool|{}".format(name)][0]
            print("{:8s} {:20s} vs bidir {:+.4f} | vs causal {:+.4f} | vs S1b {:+.4f}".format(
                name, "S2a DELTAS", d_b, d_c, d_s1))

        # --- BELT 1 (declared in advance): stream collapse ---
        sc = r["stream_collapse"]
        tp_cos = max(sc["mntp"]["train"], sc["mntp"]["dev"])
        ...
        print("img DRIFT read (S2a img vs banked bidir img; NOT asserted -- MNTP touches the decoder): train {:.6f} dev {:.6f} "
        ...
        print("drift S2a-vs-causal:", r["drift_mntp_vs_causal"])
        print("drift S2a-vs-bidir :", r["drift_mntp_vs_bidir"])
        blob[ds]["ks_mntp_1"] = {"text_acc": acc, "bars": bar, "recovery_fraction": rec,
                                 "verdict": verdict}
        ks0b_t = min(r["drift_mntp_vs_bidir"]["train_text"], r["drift_mntp_vs_bidir"]["dev_text"])
        ks0b_max = max(r["drift_mntp_vs_bidir"]["train_text"], r["drift_mntp_vs_bidir"]["dev_text"])
        print("KS-MNTP-0b adapter-not-null-op: cos(S2a text, plain-bidir text) max {:.6f} "
              "bar < 0.9999 -> {}".format(ks0b_max, "PASS" if ks0b_max < 0.9999 else "FAIL (ADAPTER DID NOT LOAD)"))
        blob[ds]["ks_mntp_0b"] = {"max_text_cos_vs_bidir": ks0b_max, "bar": 0.9999,
                                  "pass": bool(ks0b_max < 0.9999)}
        blob[ds]["collapse_belt"] = {"bar": COLLAPSE_BAR, "s1b_worst": tp_cos,
                                     "pass": collapse_pass}

    p = os.path.join(os.path.dirname(os.path.abspath(__file__)), "mntp_s2a_devscreen_OUT.json")
    with open(p, "w") as fh:
