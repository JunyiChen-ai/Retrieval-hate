...
from lb_scgp_global_r2_run2_v4_common import (  # noqa: E402
    PAYLOAD_SCHEMA_ID,
    RUN2,
    AccessLedger,
    assert_equal,
    ...
    validate_payload_and_cases_against_schema,
...
def verify_config_and_schema(cfg: dict[str, Any]) -> None:
    ...
    assert_equal(cfg["run"]["artifact_path"], "artifacts/lb_scgp_global/v4/m0/synth_kkt/manifest.json", "artifact path")
...
def main() -> int:
    ...
    case_schema_sha = sha256_file(cfg["paths"]["case_schema"])
    ...
    manifest: dict[str, Any] = {
        "schema_version": "lb_scgp_global_r2_synth_kkt_manifest_v4",
        ...
        **kkt,
        ...
        "injection_results_expected": {
            ...
            "rank_failure": "REJECT",
            "bogus_case_movement_metrics": "REJECT",
            "bogus_case_primal_residual": "REJECT",
            "bogus_case_rank_audit": "REJECT",
            "bogus_case_factor_replay": "REJECT",
            "bogus_case_robust_coverage": "REJECT",
            "bogus_rank_failure_audit": "REJECT",
            "finite_vi_only_attempted_acceptance": "REJECT",
            "identity_no_movement_claims_full": "REJECT",
            "malformed_normal_residual": "REJECT",
            "malformed_normal_presence": "REJECT",
            "malformed_complementarity": "REJECT",
            "malformed_dual_status": "REJECT",
            "malformed_stationarity_status": "REJECT",
            "malformed_finite_vi_diagnostic": "REJECT",
        },
        "gold_isolation": {
        ...
        "hashes": {
            ...
            "case_schema_sha256": case_schema_sha,
    ...
    validate_payload_and_cases_against_schema(manifest, cfg["paths"]["payload_schema"], cfg["paths"]["case_schema"])
    if manifest["acceptance"]["producer_status"] != "PASS_CANDIDATE":
        raise RuntimeError("producer refuses to publish failing candidate")
    if (
        manifest["stationarity"]["status"] != "PASS"
        or manifest["dual_feasibility"]["status"] != "PASS"
        or manifest["complementarity"]["status"] != "PASS"
    ):
        raise RuntimeError("producer refuses to publish failing serialized KKT certificate")
