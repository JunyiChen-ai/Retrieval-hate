...
PAYLOAD_SCHEMA_ID = "scgp_global_synth_kkt_payload_v3"
CASE_SCHEMA_ID = "scgp_global_synth_kkt_case_v3"
...
def validate_payload_and_cases_against_schema(
    ...
    try:
        ...
        raise RuntimeError("jsonschema dependency unavailable; refusing to validate Run2-v3 payload") from exc
    ...
    try:
        ...
        raise RuntimeError(f"Run2-v3 JSON Schema is invalid: {exc.message}") from exc
...
def verify_machine_run2(cfg: dict[str, Any], ledger: AccessLedger) -> dict[str, Any]:
    ...
    assert_equal(run["run_id"], RUN2, "machine Run2-v3 id")
    assert_equal(run["artifact_paths"], [cfg["run"]["artifact_path"]], "machine Run2-v3 artifact path")
    assert_equal(run["artifact_schema_ids"], [PAYLOAD_SCHEMA_ID], "machine Run2-v3 schema")
    assert_equal(run["slurm"], cfg["run"]["slurm"], "machine Run2-v3 slurm")
    assert_equal(run["dependencies"], [RUN1, RUN2_V1], "machine Run2-v3 dependency")
...
def build_source_manifest(cfg: dict[str, Any], ledger: AccessLedger) -> dict[str, Any]:
    ...
    cfg_hash = ledger.hash_file("configs/lb_scgp_global_r2/m0_synth_kkt_v3.json", "run2_config_hash", "schema_or_source")
    ...
    manifest = {
        "schema_version": "lb_scgp_global_r2_run2_v3_source_manifest_v1",
        "run_id": RUN2,
