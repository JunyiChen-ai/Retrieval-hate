...
from c04_a0t_small_v1_v3_common import (  # noqa: E402
    ADDITIVE_INPUT_DIM,
    ARTIFACT_ROOT,
    CONFIG_RELATIVE,
    DATASETS,
...
class AccessAudit:
    ...
    def snapshot(self) -> dict[str, Any]:
        return {
            "schema_version": "c04_guarded_access_audit_v3",
...
def verify_authorization(cfg: dict[str, Any]) -> None:
    ...
    assert_equal(cfg["run"]["implementation_version"], "v3_prospective", "implementation")
...
def expected_frame_pack_binding(
    cfg: dict[str, Any],
    dataset: str,
    ...
    return {
        "run_id": RUN_ID,
        "implementation_version": "v3_prospective",
        "dataset": dataset,
...
def strict_validate_frame_pack(
    ...
    if final_dir.is_symlink() or not final_dir.is_dir():
        raise RuntimeError("HALT_INVALID_FREEZE: frame-pack directory invalid")
    ...
    if final_resolved.parent != pack_root_resolved:
        raise RuntimeError("HALT_INVALID_FREEZE: frame-pack directory escapes root")
    manifest_path = final_dir / "manifest.json"
    if manifest_path.is_symlink() or not manifest_path.is_file():
        raise RuntimeError("HALT_INVALID_FREEZE: frame-pack manifest invalid")
    if manifest_path.resolve(strict=True).parent != final_resolved:
        raise RuntimeError("HALT_INVALID_FREEZE: frame-pack manifest escapes directory")
    with manifest_path.open("r", encoding="utf-8") as handle:
        manifest = json.load(handle)
    manifest_keys = {
        "schema_version",
        *expected_frame_pack_binding(cfg, dataset, input_row, lineage).keys(),
        "frame_backend",
        "total_frame_indices",
        "requested_indices",
        "frame_decode_failed",
    ...
    assert_equal(manifest["schema_version"], "c04_frame_pack_manifest_v3", "frame schema")
    for key, value in expected_frame_pack_binding(
        cfg, dataset, input_row, lineage
    ).items():
        assert_equal(manifest[key], value, f"frame-pack binding {key}")
    if manifest["frame_backend"] not in {"decord", "pyav", "none"}:
        raise RuntimeError("HALT_INVALID_FREEZE: frame-pack backend")
    total = manifest["total_frame_indices"]
    if not isinstance(total, int) or isinstance(total, bool) or total < 0:
        raise RuntimeError("HALT_INVALID_FREEZE: frame-pack total indices")
    expected_requested = (
        []
        if total == 0
        else [
    ...
    assert_equal(
        manifest["requested_indices"],
        expected_requested,
        "frame-pack requested indices",
    )
    ...
    if (
        ...
        raise RuntimeError("HALT_INVALID_FREEZE: frame-pack decode-failure vector")
    rows = manifest["frames"]
    if not isinstance(rows, list) or len(rows) != NUM_FRAMES:
        raise RuntimeError("HALT_INVALID_FREEZE: frame-pack frame row count")
    expected_names = {"manifest.json"} | {
        f"{index:02d}.png" for index in range(NUM_FRAMES)
    }
    actual_names = {path.name for path in final_dir.iterdir()}
    assert_equal(actual_names, expected_names, "frame-pack directory entries")
    ...
    for index, frame_row in enumerate(rows):
        ...
        assert_equal(frame_row["filename"], expected_filename, "frame-pack filename")
        if (
            not isinstance(frame_row["size"], int)
            or isinstance(frame_row["size"], bool)
            or frame_row["size"] <= 0
        ):
            raise RuntimeError("HALT_INVALID_FREEZE: frame-pack size")
        if (
            ...
            raise RuntimeError("HALT_INVALID_FREEZE: frame-pack frame SHA-256")
        path = final_dir / expected_filename
        if path.parent != final_dir or path.is_symlink() or not path.is_file():
            raise RuntimeError("HALT_INVALID_FREEZE: frame-pack frame path invalid")
        resolved = path.resolve(strict=True)
        if resolved.parent != final_resolved:
            raise RuntimeError("HALT_INVALID_FREEZE: frame-pack frame escapes directory")
        followed = path.stat()
        target = resolved.stat()
        if (followed.st_dev, followed.st_ino) != (target.st_dev, target.st_ino):
            raise RuntimeError("HALT_INVALID_FREEZE: frame-pack regular identity")
        assert_equal(followed.st_size, frame_row["size"], "frame-pack frame size")
...
def load_or_create_frame_pack(
    ...
    final_dir = pack_root / video_id
    manifest_path = final_dir / "manifest.json"
    expected_binding = expected_frame_pack_binding(cfg, dataset, input_row, lineage)
    ...
    try:
        ...
        manifest = {
            "schema_version": "c04_frame_pack_manifest_v3",
            **expected_binding,
        ...
        raise
    validated, _, manifest_sha256 = strict_validate_frame_pack(
        cfg, dataset, input_row, lineage
    )
    return {
        **decoded,
        "manifest_sha256": manifest_sha256,
        "frame_sha256": [row["sha256"] for row in validated["frames"]],
        "created": True,
    }
...
def canonicalize_dataset(
    ...
    applicability_payload = {
        "schema_version": "c04_fallback_applicability_v3",
        "run_id": RUN_ID,
        "dataset": dataset,
    ...
    for prepared_row in prepared:
        ...
        record = {
            "schema_version": "c04_a0t_small_canonical_record_v3",
            "run_id": RUN_ID,
            "dataset": dataset,
            "video_id": video_id,
...
def idempotent_complete(
    ...
    assert_equal(manifest["implementation_version"], "v3_prospective", "seal implementation")
...
def main() -> int:
    ...
    for dataset in DATASETS:
        ...
        for rank, input_row in enumerate(inputs[dataset]):
            ...
            for prompt_form in PROMPT_FORMS:
                ...
                record = {
                    "schema_version": "c04_a0t_small_prompt_record_v3",
                    "run_id": RUN_ID,
                    "dataset": dataset,
                    "video_id": video_id,
    ...
    access = {
        "schema_version": "c04_access_ledger_v3",
        "run_id": RUN_ID,
        "implementation_version": "v3_prospective",
    ...
    provisional = {
        "schema_version": "c04_provisional_gpu_usage_v3",
        "run_id": RUN_ID,
        "implementation_version": "v3_prospective",
        "lineage": lineage,
    ...
    seal = {
        "schema_version": "c04_a0t_small_seal_manifest_v3",
        "run_id": RUN_ID,
        "implementation_version": "v3_prospective",
        "lineage": lineage,
