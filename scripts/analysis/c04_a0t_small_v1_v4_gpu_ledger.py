#!/usr/bin/env python
"""Persistent single-allocation GPU ledger for C04-A0T-SMALL-v1 impl-v4.
...
def validate_ledger(ledger: dict[str, Any], cfg: dict[str, Any]) -> None:
    ...
    assert_equal(ledger["schema_version"], "c04_gpu_ledger_v4", "ledger schema")
    assert_equal(ledger["run_id"], RUN_ID, "ledger run id")
    assert_equal(
        ledger["implementation_version"], "v4_prospective", "ledger implementation"
    )
...
def verify_reconciliation_lineage(cfg: dict[str, Any]) -> dict[str, str]:
    ...
    assert_equal(claim["schema_version"], "c04_allocation_claim_v4", "claim schema")
    ...
    assert_equal(marker["schema_version"], "c04_allocation_entry_marker_v4", "marker schema")
...
def create_entry_marker(cfg: dict[str, Any], job_id: str, start_uptime: int) -> None:
    ...
    marker = {
        "schema_version": "c04_allocation_entry_marker_v4",
        "run_id": RUN_ID,
        "slurm_job_id": job_id,
...
def claim(cfg: dict[str, Any], start_uptime: int) -> int:
    ...
    with lock_path.open("a+b") as lock_handle:
        ...
        if not ledger["jobs"] and claim_path.exists():
            ...
            if not consumption_path.exists():
                exclusive_publish_json(cfg["paths"]["resource_consumption"], {
                    "schema_version": "c04_resource_ticket_consumption_v4",
                    "run_id": RUN_ID,
                    "slurm_job_id": job_id,
        ...
        assert_equal(ticket["schema_version"], "c04_resource_ticket_v4", "ticket schema")
        ...
        claim_record = {
            "schema_version": "c04_allocation_claim_v4",
            "run_id": RUN_ID,
            "slurm_job_id": job_id,
            "allocation_entry_uptime_seconds": start_uptime,
...
def publish_or_verify_resource_final_state(
    ...
    job = ledger["jobs"][0]
    immutable = {
        "schema_version": "c04_resource_final_state_v4",
        "run_id": RUN_ID,
