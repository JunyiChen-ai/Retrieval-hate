...
# v2 (M1_CACHE_V1_RESULT_TO_CLAIM_REVIEW.md): the v1 cache runs double-burned because the
# train mp4 files are in-repo symlinks whose targets escape the repo, so canonical_root_path's
# resolve() fired at the video site. v2 = REPLACE-in-place run_ids + symlink-tolerant
# canonical_video_path at the builder/producer video sites. The seal run_id is unchanged.
RUN_MHC = "LBSCGP-GLOBAL-M1-CACHE-MHC-v2"
RUN_MHC_ZH = "LBSCGP-GLOBAL-M1-CACHE-MHC_zh-v2"
...
def canonical_video_path(rel: Path | str, dataset: str) -> Path:
    """Symlink-tolerant containment guard for a train-video mp4 (v2 fix).

    The train mp4s are in-repo SYMLINKS whose targets legitimately live OUTSIDE the repo
    (the external Multihateclip/HateMM corpus); a repo-ROOT resolve() containment guard
    mis-fires on them -- this is the v1 double-burn root cause
    (M1_CACHE_V1_RESULT_TO_CLAIM_REVIEW.md). This guard enforces containment on the symlink
    LOCATION (the un-resolved relative path), NOT on the resolved target:

      - `rel` has no '..' component;
      - `rel` is under the preregistered per-dataset video root `data/video/<dataset>/All/`;
      - the LOCATION (ROOT/rel, normalized lexically, WITHOUT following symlinks) stays
        under ROOT, and (if it exists) its PARENT directory resolves in-repo -- so only the
        mp4 leaf may be a link, never a parent;
      - (if it exists) the leaf is a regular file OR a symlink (lstat; no follow).

    Returns the in-repo LOCATION path for OS-level decode/hash, which follows the symlink at
    the OS layer (as the smoke empirically proved works). The resolved external target is
    permitted by design and is recorded for audit by the access ledger. The allowlist /
    FORBIDDEN_TOKENS still run on `rel`, so no val/test/label path becomes reachable and the
    forbidden zero-counters stay 0. No isolation weakening: the only relaxation is that the
    mp4's resolved target need not be under ROOT.
    """
    root = ROOT.resolve()
    rel_path = Path(rel)
    if ".." in rel_path.parts:
        raise RuntimeError(f"video path has '..' traversal: {rel}")
    if rel_path.is_absolute():
        raise RuntimeError(f"video path must be repo-relative: {rel}")
    rel_str = rel_path.as_posix()
    video_root = f"data/video/{dataset}/All/"
    if not rel_str.startswith(video_root):
        raise RuntimeError(f"video path not under allowlisted root {video_root}: {rel}")
    location = Path(os.path.normpath(str(root / rel_str)))
    if location != root and not str(location).startswith(str(root) + os.sep):
        raise RuntimeError(f"video path location escapes repo: {rel}")
    if os.path.lexists(location):
        parent_real = Path(os.path.realpath(location.parent))
        if parent_real != root:
            try:
                parent_real.relative_to(root)
            except ValueError as exc:
                raise RuntimeError(f"video parent dir escapes repo: {rel}") from exc
        mode = os.lstat(location).st_mode
        if not (stat.S_ISREG(mode) or stat.S_ISLNK(mode)):
            raise RuntimeError(f"video path is neither regular file nor symlink: {rel}")
    return location
...
class TrainEvidenceAccessLedger:
    ...
    def note_video_read(self, path: Path | str, dataset: str) -> Path:
        """A single train-video read (frame decode or byte hash); symlink-tolerant (v2).

        Containment is enforced on the mp4 SYMLINK LOCATION via canonical_video_path
        (the resolved external target is allowed by design and recorded for audit); the
        allowlist/FORBIDDEN_TOKENS classifier still runs on the relative location, so
        forbidden_path_read_count / non_allowlisted_train_content_read_count stay 0 and no
        val/test/label path becomes reachable.
        """
        fs_path = canonical_video_path(path, dataset)          # symlink-tolerant location guard
        rel = Path(path).as_posix()
        reason = forbidden_reason(rel, self.allowlist | {rel})
        if reason is not None:
            self.counters["forbidden_path_read_count"] += 1
            raise RuntimeError(f"access ledger refuses video {rel}: {reason}")
        followed_target = os.path.realpath(fs_path)            # OS-followed target (may be out-of-repo)
        self.authorized_train_evidence_read_count += 1
        self.records.append({
            "kind": "train_video_read",
            "dataset": dataset,
            "path": rel,
            "is_symlink": os.path.islink(fs_path),
            "followed_target": followed_target,
            "followed_target_in_repo": followed_target == str(ROOT.resolve())
            or followed_target.startswith(str(ROOT.resolve()) + os.sep),
        })
        return fs_path
