...
import lb_scgp_global_r2_m1_evidence_pack_v2 as evpack  # noqa: E402
...
def main() -> int:
    ...
    for pack_sha, vids in sorted(dedup.items()):
        ...
        pack = packs[rep_vid]
        # v2 fix: symlink-tolerant containment on the mp4 location (v1 burned here on the
        # canonical_root_path resolve() of the external-escaping symlink).
        video_fs = canonical_video_path(pack["video_relpath"], args.dataset)
        frames = None
        if video_fs.exists():
            ledger.note_video_read(pack["video_relpath"], args.dataset)   # records followed target
    ...
    builder_sha = sha256_file(canonical_root_path("scripts/analysis/lb_scgp_global_r2_m1_evidence_pack_v2.py")[0])
    common_sha = sha256_file(canonical_root_path("scripts/analysis/lb_scgp_global_r2_m1_cache_v2_common.py")[0])
    ph = prompt_hash()
