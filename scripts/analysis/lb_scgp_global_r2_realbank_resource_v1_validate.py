#!/usr/bin/env python
"""SLURM-only realbank preflight validator (runs before the producer)."""
...
from lb_scgp_global_r2_realbank_resource_v1_common import (  # noqa: E402
    CONFIG_PATH,
    DATASETS,
    EXPECTED_N,
    RUN3,
...
def verify_run1_hashes(cfg: dict[str, object]) -> dict[str, object]:
    ...
    if old_hash != expected_old["manifest_sha256"] or old_count != expected_old["path_count"]:
        mismatches.append({"path": "old_protected_scope", "expected": expected_old,
                           "actual": {"manifest_sha256": old_hash, "path_count": old_count}})
    return {"status": "PASS" if not mismatches else "FAIL", "mismatches": mismatches}


def verify_authoritative_hashes(cfg: dict[str, object]) -> dict[str, object]:
    mismatches = []
...
def verify_train_bank_bindings(cfg: dict[str, object]) -> dict[str, object]:
    mismatches = []
    for dataset in DATASETS:
        bank = cfg["train_banks"][dataset]
        actual = sha256_file(ROOT / bank["path"])
        if actual != bank["sha256"]:
            mismatches.append({"dataset": dataset, "path": bank["path"], "expected": bank["sha256"], "actual": actual})
        if int(bank["train_n"]) != EXPECTED_N[dataset]:
            mismatches.append({"dataset": dataset, "train_n": bank["train_n"], "expected_n": EXPECTED_N[dataset]})
    return {"status": "PASS" if not mismatches else "FAIL", "mismatches": mismatches}
...
def python_dependency_check() -> dict[str, object]:
    script = (
        ...
        "names=['numpy','torch','jsonschema']\n"
        "missing=[name for name in names if importlib.util.find_spec(name) is None]\n"
        "print(json.dumps({'checked':names,'missing':missing}))\n"
        "sys.exit(1 if missing else 0)\n"
    )
    result = run_command([sys.executable, "-c", script])
    result["check"] = "python_dependency_numpy_torch_jsonschema"
    return result


def resource_and_run_check(cfg: dict[str, object], run_id: str) -> dict[str, object]:
    errors = []
    if run_id != RUN3 or cfg["run"]["run_id"] != RUN3:
        errors.append("run_id")
    if cfg["run"]["schema_id"] != SCHEMA_ID:
        errors.append("schema_id")
    if cfg["run"]["artifact_path"] != "artifacts/lb_scgp_global/v1/m0/realbank_resource/decision.json":
        errors.append("artifact_path")
    if cfg["run"]["slurm"] != {"cpu": 16, "ram_gb": 96, "gpu": 0, "env": "HateVideo", "no_time_flag": True}:
        errors.append("slurm_resource")
    if cfg["authorization"]["authorized_run_ids"] != [RUN3]:
...
def main() -> int:
    ...
    if args.config != CONFIG_PATH:
        raise RuntimeError(f"unexpected config path {args.config}")
    ...
    checks.append(schema_strict_check([cfg["paths"]["payload_schema"]]))
    ...
    checks.append(verify_authoritative_hashes(cfg))
    checks.append(verify_train_bank_bindings(cfg))
    checks.append(no_clobber_check(cfg))
    checks.append(resource_and_run_check(cfg, args.run_id))
    status = "PASS" if all(item["status"] == "PASS" for item in checks) else "FAIL"
    result = {
        "schema_version": "lb_scgp_global_r2_realbank_resource_validation_v1",
        "run_id": args.run_id,
        "status": status,
        "checks": checks,
        "validator_sha256": sha256_file("scripts/analysis/lb_scgp_global_r2_realbank_resource_v1_validate.py"),
