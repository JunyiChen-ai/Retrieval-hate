...
readonly PRODUCER_SCRIPT=scripts/analysis/c04_a0t_small_v1_v8_producer.py
readonly LOCK_PATH=artifacts/c04/a0t_small_v1_impl_v8/resource/serial_gpu.lock
readonly BREACH_RECORD=artifacts/c04/a0t_small_v1_impl_v8/resource/budget_breach.json
...
readonly PREFLIGHT_MANIFEST=artifacts/c04/a0t_small_v1_impl_v8/freeze/preflight_manifest.json
if [[ ! -f "$PREFLIGHT_MANIFEST" ]]; then
  echo "HALT_REVIEW_LINEAGE: no frozen preflight manifest; refusing to enter the namespace" >&2
  exit 2
fi

readonly ENTRY_MARKER=artifacts/c04/a0t_small_v1_impl_v8/resource/allocation_entry_marker.json
...
readonly SEAL=artifacts/c04/a0t_small_v1_impl_v8/seal/seal_manifest.json
jq -e '
  .terminal_state == "SEALED_PRELABEL_RELIABILITY_PASS"
  or .terminal_state == "KILL_C04_TEACHER_SEMANTIC_RELIABILITY"
' "$SEAL" >/dev/null
echo "C04 impl-v8 pre-label seal complete; terminal CPU sacct reconciliation is still required."
