...
RUN_ID=${RUN_ID:-LBSCGP-GLOBAL-G0-M0-SYNTH-KKT-v2}
CONFIG=${CONFIG:-configs/lb_scgp_global_r2/m0_synth_kkt_v2.json}
EXPECTED=LBSCGP-GLOBAL-G0-M0-SYNTH-KKT-v2
ARTIFACT_ROOT=artifacts/lb_scgp_global/v2/m0/synth_kkt
COMPLETE=0
VALIDATION_JSON=""
PROSPECTIVE_OUTPUTS=(
...
if [[ "$CONFIG_ARTIFACT" != "artifacts/lb_scgp_global/v2/m0/synth_kkt/manifest.json" ]]; then
  echo "Config artifact path mismatch: $CONFIG_ARTIFACT" >&2
  exit 2
fi

VALIDATION_JSON=$(mktemp "${TMPDIR:-/tmp}/lbscgp_global_r2_run2_v2_validation.XXXXXX.json")
python scripts/analysis/lb_scgp_global_r2_run2_v2_validate.py \
  --config "$CONFIG" \
  --run-id "$RUN_ID" \
  --json-out "$VALIDATION_JSON"

python scripts/analysis/lb_scgp_global_r2_run2_v2_producer.py \
  --config "$CONFIG" \
  --run-id "$RUN_ID" \
  --validation-json "$VALIDATION_JSON"

python scripts/analysis/lb_scgp_global_r2_run2_v2_independent_verify.py \
  --config "$CONFIG" \
