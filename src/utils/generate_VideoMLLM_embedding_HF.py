...
# embeddings for the RGCL training pipeline (Config MLLM-A, DESIGN_iter1.md §2.1).
#
# It mirrors the exact cache contract produced by generate_VideoCLIP_embedding_HF.py
...
#
# Two streams, both frozen Qwen2.5-VL-7B, bf16, torch.no_grad(), output_hidden_states=True:
#
...
def parse_args_sys(args_list=None):
    arg_parser = argparse.ArgumentParser(
        description="Generate video MLLM (Qwen2.5-VL) hidden-state embeddings in the RGCL cache contract."
    )
    ...
    arg_parser.add_argument(
        ...
        help="The Qwen2.5-VL model (HF id) to use.",
    )
    arg_parser.add_argument(
        "--out_model_tag",
        type=str,
