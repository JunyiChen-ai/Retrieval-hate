# C09 Stage-0 (A0) — preregistration **v7** (round-6 repairs)
...
> ## STATUS: `V7_REPAIRED_NOT_FROZEN_NOT_SUBMITTED` — awaiting fresh independent review.
>
...
Round 6 returned **no Critical** and two Highs, both narrow.

1. **The v6 repair of R5's H-3 was written in the `τ_hi` convention the v5 repair of
   R4's H-1 had already replaced.** §5.2 defined the analysis set with the
   *population* convention (`τ_hi^{(fold(i))}` per item) while §4.3(b) had already
   moved the **fitting target** to the *per-scoring-fold* convention (`τ_hi^{(f)}` for
   every row in that fit). Read literally the two disagree about which rows are
   positives and which are negatives at the co-primary. Both readings are conservative
   and the magnitude is a handful of items, but a frozen preregistration cannot leave
   its estimator's class composition to the implementer. **v7 recomputes the analysis
   set per scoring fold, for both classes.**
2. **§11 omitted F99, the one banked finding written on exactly the object it names.**
   F99 closes the head/key-map class on novelty via GAP-7/D7 and caps the successor's
   set-preserving channel at `+0.0279` (HateMM) / `+0.0470` (ZH) — a closed-form bound
   derived from the same purity marginals this design already cites, and a headwind
   stronger than several v6 registers at length. **v7 adds it to §10 and to §11's
   binding Stage-1 adjudication list.**

Ten further precision and propagation items are corrected: three v6 repairs that landed
in one section and not its sibling (the RNG substreams, the `R(x)` rounding, the
permutation pool), a stale feature count in three places, one arithmetic slip inside
the very re-derivation round 5 demanded, an unbudgeted `GATE-NULL` sensitivity leg, a
six-item provenance bundle, the authorisation line, and a named adjudication of
`banned_constraints[2]` / `hard_constraints[4]`.

**Rounds 3–5's repairs, retained unchanged in v7:**
...
- **CAL-0** restated and scoped: *"The raw train-space arena is not established as
  ...
  **CAL-2 leg (1) is now RUN as a HALT gate** — `GATE-FIXK20` (§8.1). **CAL-2 leg (2)
  ...
  `curves["<DS>/seed<n>/<protocol>"]["test"]` sub-key at `n_test = 215`, and this run
  materialises no test-derived quantity anywhere, not even in a non-gating report.
...
- **`PERM-STRUCT` — the primary (marginal) null, permuted at the ITEM level.**
  `D = 1000` draws. **Pool (R6 I-3):** `π` runs over the **`D-FELDMAN` analysis set
  only** — exactly the items the estimator sees, never all `n` — matching
  `SHUFFLE-POP`'s pinned domain; at `τ_hi`, where the analysis set is `c`-restricted,
  permuting over all `n` would import structural vectors from low-`c` items and change
  the null distribution. Since the analysis set is now per scoring fold, the pool is
  `A^{(f)}` for the fold-`f` refit. **The drawing unit is unambiguous:** per
  `(dataset, τ, draw)` **one** permutation `π` of that pool's **items** is drawn and
  applied
...
- **`PERM-STRUCT-COND` — the conditional null, by WITHIN-STRATUM permutation, and a
  ...
  **nonlinear `BASE` content** as well as its conditional association with the target,
  ...
  > at the item level **within `ITEM-STRATUM`**, over the same pool `A^{(f)}`, one
  > shared `π` across the five columns, applied identically in all three seed cells;
  > `FULL` refit exactly as above.
...
- **`SHUFFLE-POP`.** A **uniform random permutation of the per-item target vector over
  the `D-FELDMAN` analysis set** — `A^{(f)}` per scoring fold (§5.2), exactly the rows
  the estimator sees, not all `n` — **RNG child `2`** (§5.2's frozen substream policy —
...
## 12. Repair ledger — the 12 round-6 findings

| # | round-6 finding | repair | where |
|---|---|---|---|
| **H-1** | §5.2's analysis-set definition used the *population* `τ_hi` convention while §4.3(b)'s fitting target already used the *per-scoring-fold* one, leaving the estimator's class composition under-determined at the co-primary | **the analysis set `A^{(f)}` is recomputed per scoring fold, for both classes**, at `τ_hi^{(f)}` — so class composition, confidence restriction and fitting target share one threshold; §4.3's `P_{τ_hi}` is retained for `\|P_τ\|`, `K-REACH`, `O1` and `NET`'s `k` only; `GATE-NESTED`'s per-item check extended to assert it; realised positive/negative counts emitted per `(dataset, seed, fold, τ)` | §5.2, §8.1 |
| **H-2** | §11 omitted F99, the one banked finding written on the successor's own object class | **F99 added to §11 and §10**: its novelty leg (`"D7 additionally kills the NOVELTY CLAIM regardless of the number (GAP-7 head/key-map class)"`, with GAP-7 at `REDTEAM_BAN_SCOPE_AUDIT.md:303-308`) and its closed-form cap on the set-preserving channel (`≤ +0.0279` HateMM / `≤ +0.0470` ZH, zero-break, from the `6/20`–`7/20` permutation threshold crossed with `ERRPAT_HateMM:143-144` and `ERRPAT_MHC-ZH:222-223`), with test-split provenance and the channel scope carried; **F99 added to precondition (d)'s binding adjudication list** | §10, §11 |
| **I-1** | feature-set cardinalities stale in three places | header corrected to **8 and 13**, `FULL` labelled `= 13`, and both *"12-feature probe"* descriptions corrected to **13-feature** | §5.2, §3.2, §10 |
| **I-2** | the RNG substream repair did not propagate to §6.3 | `SHUFFLE-POP` and `RANDOM-POP` now cite **child `2`** and **child `3`** of §5.2's frozen `.spawn(6)` policy; the bare `default_rng(20260801)` calls are deleted | §6.3 |
| **I-3** | the permutation *pool* was unstated for both nulls | pinned to the **`D-FELDMAN` analysis set only** — `A^{(f)}` for the fold-`f` refit — matching `SHUFFLE-POP`'s pinned domain, with the reason (at `τ_hi` a full-`n` pool would import low-`c` donors and change the null) | §5.2 |
| **I-4** | §9 reverted to the word `round` | §9 now uses **`R(x) ≡ int(numpy.floor(x + 0.5))`** with the §5.3 cross-reference | §9 |
| **I-5** | `56.0/62.33 = 0.898`, stated `0.899`, twice | corrected in §5.3; the ledger row is rewritten here | §5.3, §12 |
| **I-6** | `GATE-NULL`'s remove-null sensitivity had no budget line | **added (~24 min, HateMM only, no re-mint)**; total restated **≈ 115 CPU-minutes** | §2 |
| **I-7** | six provenance items | (a) the `95.03 / 97.75 / 99.45 %` figures re-cited to **`RESTRANS_PREGATE_RECORD.md:409`** (they are not in F96's `ban_scope`, which carries the standing-gate sentence); (b) the F98 epitaph attributed to **the reopen's rendering** in `TARGET_STATE.json`, with F98's own body and `AGGNET_PREGATE_RECORD.md:690` noted as different phrasings; (c) *"self-labelled"* **withdrawn** — the compression is the reopen's at `GATE0_REOPEN_2026-07-31.md:243-244`, and `ERRPAT_HateMM:395-396`'s own words are quoted; (d) three sentences re-attributed from F113 to their real homes — the KILL/PROMOTE clause to **`unified_pilot_gate.arena`** (lower case, as written), and the two measurement sentences to **`directions_tried.json::dead[F113].status`** — with the note that F113's entry carries **no `ban_scope`** and closes *"STANDING RULE PROPOSED (not yet ruled)"*; (e) the nine-arm conclusion re-cited to `:863-864`; (f) the two `C02_A0_OUT.json` reads given their **full paths** | §2, §1, §3.1, §5.5, §6.1, §6.2, §6.3, §8.1 |
| **I-8** | the authorisation line contradicts the record it cites, and one `c09_next_step` precondition was dropped | header now reads **"Promoted by … authorised by nothing yet"** with `what_this_reopen_does_not_do[2]` and *"NOTHING IS AUTHORIZED BY THE REOPEN"* quoted; the **re-review-before-freeze** clause added as precondition 1 of three | header |
| **I-9** | `banned_constraints[2]` / `hard_constraints[4]` never named, though the design's three most central objects are cross-seed aggregates | **named and adjudicated** on three grounds — `[4]`'s own *"as a final performance method"* qualifier, the three seeds being the instrument's replication axis (F113's arena and `GATE-FLOOR`'s three per-seed floors), and `H-L4`'s foreclosure — with the note that a successor averaging seeds at inference **would** engage `[4]`, which §11's precondition (a) already forbids | §3 |
| **I-10** | three residual scoping precisions | (a) CAL-2(2)'s **primary** skip reason is now CAL-2's own *"deliberately no threshold" / "not as a validity gate"*, with the test-split-payload reason narrowed to the `["test"]` sub-key and the existence of a `dev` sub-key acknowledged; (b) `directions_tried.json::queued.headspace_arena_EN`'s **`~15 CPU-min`** price quoted beside the zero-slack statement; (c) the caps written exactly (`131.4667 / 95.6667`, i.e. `k ≥ 132 / 96` dead) and the `28.95` vs `29.0` ZH figures reconciled with a proof that no attainable `mean_s net_s` lies between them | §2, §5.3 |

---

*v7. No hash frozen, no config written, no code implemented, no namespace created, no
