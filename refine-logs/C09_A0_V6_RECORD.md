# C09 Stage-0 (A0) — preregistration **v6** (round-5 repairs)
...
> ## STATUS: `V6_REPAIRED_NOT_FROZEN_NOT_SUBMITTED` — awaiting fresh independent review.
>
...
> (**`REVISE 0C/3H/10I` — the first round with no Critical**) → **this file**. v1–v5 are
> superseded in full and must not be implemented. The v6 repair ledger is §12; each
> earlier version carries its own. R5 audited R1, R2 and R3 as fully discharged and 12
> of R4's 14 as discharged cleanly, including R4's Critical; its three Highs all sit on
> one seam — what the `τ_hi` co-primary and the conditional null actually measure — and
...
Round 5 returned **no Critical**. Its three Highs all sit on one seam — what the `τ_hi`
co-primary and the conditional null actually measure — and two of the three leaned
toward a false CONTINUE.

1. **`PERM-STRUCT-COND`'s residualisation was linear, and the dependence it exists to
   absorb is a censored step function.** Structural feature 1 takes the frozen sentinel
   `21` exactly when the top-20 bank labels are uniform, which is a step in
   `pred_purity` — a `BASE` feature. OLS pushes that censoring into the residual, so
   permuting the residual destroys nonlinear `BASE` content as well as the conditional
   association, and `p_cond` can reject under the very null it was added to protect.
   **v6 replaces the OLS residualisation with a within-stratum permutation**, which is
   an *exact* permutation null for `struct ⊥ target | stratum` and needs no regression
   at all.
2. **The `τ_hi` co-primary is squeezed from two sides the design had numbers for and
   had not multiplied together.** `p_w ≤ |P_τ|`, and `|P_{τ_hi}| ≈ 28` on MHC-ZH is
   already below the `p_w ≥ 30` power floor, so the underpowered tag — which v5 makes
   fatal to a CONTINUE — fires on ZH by arithmetic. v6 computes the cap in-run, marks
   each identifiability cell `LIVE` or `ARITHMETICALLY_DEAD_AT_THIS_POWER`, and
   pre-declares the consequence.
3. **At `τ_hi` the positives were confidence-restricted and the matched negatives were
   not**, so `ΔAUC(τ_hi)` was a different estimand from `ΔAUC(τ_0)` and Holm treated
   them as one family. v6 applies the same out-of-fold threshold to the negative class
   and adds `c_i` to `BASE`, so the definitional quantity is absorbed by the baseline
   the increment is measured against.

Ten further specification and provenance items are corrected, including pinning the RNG
substream policy, the `k` rounding rule, and MHC-EN's actual primary out-of-scope
reason.

**Round 4's four repairs, retained unchanged in v6:**
...
recorded inside the six banked arena outputs, which the on-disk file still matches —
so adding EN would break that pin. Two datasets is what the bar requires.
...
- **CAL-0** restated and scoped: *"The raw train-space arena is not established as
  ...
  **CAL-2 leg (1) is now RUN as a HALT gate** — `GATE-FIXK20` (§8.1). **CAL-2 leg (2)
  ...
  the deployed curve does not change with the arena. **The reason for skipping it is
  that `ksweep_OUT.json`'s per-`k` payload is a TEST-SPLIT read**
  (`curves["<DS>/seed<n>/<protocol>"]["test"]`), and this run materialises no
  test-derived quantity anywhere, not even in a non-gating report. v5 additionally
  offered the clause document's header scope (*"every `$0` pregate run in the raw
  banked train-space arena"*, `PREGATE_CALIBRATION_CLAUSE.md:3`) as a second reason;
  **that reason is withdrawn**, because the same header scopes CAL-0…CAL-5 alike and
  cannot out-scope CAL-2(2) while CAL-0/1/3/5 are treated as binding. CAL-2 itself
  declares leg (2) *"deliberately no threshold"* and *"not as a validity gate"*, so
  nothing gating is lost. **CAL-3** is discharged in §9. **CAL-4** labels every quantity closed-form or
...
**Positives:** `P_τ`. **Negatives:** `CONFIG-MATCHED-CORRECT at τ` = query items
correct at all three seeds **and satisfying the same out-of-fold confidence
restriction**, `c_i ≥ τ_hi^{(fold(i))}` at `τ = τ_hi` (no restriction at `τ = τ_0`).
...
- **`PERM-STRUCT` — the primary (marginal) null, permuted at the ITEM level.**
  `D = 1000` draws. **The drawing unit is unambiguous (R5 I-1a):** per `(dataset, τ,
  draw)` **one** permutation `π` of that dataset's **items** is drawn and applied
...
- **`PERM-STRUCT-COND` — the conditional null, by WITHIN-STRATUM permutation, and a
  ...
  **nonlinear `BASE` content** as well as its conditional association with the target,
  ...
  > at the item level **within `ITEM-STRATUM`**, one shared `π` across the five columns,
  > applied identically in all three seed cells; `FULL` refit exactly as above.
  > `p_cond = (1 + #{d : ΔAUC_d ≥ ΔAUC_obs}) / (D + 1)`.

  **This is an exact permutation null for `struct ⊥ target | ITEM-STRATUM`** — no
  regression, no functional-form assumption, and the conditioning set is the item-level
  image of the very discretisation `AUC_strat` already conditions on. It does not
  condition on the *continuous* `BASE`, and the design says so: **`p_cond` is exact for
  the stratum-conditional null and is scoped to it**, and
  `MARGINAL_ONLY_NOT_CONDITIONAL` (below) is a statement about `ITEM-STRATUM`, not about
  `BASE` in full.
...
recall `≥ 56.0/62.33 = 0.899` and precision `≥ 56.0/83 = 0.675`;
...
concluding at `:864` *"9 of 9 raw positives fail to transfer; the median shrink is >7×;
three invert sign."*
...
- **`p_cond` is exact for the `ITEM-STRATUM`-conditional null, not for conditioning on
  the continuous `BASE`** (§5.2). `MARGINAL_ONLY_NOT_CONDITIONAL` is a statement about
...
## 12. Repair ledger — the 13 round-5 findings

| # | round-5 finding | repair | where |
|---|---|---|---|
| **H-1** | `PERM-STRUCT-COND`'s OLS residualisation does not implement the conditional null: the named dependence (structural feature 1's sentinel is a step in `pred_purity`) lands in the residual, so `p_cond` can reject under the null it protects | **OLS residualisation withdrawn.** `PERM-STRUCT-COND` now permutes the structural block **within `ITEM-STRATUM`** — the item-level analogue of §4.4's row strata (`c_i` tercile × `mean_s pred_purity` bucket, identical frozen edges) — which is an **exact** permutation null for `struct ⊥ target | ITEM-STRATUM`, needs no regression and makes no functional-form assumption; the conditioning set is named and `MARGINAL_ONLY_NOT_CONDITIONAL` is scoped to it | §5.2, §10 |
| **H-2** | `p_w ≤ \|P_τ\|`, and `\|P_{τ_hi}\| ≈ 28` on MHC-ZH is already below the `p_w ≥ 30` floor, so the co-primary is closed on power by arithmetic — undeclared | **closed-form cap and cell marking added**, mirroring §5.3: each `(τ, dataset)` identifiability cell is marked `LIVE` or `ARITHMETICALLY_DEAD_AT_THIS_POWER`; the joint `K-REACH ∧ p_w` requirement on ZH (`\|P_{τ_hi}\| ≥ 30`, i.e. `\|P_0\| ≳ 60` against `≈ 55`) and the HateMM requirement (`≥ 79 %` of positives in two-class strata) are written out; **the KILL record must name a dead cell as unreachable, not tested-and-failed**; §9 adds the cell-liveness condition to `K-FELDMAN` | §5.2, §9 |
| **H-3** | at `τ_hi` the positives were confidence-restricted and the matched negatives were not, so `ΔAUC(τ_hi)` is a different estimand that Holm treated as one family with `ΔAUC(τ_0)` | **the same out-of-fold threshold now applies to `CONFIG-MATCHED-CORRECT`**, and **`c_i` is added to `BASE`** (7 → 8 features) so the definitional quantity is absorbed by the baseline; realised negative counts emitted per cell; §6.1's table scoped to `τ_0` and §10 states the restriction | §5.2, §6.1, §10 |
| **I-1** | three `PERM-STRUCT` specification gaps, one anti-conservative | **unit pinned**: one `π` per `(dataset, τ, draw)` over **items**, applied identically in all three seed cells; **one shared `π` across the five structural columns** (conservative — five independent draws would destroy the block's internal covariance); **refit per scoring fold with standardisation re-fit on that fold's training rows inside every draw** | §5.2 |
| **I-2** | one seed named for five procedures | **substream policy pinned**: one root `default_rng(20260801)`, `.spawn(6)`, frozen child indices `0 = PERM-STRUCT`, `1 = PERM-STRUCT-COND`, `2 = SHUFFLE-POP`, `3 = RANDOM-POP`, `4 = item bootstrap`, `5 = reserved`; the six child seed sequences emitted | §5.2 |
| **I-3** | `round()` unpinned, and the worked example disagrees with banker's rounding | **`R(x) ≡ int(numpy.floor(x + 0.5))`** pinned, with the reason (`1.5 × 55 = 82.5 → 83`, not `82`) and every downstream figure restated under it | §5.3 |
| **I-4** | *"item `i`'s label enters that fit nowhere"* is false in this arena | **restated exactly**: the remaining channel is the arena's own **bank channel**, the same one the deployed vote reads and `GATE-BLIND` books as `bank_label_reads`; it is diffuse (`≈ 20/595` at weight `≤ 20/210`) and **identical for `BASE` and `FULL`, so it cancels in the paired `ΔAUC`**; `GATE-NESTED`'s assertion text is confirmed accurate about what it covers | §4.3 |
| **I-5** | six provenance items | (a) the nine-arm enumeration re-cited to `HEADSPACE_TRANSFER_PREGATE.md:859-864` with the concluding clause at `:864`; (b) *"four distinct operator families"* → **at least six**, with the reason it was not re-derivable; (c) the reopen sentence re-cited to `:1004-1006`; (d) the clause-header scope reason for skipping CAL-2(2) **withdrawn** (it scopes CAL-0…CAL-5 alike), leaving the test-split-payload reason alone; (e) **DET-4 is *"recommended, not mandatory"*** and its Tier-C requirement restated conditionally; (f) `GATE-NULL` now names the two operative caches from `mechnov_pairverify.DATASETS` instead of a glob | §2, §1, §5.2, §6.1, §8.2 |
| **I-6** | `banned_constraints[9]` never named | **named and adjudicated**: its object is the **deployed decision operator**, which C09 never re-orders or re-composes; feature 5 is a probe input read by a classifier never consulted at query time, so `[9]` is not engaged — and would fire if a future proposal moved that comparison into the vote | §3 |
| **I-7** | the `LIVE` marking is derived from the net leg only | relabelled **`LIVE_ON_NET` / `ARITHMETICALLY_DEAD_ON_NET`**, with the explicit statement that the macro-F1 leg carries no closed-form cap | §5.3 |
| **I-8** | `THRESH-BEST`'s selection necessarily computes the object F98(d) bans re-deriving | **declared**: the twins' own `net` and `Δacc` are computed solely to select and score the twin, emitted only inside the `K-DEG` block, and **not reported as findings**; F98(d)'s observation is cited from F113's `+0.0041`, not re-derived | §6.2 |
| **I-9** | the recall/precision figures were inherited without re-derivation | **re-derived in place**: HateMM `k = 114` ⇒ recall `≥ 75.6/84.33 = 0.896`, precision `≥ 75.6/114 = 0.663`; MHC-ZH `k = 83` ⇒ recall `≥ 56.0/62.33 = 0.899`, precision `≥ 56.0/83 = 0.675`, with the `R(x)` convention stated | §5.3 |
| **I-10** | MHC-EN's primary out-of-scope reason never stated; "sha-pinned artifact" imprecise | **primary reason stated first**: **no `headspace_arena_en_*_OUT.json` exists**, so `GATE-FLOOR` has no anchor and the arena cannot be gated at all; the `CLI` restriction is secondary; and the mint is pinned by `meta.mint_script_sha256 = cefdf8dc…` inside the banked arena outputs, not by a `frozen_artifact_policy` entry | §2 |

---

*v6. No hash frozen, no config written, no code implemented, no namespace created, no
