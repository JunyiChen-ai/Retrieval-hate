...
- Original endpoint, supervision, teacher firewall, cache order and final evidence are unchanged.
- Exact trust projections, direct-control attribution and design inference are local specification fixes; they add no method module.
- No segment/timestamp/span/localization signal exists. No teacher key, free text, direct rule loss in FULL or test artifact was introduced.

## Simplicity Check

- Dominant contribution remains one exact-vote-safe proximal realization of one support-selected structural reflection.
- One solver, one exception family, one global coefficient per direct control, and only the stronger direct control proceeds to final three seeds.
- Farkas remains a named-cone audit. All matched direct methods remain controls.

## Changes Made

1. **Exact Dykstra projections:** fixed the ambient product norm and supplied the KKT/root solution for row, class-mean and semantic preimage balls, plus independent optimality parity.
2. **Direct-control attribution:** froze one pooled outcome-blind coefficient per control, exact eval/train/full-bank schedule, cumulative-strength reporting, seed-0 strongest-control selection, and final paired superiority gates.
3. **Pilot design inference:** family selection is repeated inside every Rao-Wu replicate; every replicate rebuilds totals, family, cross-fit profiles and statistics, with failure encoded conservatively.
...
For abstract and realized row-tangent displacement, registered normalized descent cones cover singleton RGCL; legal same-label attraction/opposite-label repulsion; every currently active legal triplet via exact joint blocked `(p,n)` oracle with small-bank brute parity; and complete-bank per-anchor SupCon. Machine manifest stores full universe/count/active hash, all generated columns and the **global minimum** witness inner product at termination.

NNLS residual and independent Farkas witness feasibility/separation must each be `>=.25`, gap `<=1e-5`, on abstract and realized displacement. Claims are limited to these cones; learned controls remain binding for AdamW.
