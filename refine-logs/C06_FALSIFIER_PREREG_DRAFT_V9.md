# C06 `$0` CPU falsifier — preregistration **DRAFT v9** (2026-08-04)

**SUPERSESSION.** Supersedes `C06_FALSIFIER_PREREG_DRAFT_V8.md` → V7 → V6 → V5 → V4 → V3 → V2 → v1.
All eight remain on disk **unmodified** as the record of what each round reviewed. Reviews of
record: `C06_FALSIFIER_PREREG_REVIEW{,_R2,…,_R8}.md` — REVISE 3C/6H/10I+4M, 3C/3H/7I+3M,
2C/1H/6I+4M, 3C/3H/8I+4M, 3C/3H/6I+5M, 2C/3H/5I+6M, 4C/2H/3I+4M, 2C/2H/4I+6M. Complete standalone
document.
...
**Disposition: 14 round-8 findings, dispositioned at LIMB level — 26 limbs, every one landed and
cited to a diffed section (§14, §14.1). This version makes no blanket "all N ADOPTED" claim.**

**Why the vocabulary changed.** For three consecutive versions this document's header asserted
unqualified adoption over an artifact that was partially adopted, and round 8 correctly graded v9's
predecessor on that basis: its audit of v8 returned **9 adopted, 4 partial**. The cause was
structural, and round 8 diagnosed it precisely — §14.1's audit asked *"did the section this row
cites change?"* and could never ask *"did every **limb** of the finding land, including limbs in
sections no row cites?"* **Every unlanded limb in v8 was in §13, which was byte-identical v7→v8 and
is cited by no disposition row** — the audit's blind spot coincided with the document's weakest
section. v9 adopts round 8's prescribed fix: **§14 carries a limb table**, and §14.1's audit gains
**step (5)** (each limb must name the section it landed in) and **step (6)** (sections that changed
but no row cites; sections a row names that did not change).

**What v9 changes.** Round 8's two Criticals and two Highs.

* **C-1** — §14.1's transcript reported its own byte count as `2626` where the script prints
  `2658`, because the transcript was captured before the section it measures was final. **§14.1 is
  now self-excluding**: it never reports its own size, so the transcript is a fixed point by
  construction, and the transcript is **regenerated by running the script against the finished
  on-disk v9 as the last action before this document was closed**.
* **C-2** — the two unlanded §13 limbs are landed: **item 19** now names endpoint pre-normalisation
  and the bit-exact predicate as checked properties, and **item 15**'s *"does not import"* is
  corrected to *"never **calls**"*.
* **H-1** — `GATE-SHA`'s scope in §6 now covers the sixteen banked artifacts §11 said it did.
* **H-2** — §3.7 is split into **population-derived (computed)** and **frozen C01 config constants
  (read and asserted)**, and §13 item 5 no longer instructs the code lineage to compute a comparison
  operator from the arena.
...
* **Small-set comparison operator — frozen as `<=`** (round-6 I-4; round-7 C-4 found it frozen
  ...
  which is `"registered_null_rows_excluded"` inside the tiny-fraction audit). Because the
...
**v7's measurements** added ≈ **5 wall-minutes / ≈ 25 CPU-minutes**. **Five CPU head mints were
trained in v7:** one for §7.8's `GATE-FLOOR` discharge (`33.5 s`) and **four more** for its
four-cell displacement-tail table, spanning both lineages and both datasets (`≈ 33–40 s` each).
*(Round-8 I-1: §7.8's narrative attributes the discharge mint to "v6" and the tail table to "v7".
The discharge mint was trained in the **v6** drafting round and re-verified by round 6; the four
tail mints were trained in the **v7** round. §7.8 describes when each measurement was first
**reported**; this section accounts for when the compute was **spent**. Both are now stated so the
two cannot be read as contradicting.)* v7 also ran the untrained-residual
...
the audit re-runs. **v9 trained no heads.**
...
tail cells) + 0 (v8) + 0 (v9) = ` **12 CPU head mints**; time `≈ 21 (v1–v6) + 5 (v7) + 2 (v8) + 1
(v9) = ` **≈ 29 wall-minutes**, `≈ 85 + 25 + 6 + 3 = ` **≈ 119 CPU-minutes**. All `$0`, zero GPU, no
battery-arm accuracy at any point. *(v8's "eleven" did not reconstruct: `7 + 5 = 12`.)* The CPU-cap conflict was knowable from C09's
...
| **7z** `GATE-ZEROOP`'s mismatch scan and tie-casualty evaluation (**round-8 I-2**, the seventh uncounted loop) | `2 identities × 60` vectorised comparisons, plus tie work bounded by `cap × cells` = `7×30 + 5×30 = 360` items worst case | vectorised scan sub-`0.1 s`; tie work is **zero on a clean run** and capped before HALT | `0.3 s` |
...
**Total, re-multiplied:** `2927.6 + 1.0 + 0.7 + 0.3 = ` **`2929.6 s = 48.8 min`** corroborating;
**`× 1.25 = 3662.0 s = 61.0 min`** conservative.
...
`9.3 %`; a 2× miss moves the total to `3203.3 s = 53.4 min`, a 5× miss to `4024.4 s = 67.1 min`.
...
## 14. Cumulative disposition — all eight rounds

**Every row cites the v8→v9 section(s) that implement it; every *limb* of every round-8 finding is
listed separately below and cites the section it landed in; §14.1 prints the scripted audit that
checks both, plus the two reports round 8 prescribed.** No blanket adoption claim is made anywhere
in this document.

### Round 8 (10 findings + 6 Minor) — row level

| finding | disposition | §(diffed) |
|---|---|---|
| **C-1** §14.1's transcript reports a stale byte count for itself | **ADOPTED** — §14.1 is **self-excluding** (never reports its own size), the convention is stated, and the transcript is regenerated against the finished on-disk v9 as the last action | §14.1 |
| **C-2** two unlanded §13 limbs; blanket "all ADOPTED" over a partial artifact | **ADOPTED** — item 19 carries endpoint pre-normalisation and the bit-exact predicate; item 15 reads *"never calls"*; the header makes no blanket claim; §14.1 gains steps (5) and (6) | §13.1, header, §14.1 |
| **H-1** `GATE-SHA`'s scope did not cover the sixteen new digests | **ADOPTED** — §6's row widened; `U7` re-described and re-priced at `0.13 s` | §6, §7.7 |
| **H-2** three C01 config constants under a population-derived preamble | **ADOPTED** — §3.7 split into two blocks with two verbs; item 5 rewritten | §3.7, §13.1 |
| **I-1** §7.8/§7.9 disagree on which version trained the discharge mint; count does not reconstruct | **ADOPTED** — the reported-versus-spent distinction stated; the count shown as a checkable sum (`7+1+4+0+0 = 12`) | §7.9 |
| **I-2** seventh uncounted loop: `GATE-ZEROOP` | **ADOPTED** — Phase 7z row added; item 10 requires the worst-case-over-orderings computation to be analytic; total `2929.6`/`3662.0` | §8, §13.1 |
| **I-3** `normalization_epsilon` registered nowhere | **ADOPTED** — frozen in §3.7's config block with its consumption path; §13 item 8 requires the head-space builder to pass it | §3.7, §13.1 |
| **I-4** the reference scan under-scans | **ADOPTED** — emphasis-tolerant `§13 item N` pattern, bare `§N` included, both counts printed **with their scope named** | §14.1 |
| **M-1** `:2049` → `:2036` | **ADOPTED** | §5.2.2 |
| **M-2** rotation counts attributed to §1's table | **ADOPTED** — attributed to `C01_A0_OUT.json` with the values | §10.2 |
| **M-3** §7.3's scope stale at v1–v7 | **ADOPTED** — v1–v9 | §7.3 |
| **M-4** unbalanced emphasis in §10.2 | **ADOPTED** | §10.2 |
| **M-5** `~600×` attached to the wrong test | **ADOPTED** — attached to the minimum norm over the epsilon | §5.2.3 |
| **M-6** audit script path not followable | **ADOPTED** — the script is reproduced in full in §14.2 | §14.1, §14.2 |

### Round 8 — LIMB level (the check v8 could not perform)

LIMB-TABLE-BEGIN

| finding | limb, as round 8 prescribed it | landed in |
|---|---|---|
| **C-1** | make §14.1 self-excluding so the transcript is a fixed point | §14.1 |
| **C-1** | state which convention is in force | §14.1 |
| **C-2** | extend item 19 or 23 with endpoint pre-normalisation as a checked property | §13.1 |
| **C-2** | state that `GATE-C01PARITY` must be asserted at bit-exactness, and why | §13.1 |
| **C-2** | fix item 15's *import* → *call* | §13.1 |
| **C-2** | stop the blanket "all N ADOPTED" the machinery has not verified | header |
| **H-1** | widen `GATE-SHA`'s scope in §6 to the sixteen banked artifacts | §6 |
| **H-1** | update `U7`'s description, which still said "8 caches + 6 modules" | §7.7 |
| **H-2** | split §3.7's table, or add a per-row provenance verb | §3.7 |
| **H-2** | correct the preamble to quantify over the population-derived block only | §3.7 |
| **H-2** | rewrite item 5 to name the config constants as read-and-asserted | §13.1 |
| **I-1** | make §7.8 and §7.9 agree on which version trained the discharge mint | §7.9 |
| **I-1** | show the mint count as a sum so it is checkable | §7.9 |
| **I-2** | add a Phase 7z row for `GATE-ZEROOP`'s scan and tie-casualty work | §8 |
| **I-2** | require the worst-case-over-orderings computation to be analytic | §13.1 |
| **I-3** | register `normalization_epsilon = 1e-12` with its source | §3.7 |
| **I-3** | require the head-space builder to pass that value and no other | §13.1 |
| **I-4** | make the `§13 item N` pattern emphasis-tolerant | §14.1 |
| **I-4** | include bare `§N` references | §14.1 |
| **I-4** | print both counts with their scope named | §14.1 |
| **M-1** | `:2049` → `:2036` | §5.2.2 |
| **M-2** | attribute the rotation counts to `C01_A0_OUT.json` | §10.2 |
| **M-3** | scope §7.3's blindness claim to v1–v9 | §7.3 |
| **M-4** | balance §10.2's emphasis markers | §10.2 |
| **M-5** | attach `~600×` to the minimum norm, not to `tiny_ok` | §5.2.3 |
| **M-6** | make the audit script followable by a reader | §14.2 |

LIMB-TABLE-END

**Round-8 rulings accepted without change.** No gate can fire on a warranted CLOSE (all twenty,
re-derived). The verdict-path enumeration is total and mutually exclusive with one lawful absence
path. `GATE-C01PARITY` at bit-exactness is correct and settled, and no other gate inherited the
retired tolerance. §3.4's pin is sufficient — round 8 rebuilt all thirteen arms from the prose at
the first attempt supplying nothing, and every wrong reading it constructed is now caught. §5.2.3's
`tiny_ok` non-carriage is sufficient, its warrant being §13 item 25's per-cell record rather than
*"four cells generalise"*. The conservative timing figure is the right one to freeze.

**Round-8 rulings that changed this document's method**, both adopted in §14.1: an embedded
self-audit is *necessary and not sufficient*, and the mechanism that actually catches unlanded limbs
is an independent reader diffing against the **previous round's prescriptions** rather than against
this table — which is why the round-9 request makes that a review obligation rather than delegating
it to a script.

### Rounds 1–7 — carried

Round 8 audited v8's round-7 block by execution: **9 verified adopted, 4 partial, 0 not adopted, 0
rebutted.** The four partials were round-7 C-1 limb 3, H-2, I-3 and M-1 limb 2 — **all four closed
in v9** as C-2, I-1, H-1 and C-2 respectively. Earlier rounds stand as recorded in v3/v4/v6/v7/v8
§14; where a later round refined an earlier disposition, the later section governs.
...
does not buy; `B = 2000`; and the 92-family's auditability warrant.
...
**The convention for this section, stated because it measures the document that contains it
(round-8 C-1).** §14.1 is **self-excluding**: the audit never reports §14.1's own byte count, only
that it changed. v8's transcript reported `ADDED §14.1 2626 chars` where the script printed `2658`,
because pasting the transcript changed the length of the section being measured. Self-exclusion
makes the transcript a **fixed point by construction**, so no future edit to this section can
invalidate it.

**How the transcript below was produced.** The script was executed against the **finished, on-disk
v9** as the last action before this document was closed — not assembled from staged state, which is
what produced v8's stale line. **It was then re-executed against the document with the transcript
already embedded, and the two outputs are byte-identical** — so the transcript is a verified fixed
point, not merely one by construction. v9's drafting also writes after **every** edit and verifies each
write, which is the change that matters more than the script.

**What the audit does.** (1) Splits v8 and v9 into sections and diffs them. (2) For every §14 row,
checks each cited section appears in the diff. (3) Flags any row whose cited section shows no diff.
(4) Resolves references — **emphasis-tolerant** for `§13 item N`, and reporting **dotted `§N.N` and
bare `§N` counts separately with their scope named** (round-8 I-4). (5) **Limb level**: parses the
`LIMB-TABLE` above and checks each limb's landing section diffed; a limb with no section is `OPEN`.
(6) **Coverage both ways**: sections that changed but no row or limb cites, and sections a row names
that did not change — the report that would have surfaced §13 in v8.

**Output, verbatim:**

```
=== (1) SECTION DIFF v8 -> v9 ===
  CHANGED  §6        +343 chars
  CHANGED  §8        +1005 chars
  CHANGED  §14       +1053 chars
  CHANGED  §15       -91 chars
  CHANGED  §3.7      +1146 chars
  CHANGED  §7.3      +0 chars
  CHANGED  §7.7      +164 chars
  CHANGED  §7.9      +840 chars
  CHANGED  §10.2     +287 chars
  CHANGED  §13.1     +1904 chars
  CHANGED  §14.1     (self, size not reported)
  ADDED    §14.2     6107 chars
  CHANGED  §5.2.2    +116 chars
  CHANGED  §5.2.3    +214 chars
  CHANGED  header    -48 chars
  UNCHANGED: 43 sections

=== (2)+(3) DISPOSITION ROWS vs DIFF ===
  OK    C-1   cites §14.1
  OK    C-2   cites §13.1, §14.1, §header
  OK    H-1   cites §6, §7.7
  OK    H-2   cites §3.7, §13.1
  OK    I-1   cites §7.9
  OK    I-2   cites §8, §13.1
  OK    I-3   cites §3.7, §13.1
  OK    I-4   cites §14.1
  OK    M-1   cites §5.2.2
  OK    M-2   cites §10.2
  OK    M-3   cites §7.3
  OK    M-4   cites §10.2
  OK    M-5   cites §5.2.3
  OK    M-6   cites §14.1, §14.2
  rows verified against diff hunks: 14 ; rows failing: 0
...
=== (4) REFERENCE RESOLUTION (emphasis-tolerant; dotted AND bare) ===
  ...
  §13 item refs (emphasis-tolerant): [5, 7, 8, 15, 19, 23, 25, 26] ; defined 1..26 ; unresolved: NONE
...
=== (5) LIMB-LEVEL DISPOSITION (round-8 prescription) ===
  ...
  OK    C-2   extend item 19 or 23 with endpoint pre-normalisation -> §13.1
  OK    C-2   state that `GATE-C01PARITY` must be asserted at bit- -> §13.1
  OK    C-2   fix item 15's *import* → *call*                      -> §13.1
  OK    C-2   stop the blanket "all N ADOPTED" the machinery has n -> §header
  OK    H-1   widen `GATE-SHA`'s scope in §6 to the sixteen banked -> §6
  OK    H-1   update `U7`'s description, which still said "8 cache -> §7.7
  OK    H-2   split §3.7's table, or add a per-row provenance verb -> §3.7
  OK    H-2   correct the preamble to quantify over the population -> §3.7
  OK    H-2   rewrite item 5 to name the config constants as read- -> §13.1
  OK    I-1   make §7.8 and §7.9 agree on which version trained th -> §7.9
  OK    I-1   show the mint count as a sum so it is checkable      -> §7.9
  OK    I-2   add a Phase 7z row for `GATE-ZEROOP`'s scan and tie- -> §8
  OK    I-2   require the worst-case-over-orderings computation to -> §13.1
  OK    I-3   register `normalization_epsilon = 1e-12` with its so -> §3.7
  OK    I-3   require the head-space builder to pass that value an -> §13.1
  ...
  OK    M-1   `:2049` → `:2036`                                    -> §5.2.2
  OK    M-2   attribute the rotation counts to `C01_A0_OUT.json`   -> §10.2
  OK    M-3   scope §7.3's blindness claim to v1–v9                -> §7.3
  OK    M-4   balance §10.2's emphasis markers                     -> §10.2
  OK    M-5   attach `~600×` to the minimum norm, not to `tiny_ok` -> §5.2.3
  OK    M-6   make the audit script followable by a reader         -> §14.2
  limbs landed: 26 ; limbs open/failing: 0

=== (6) CHANGED-BUT-UNCITED / NAMED-BUT-UNCHANGED ===
  changed but cited by no row/limb: ['14', '15']
  named by a row but unchanged:    NONE
```

**Reading it.** Every round-8 row cites a section the diff shows as changed or added, and every one
of the 26 limbs names a section that diffed — including **§13.1**, the section v8 never opened and
whose absence from v8's citation set was the blind spot round 8 identified. Step (6)'s
*changed-but-uncited* list returns **§14 and §15 only**: the disposition record and the open-issues
list necessarily change every round and are cited by no row, which is structural rather than a
finding. Step (6)'s *named-but-unchanged* list is empty — in v8 it would have been non-empty, which
is the report round 8 asked for. The row scan excludes the limb-table region, so the two counts
(**14 rows**, **26 limbs**) are disjoint rather than double-counted.

### 14.2 The audit script, in full (round-8 M-6)

The script lives in the session scratchpad, which a reader of this repository cannot open. It is
reproduced here so the claim *"re-runnable"* is one a reader can act on. It is a drafting
instrument: it reads two draft files, writes nothing, and touches no repository artifact.
...
"""
import re, sys
V_OLD='/data/jehc223/RGCL/refine-logs/C06_FALSIFIER_PREREG_DRAFT_V8.md'
V_NEW='/data/jehc223/RGCL/refine-logs/C06_FALSIFIER_PREREG_DRAFT_V9.md'
...
print('=== (1) SECTION DIFF v8 -> v9 ===')
changed=set(); added=set()
for k in sorted(SB, key=lambda x:(len(x),x)):
    if k not in SA: added.add(k); print('  ADDED    §%-8s %s'%(k,'(self, size not reported)' if k==SELF else '%d chars'%len(SB[k])))
    elif SA[k]!=SB[k]:
        changed.add(k)
        print('  CHANGED  §%-8s %s'%(k,'(self, size not reported)' if k==SELF else '%+d chars'%(len(SB[k])-len(SA[k]))))
...
print('  §13 item refs (emphasis-tolerant): %s ; defined 1..%d ; unresolved: %s'
...
## 15. Open issues for round 9

1. **The limb-level protocol (§14, §14.1 steps 5–6).** Round 8 prescribed it and ruled that an
   embedded self-audit is *necessary and not sufficient* — that what actually catches unlanded limbs
   is an independent reader diffing v8→v9 against **round 8's prescriptions**, not against §14's
   self-description. Round 9 should do exactly that, and rule whether the limb table plus steps 5–6
   closes the gap or merely moves it.
2. **`GATE-SHA`'s widened scope (§6, §7.7).** The gate now covers 37 artifacts. Round 9 should
   confirm nothing else in §6 or §12 still describes the narrower scope, and that `U7`'s `0.13 s`
   is consistent with §8 Phase 1d.
3. **§3.7's two blocks (§3.7, §13 item 5).** The table now has two verbs — *computed* and *read and
   asserted*. Round 9 should check every consumer of a §3.7 constant uses the right verb, and that
   `GATE-POP`'s row (which quantifies over the population-derived subset) is still correct after the
   split.
4. **The frozen timing figures (§8 Phase 7z).** Three parties measured the same two loops across a
   `8×` and `18×` spread. v9 freezes the conservative bound of each and records that the spread is
   about timing boundaries. Round 9 should rule whether freezing the max is right, or whether the
   loops should be re-specified so the boundary is unambiguous.
5. **The eighth uncounted loop.** Eight rounds, seven found. Hunt again.
6. **Is the record now sound?** Round 8 answered *no* and named §13 as the common cause of three of
   its four record findings. §13 is edited this round for the first time since v7. If round 9 agrees
   the record is sound, say so plainly; if not, name the specific defect. Do not grade on
   trajectory — eight rounds is evidence of nothing.

---

*No GPU, SLURM, Modal, teacher call, cache write, test-split access, job submission or commit
occurred in producing this document. **v9 trained no heads**; its measurements were the round-8 C-1
byte-count reproduction, the `GATE-SHA` widening cost, and the audit re-runs — all read-only on
banked train-split artifacts and on the drafts themselves. The **twelve** CPU head mints across
v1–v9 (§7.9's sum) wrote nothing outside the session scratchpad and computed no battery-arm
accuracy. `TARGET_STATE.json` was read and not modified. v1–v8 are unmodified.*
