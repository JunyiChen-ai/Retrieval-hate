# C09 Stage-0 (A0) — preregistration **v11** (round-10 repairs)
...
> ## STATUS: `V11_REPAIRED_NOT_FROZEN_NOT_SUBMITTED` — awaiting fresh independent review.
>
...
> `C09_A0_V10_PREREG_REVIEW.md` (`REVISE 0C/0H/2I`) → **this file**.
>
> **`v1–v10 are superseded in full and must not be implemented.`** Each carries its own
> ledger; **the v11 ledger is §12**. Rounds 9 and 10 both found the science unmovable —
> round 10: *"The science is finished and I could not move it, and neither of my two
> findings touches it… they do not touch a single rule, threshold, feature set, null,
> gate or arithmetic."* Round 10's two Importants were a false scope sentence in §0 and
> a retired object name in §7; both are closed here, together with five items round 10
> checked and deliberately placed below its own line.
>
...
## 0. What v11 changes, and how this design reached its present form

Round 10 returned **`0C / 0H / 2I`**, with the bottom line *"The science is finished and
I could not move it, and neither of my two findings touches it… Fix them and the frozen
set is ready."* Both findings, and five further items round 10 listed below its own line,
are closed here.

1. **§0's own scope sentence was false, and it re-committed R9 I-1 inside the repair
   that closed R9 I-1.** v10 said *"Nothing in §§2–9 changed in v10"* while §6.1 — which
   is inside §§2–9 — had two sentences changed, as v10's own next clause and §12 both
   said. **Corrected below, mirroring §12's wording (R10 I-1).**
2. **§7 named `CONFIG-MATCHED-CORRECT` as a live control and rested the registry claim's
   break-exposure conjunct on it, but the term is defined nowhere in v10** and §5.3 calls
   the phrasing containing it *superseded*; the object it named also changed twice (R5
   H-3 restricted it on confidence, R6 H-1 made it fold-dependent). **§5.2 now
   re-introduces the name as an explicit alias at the point of definition, and §7
   attributes break exposure to what actually measures it (R10 I-2).**
3. **Five items round 10 checked and placed below its line, fixed anyway because they
   are free:** §6.1's sentence fragment left by the R9 I-2 repair; §5.3's *"7.5 points
   at `k = 100`"* (true value `7.45`); §5.3's *"the six `(τ, k)` cells"* (the caps are
   per-dataset, so there are twelve markings); §5.3's `131.4667` (the exact mean `253/3`
   value, `2 × 84.3333 − 37.2 = 131.4666` on the printed 4-dp mean — both give `k ≥ 132`
   dead, and the exact form is now stated); and §11's silence on F66's own re-open
   clause, now quoted beside F99's.

**Scope of v11, stated exactly.** The edits are the title, the STATUS block, §0, one
line of §5.2, three sentences of §5.3, two sentences of §6.1, §7's two `CONFIG-MATCHED-CORRECT`
sentences, one sentence of §11, and §12. **§§1, 3, 4, 8, 9 and 10 are byte-identical to
v10, and no decision rule, threshold, feature set, null, gate or arithmetic has changed
since v8.**
...
## 12. Repair ledger — the 2 round-10 findings, plus five below-the-line items

Round 10's verdict was **`REVISE (0C / 0H / 2I)`**, with the bottom line *"The science is
finished and I could not move it, and neither of my two findings touches it… Both repairs
are edits to three sentences in §0 and §7. They need no re-measurement, no GPU, no
re-derivation of anything in §§1–11, and they do not touch a single rule, threshold,
feature set, null, gate or arithmetic."*

**Scope of v11, stated exactly.** The edits are the title, the STATUS block, §0, one
sentence of §5.2, three sentences of §5.3, two sentences of §6.1, §7's two
`CONFIG-MATCHED-CORRECT` sentences, one sentence of §11, and §12. **§§1, 3, 4, 8, 9 and
10 are byte-identical to v10, and no decision rule, threshold, feature set, null, gate or
arithmetic has changed since v8.**

| # | round-10 finding | repair | where |
|---|---|---|---|
| **I-1** | §0's scope sentence was false — *"Nothing in §§2–9 changed in v10"* while §6.1 (inside §§2–9) changed two sentences, as v10's own next clause and §12 both said — re-committing R9 I-1 inside the repair that closed it | **§0's scope statement rewritten to mirror §12's**, naming every section touched and stating the invariant that matters: no rule, threshold, feature set, null, gate or arithmetic has changed since v8 | §0 |
| **I-2** | §7 named `CONFIG-MATCHED-CORRECT` as a live control and rested the registry claim's break-exposure conjunct on it, but v10 defined the term nowhere, §5.3 marked the phrasing containing it superseded, and the object changed twice under the name (R5 H-3 confidence-restricted it; R6 H-1 made it fold-dependent) | **both halves fixed**: §5.2 re-introduces the alias **at its point of definition**, with the two changes of object named; and §7 stops asserting break exposure by cross-reference and **attributes it to what measures it** — the per-seed `net_s = 2·\|S ∩ wrong_s\| − k` accounting, `GATE-SELFTEST`'s identity to `n · Δacc_s`, and the reported class composition of `S` plus the fraction outside `A^{(f)}` | §5.2, §7 |

**Five items round 10 checked and placed below its own line, fixed anyway because they
are free and reduce residual surface:**

| item | repair |
|---|---|
| §6.1's R9 I-2 repair left a **sentence fragment** (*"Concluding at `:863-864` …"*) | recast as a complete sentence |
| §5.3's *"7.5 points is the gap at `k = 100`"* | corrected to **`7.45`**, with the arithmetic shown |
| §5.3's *"the six `(τ, k)` cells"* | corrected to **twelve `(dataset, τ, k)` cells, six per dataset**, since the caps are per dataset |
| §5.3's `131.4667` / `95.6667` | restated from the **exact** means `253/3` and `187/3` rather than their printed 4-dp roundings, so the cap arithmetic is reproducible in closed form; `k ≥ 132 / 96` dead either way |
| §11 quoted F99's burden but not F66's | **F66's own re-open clause quoted** — *"Re-open requires an operator that converts symmetric structure — the beta decomposition proves none of the tried classes can"* — with the observation that §11's successor **is** a symmetric operator, so that sentence is its **precondition**, not its context |

*(Round 10's remaining below-the-line items are left as it left them: §6.1's "four
operator families" is the source's own count and v11's enumeration is exact and labelled;
§2's `ksweep` parenthetical errs against the design, on a secondary reason for skipping a
clause CAL-2 itself declares non-gating; and §2's budget lines sum to 116 against a
declared "≈ 115" that is ~7× conservative against a measured fit time.)*

---

*v11. No hash frozen, no config written, no code implemented, no namespace created, no
