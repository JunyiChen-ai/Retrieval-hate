# C09 Stage-0 (A0) — preregistration **v13** (round-12 repairs)
...
> ## STATUS: `V13_REPAIRED_NOT_FROZEN_NOT_SUBMITTED` — awaiting fresh independent review.
>
...
> `C09_A0_V12_PREREG_REVIEW.md` (`REVISE 0C/1H/2I`) → **this file**.
>
> **`v1–v12 are superseded in full and must not be implemented.`** Each carries its own
...
## 0. What v13 changes, and how this design reached its present form

Round 12 returned **`0C / 1H / 2I`**, with the science confirmed intact for the fourth
round running and the High aimed at v12's own repair.

1. **v12 corrected the wrong term (R12 H-1).** Round 11 proved that the pre-declared
   `n_unstable ≈ 7–9` and `|P_0| ≈ 76 / 55` cannot both hold given the measured per-seed
   error counts, and v12 resolved the contradiction by keeping `76 / 55`. **The
   arithmetic says the opposite.** `|P_0|` and `n_unstable` are two ratios of the *same*
   F88 seed-consensus table and must be transferred jointly, denominated on the only
   quantity this arena measures — the per-seed error count. Done that way they give
   **`|P_0| ≈ 79 / 60` and `n_unstable ≈ 9 / 8`**, and they reproduce the measured
   `Σ_s |E_s| = 253 / 187` exactly. The old `55` was `0.88 × 62.33`, applying F88's
   **union**-denominated ZH rate (*"22 of the 25-item union"*) to a **per-seed** base;
   the old `76` took the low end of a `"≈89-93 %"` band computed against a fuzzy
   `"~26-28"`. The two datasets' figures were computed by different conventions, which
   is exactly why the incompatibility existed. **§4.2 now shows the derivation in four
   lines**, and §5.2, §5.3, §6.3 and §10 are restated on it.
2. **§6.1's *"the count is the source's four"* equivocates between two different fours
   (R12 I-1).** The ladder's four numbered rungs are VSW / F95 / FIXK / **F89**, with
   `THRESH_best` deliberately un-runged; the nine raw positives occupy rungs 1–3 plus the
   un-runged arm and contain **no rung-4 member at all**. Restated — and **rung 4's own
   head-space numbers are carried, because they are an independent headwind against
   C09's structural block that no version has registered.**
3. **`F114` was cited three times with no locator (R12 I-2)**, and the bare number
   resolves to a different, C04-lineage finding in `TARGET_FINDINGS.md`. **Locator added
   at first use.**

**Scope of v13, stated exactly.** The edits are the title, the STATUS block, §0, a new
derivation block in §4.2, four sentences of §5.2, four sentences of §5.3, three sentences
of §6.1, one sentence of §3.1, three sentences of §6.3, one bullet of §10, and §12.
**§§1, 2, 3.2–3.4, 4.1, 4.3, 4.4, 5.1, 5.4–5.6, 6.2, 7, 8, 9 and 11 are byte-identical to
v12, and no decision rule, threshold, feature set, null, gate, operating-point definition
or arithmetic *rule* has changed since v8** — what changed here are the *transferred
expectations* those rules are pre-declared against, which no gate reads.
...
>
> | | HateMM (`ERRPAT_HateMM §1.1`, **final** row) | MHC-ZH (`ERRPAT_MHC-ZH §1`) |
> |---|---|---|
> | per-seed errors | `26 / 27 / 27` ⇒ `Σ = 80`, mean `26.6667` | `23 / 24 / 22` ⇒ `Σ = 69`, mean `23` |
> | `3/3` = `\|P_0\|` | `25` | `22` |
> | `2/3` = `a` | `2` | `0` |
> | `1/3` = `b` | `1` | `3` |
...
> checkpoint selection — §5.6). The val-sel row (`24 / 3 / 2`, `Σ = 79`) would give
> `|P_0| ≈ 78`. Nothing in the decision rule reads either.
...
>   bar. A shortfall of three items on HateMM or one on MHC-ZH closes the branch on reach
>   alone.
...
**And rung 4 is an independent headwind this design had not registered (R12 I-1).**
F89's four arms are closed-form **geometric transforms of the head keys** — the family
nearest to `FULL`'s structural block — and in head space (`:644-647`) they read
`F89_T1 +0.0000`, `F89_T2a −0.0004`, `F89_T2b +0.0027`, `F89_T4 +0.0054` per-dataset
first entries, i.e. **null to sub-noise, with a cross-dataset sign flip**. That is
...
## 12. Repair ledger — the 3 round-12 findings

Round 12 confirmed the science intact for the fourth consecutive round — *"The instrument,
the legality spine, the executability, the budget and the inference are all intact, and I
could not move any of them… A KILL remains fully available and is still the honest
expectation"* — and independently re-derived both permutation nulls' exactness arguments,
the Hanley–McNeil algebra, the macro-F1 monotonicity, both of the mint's run-time sha256
assertions, and F98's DEG-A from `aggnet_main_*_OUT.json`. It also confirmed v12's scope
claim exact *"for the first time in five rounds"*.

**Scope of v13, stated exactly.** The edits are the title, the STATUS block, §0, a new
derivation block in §4.2, one sentence of §3.1, one paragraph of §5.2, one paragraph of
§5.3, one list item of §5.3, two paragraphs of §6.1, one block of §6.3, one bullet of
§10, and §12. **§§1, 2, 3.2–3.4, 4.1, 4.3, 4.4, 5.1, 5.4–5.6, 6.2, 7, 8, 9 and 11 are
byte-identical to v12.** **No decision rule, threshold, feature set, null, gate,
operating-point definition or arithmetic *rule* has changed since v8** — what v13 changes
are the *transferred expectations* those rules are pre-declared against, and no gate
reads one.

| # | round-12 finding | repair | where |
|---|---|---|---|
| **H-1** | v12's headline repair corrected the wrong term. `\|P_0\|` and `n_unstable` are two ratios of the **same** F88 table and must be transferred jointly, per-seed-denominated; done that way they give **`\|P_0\| ≈ 79 / 60`** and **`n_unstable ≈ 9 / 8`** and reproduce the measured `Σ_s\|E_s\| = 253 / 187` exactly. v12's `76 / 55` was the mis-derived term — ZH's `55 = 0.88 × 62.33` applied a **union**-denominated rate to a **per-seed** base — and v12 propagated it into §5.3, §6.3, §10 and §12, leaving four further `τ_hi` pre-declarations standing on `\|P_{τ_hi}\| ≈ 28` when the corrected figure is `≈ 30`, the exact value at which "arithmetically dead" flips | **§4.2 now derives the transfer in four lines and a table**, names both conventions that produced the old figures, and states the protocol choice (final-epoch, matching this arena's mint). Restated on it: **§5.2** — the `τ_hi` co-primary is **not** arithmetically dead but **live and marginal on both legs on both datasets** (`reach 0.0538 / 0.0518`; `p_w ≥ 30` needs `≥ 75 %` of HateMM's positives and **all** of MHC-ZH's), and v12's *"cannot produce a CONTINUE at all"* is **withdrawn**; **§5.3** — the `k` grid re-derived at `79 / 60` (`158 / 120` dead, `119 / 90` live at recall `≥ 0.926 / 0.955`, precision `≥ 0.656 / 0.661`), and all three `τ_hi` cells shown to clear the net caps; **§6.3** — `CONTROL_UNDERPOWERED` restored as the **expected** outcome on both datasets, as v11 had it; **§10** — the stability-premise bullet restored, with the realised-tag framing kept | §4.2, §5.2, §5.3, §6.3, §10 |
| **I-1** | §6.1's *"the count is the source's four"* equivocates: the ladder's four rungs are VSW / F95 / FIXK / **F89**, and the nine positives contain **no rung-4 member** | restated precisely — the nine occupy rungs 1–3 plus the **un-runged** `THRESH_best`, so the two fours are different sets of the same cardinality and `:623` does not license the count alone — **and rung 4's own head-space numbers are now carried as an independent headwind**: F89's four closed-form geometric head-key transforms, the family nearest `FULL`'s structural block, read `+0.0000 / −0.0004 / +0.0027 / +0.0054`, null to sub-noise with a cross-dataset sign flip | §6.1 |
| **I-2** | `F114` cited three times with no locator, and the bare number resolves to an unrelated C04-lineage finding in `TARGET_FINDINGS.md` | **locator added at first use**: `findings.jsonl:115`, with the collision named | §3.1 |

**What round 12 checked and left below its own line, left as it left it:** the scope
statement's "three sentences of §6.3" understating the *result* by ~3× while being true
of the edit's footprint; `GATE-LEDGER`'s six trainlog reads being of mixed files whose
safety rests on `headspace_fidelity.py`'s hard `VAL_RE` filter (no test label is
materialised and `GATE-DEVFID` gates nothing); §5.6's "30-epoch Adam" where `run_rac.py`
uses `AdamW`; `load_split` calling `_ORIG_TORCH_LOAD` (safe because both call sites are
the literals `"train"` and `"dev_seen"`, and the driver's `open()`-level guard covers it
regardless); `ITEM-STRATUM` occupancy not being emitted the way the row strata are (the
direction is conservative, toward KILL); the ellipsis omissions that all run **against**
C09; `queued.screening_arena_switch`; §2's budget and `ksweep` parenthetical; §4.3's
`≈ 1.4 %`; and §2's "~25–60 s" lower bound against a measured `33.2`.

---

*v13. No hash frozen, no config written, no code implemented, no namespace created, no
