# C06 `$0` CPU falsifier — preregistration **DRAFT v15 + ERRATUM 1** (2026-08-05)

**THIS IS v15 PLUS ERRATUM 1 AND NOTHING ELSE.** `C06_FALSIFIER_PREREG_DRAFT_V15.md` remains on
...
**Total, re-multiplied:** `2642.3 + 1.0 + 0.7 + 0.1 (Phase 7z) + 1.3 (Phase 1f) + 3.8 (Phase 1g)
+ 7.0 (Phase 4, ERRATUM 1) + 1013.8 (Phase 3, CODE-R1 H-4) = ` **`3670.0 s = 61.2 min`**
corroborating; **`× 1.25 = 4587.5 s = 76.5 min`** conservative, where `2642.3` is the sum of every
row except the seven named. *(Two corrections, both explicit. **ERRATUM 1** moved Phase 4
...
the earlier text treated it as: a 2× miss moves the total to `4683.8 s = 78.1 min` and a 5× miss to
`7725.2 s = 128.8 min`, the latter `1.68×` the conservative figure — still inside the `2×` clause
...
* Each line: ISO-8601 timestamp · phase · units done / total · elapsed · elapsed ÷ **§8's frozen
  projected** value. *(ERRATUM 1 set it to `2929.9 s`; **CODE-R1 H-4 sets it to `3670.0 s`**. The
  denominator is pinned to §8 by name, so it tracks automatically; the literal in
  `c06_falsifier_arena.py` and `configs/c06/c06_falsifier.json` is updated with each correction.)*
...
export exists and that layer 3 is actually active in all 73 processes, not merely importable.
...
at `1.216–1.273 ms` per draw, `3.82 s` over all `3072`, **`0.13 %` of the `2934.5 s` total** and
...
reported for their own timings, and **v15 ran no timings at all**. **No `.py` source moved** — all 37
§11 digests recompute. Nothing else was written outside `refine-logs/` and the session scratchpad.
