...
1. **Raw space, train split, not the deployed head space and not test (L1, inherited from F95 §6).**
   **[ERRATUM — see the appended ERRATUM at the end of this record. The inherited justification (F47's
   "head LOO 0.998") is a **CLIP** number; the deployed **Qwen** heads measure 0.9406 / 0.8915 / 0.8154,
   and F113 has since shown a fold-head arena is unsaturated at ~35 s CPU per fold-head. This record's
   K-VGA-3 bar is a within-session *relative* comparison and is untouched — indeed strengthened, since
   F113 measures the verifier features getting *worse* in head space.]**
