# C09 Stage-0 (A0) — preregistration **v14** (round-13 repairs)
...
> ## STATUS: `V14_REPAIRED_NOT_FROZEN_NOT_SUBMITTED` — awaiting fresh independent review.
>
...
> **this file**.
>
> **`v1–v13 are superseded in full and must not be implemented.`** Each carries its own
...
## 0. What v14 changes, and how this design reached its present form

Round 13 returned **`0C / 0H / 5I`** and discharged round 12's High: it re-derived the
whole F88 transfer from the primary tables itself and confirmed `|P_0| ≈ 79 / 60`,
`n_unstable ≈ 9 / 8`, `|P_{τ_hi}| ≈ 40 / 30`, together with every one of the twelve
figures in §5.3's re-derived `k` grid, both legs of §5.2's knife edge, §6.3's restored
expectation and §10's restored bullet. Its bottom line: *"The science is finished and I
could not move it — for the fifth round running."*

Four of its five findings are **inside the material v13 added**, and three are provenance
failures of exactly the kind round 12's High was about. All five are text edits.

1. **§4.2's derivation block — the one block written to be checkable — mis-transcribed
   and mis-located its own primary source (R13 I-1).** HateMM's final-protocol per-seed
   triple is **`28 / 26 / 26`** (`ERRPAT_HateMM §1`'s cell table), not `26 / 27 / 27`;
   the ZH seed-consensus table is in **§2** (*"ERROR INVENTORY (Tier 2, final-epoch
   protocol)"*), not §1; and the val-sel counterfactual is **`≈ 77`**, not `78`
   (`24/26.3333 × 84.3333 = 76.86`). The sum, the mean and every derived figure were
   already right — the row is pinned by `(3/3, a, b) = (25, 2, 1)` — but a block whose
   purpose is two-minute checkability must match the table it cites. **All corrected,
   and the val-sel row is now given in this block's own `(|P_0|, a, b)` order.**
2. **Four statements still stood on the retired `|P_0| ≈ 76 / 55` (R13 I-2)**, so the
   document asserted two values of the same quantity: §5.2's *"≈76 values"*, §6.2's
   `k ≈ 76` worked mapping, §5.3's `R(x)` rounding example built on `1.5 × 55`, and §0's
   history bullet. **All re-based on `79`** — including the rounding example, which now
   uses the live hazard `1.5 × 79 = 118.5 → 119` rather than a ZH figure that is now an
   integer.
3. **§6.1's new rung-4 headwind was attributed to lines that do not contain it (R13
   I-3).** `HEADSPACE_TRANSFER_PREGATE.md §4.10` is headed *"3-seed means, head arena.
   RAW column re-read from `vsw_main_hatemm_OUT.json`"* — a **HateMM-only** ladder whose
   three-entry column is **per seed**, not per dataset, so no cross-dataset comparison
   exists at `:644-647`. **The real cross-dataset sign flip is stronger and is now cited
   where it lives** (`:784`, `:786-790`).
4. **§5.2's reach bullet misattributed the MHC-ZH knife-edge leg (R13 I-4).** A one-item
   shortfall there closes the branch on **power**, not reach — `29/579 = 0.05009` still
   clears the bar, while `p_w = 29 < 30` fires. **Corrected.**
5. **The scope statement diverged between §0 and §12 again (R13 I-5, an R11 I-2
   regression).** **One convention, written once, copied.**

**Scope of v14, stated exactly** — one convention, this paragraph, copied verbatim into
§12. **Nine hunks:** the title; the STATUS block; §0; §4.2's derivation block; one
sentence of §5.2's analysis-set paragraph; one bullet of §5.2's knife-edge paragraph;
one parenthetical of §5.3; one sentence of §6.1; one sentence of §6.2; and §12.
**§§1, 2, 3, 4.1, 4.3, 4.4, 5.1, 5.4–5.6, 6.3, 7, 8, 9, 10 and 11 are byte-identical to
v13.** **No decision rule, threshold, feature set, null, gate, operating-point definition
or arithmetic rule has changed since v8.**

### What v9 changed, for the record
...
Importants **all in the front matter**, which no earlier round had audited. v9 rewrote
§0's "retained repairs" list (which had asserted, in the affirmative, the
OLS-residualised conditional null §5.2 withdraws and the per-`(dataset, seed)`-cell
permutation unit §5.2 replaces) as the history subsection below; rewrote the stale STATUS
block; corrected §1's *"establishes that no operator exists"* to *"establishes the
existence of no operator"*; and added the F51 paragraph to §11 with its companion bullet
to §10. **No decision rule, threshold, feature set, null, gate or arithmetic changed in
v9 or v10.**
...
at `:784` gives `F89_T4 −0.0063` against HateMM's `+0.0054`, and `:786-790` records that
...
## 12. Repair ledger — the 5 round-13 findings

Round 13 **discharged round 12's High**: it re-derived the whole F88 transfer from the
primary tables itself and confirmed `|P_0| ≈ 79 / 60`, `n_unstable ≈ 9 / 8` and
`|P_{τ_hi}| ≈ 40 / 30`, together with all twelve figures in §5.3's re-derived `k` grid,
both legs of §5.2's knife edge, §6.3's restored expectation and §10's restored bullet.
Its bottom line was *"The science is finished and I could not move it — for the fifth
round running."* Four of its five findings sit inside the material v13 added.

**Scope of v14, stated exactly** — one convention, this paragraph, copied verbatim into
§0. **Nine hunks:** the title; the STATUS block; §0; §4.2's derivation block; one
sentence of §5.2's analysis-set paragraph; one bullet of §5.2's knife-edge paragraph;
one parenthetical of §5.3; one sentence of §6.1; one sentence of §6.2; and §12.
**§§1, 2, 3, 4.1, 4.3, 4.4, 5.1, 5.4–5.6, 6.3, 7, 8, 9, 10 and 11 are byte-identical to
v13.** **No decision rule, threshold, feature set, null, gate, operating-point definition
...
| # | round-13 finding | repair | where |
|---|---|---|---|
| **I-1** | §4.2's derivation block — written to be checkable in two minutes — mis-transcribed and mis-located its own primary source: HateMM's final per-seed triple is `28 / 26 / 26`, not `26 / 27 / 27`; the ZH consensus table is in **§2**, not §1; and the val-sel counterfactual is `≈ 77`, not `78` | all three **corrected at source** (`ERRPAT_HateMM §1`'s cell table gives `28 / 26 / 26` for the final rows; `ERRPAT_MHC-ZH §2` is headed *"ERROR INVENTORY (Tier 2, final-epoch protocol)"*; `24 / 26.3333 × 84.3333 = 76.86`), the val-sel alternative restated in this block's own `(\|P_0\|, a, b)` order as `(24, 2, 3)`, and the ZH protocol match named. **`Σ = 80`, mean `26.6667` and every derived figure were already right** — the row is pinned by `(25, 2, 1)` | §4.2 |
| **I-2** | four statements still stood on the retired `\|P_0\| ≈ 76 / 55` | all four re-based on `79`: §5.2's *"≈76 values"*; §6.2's mapping (`k ≈ 79`, `1 − 0.2124(1−ov)`, `ov ≈ 0.765`, `0.9894`, with the retired figures shown to give the same conclusion at the same margin); §5.3's `R(x)` example, now on the **live** hazard `1.5 × 79 = 118.5 → 119`; and §0's history bullet | §5.2, §5.3, §6.2, §0 |
| **I-3** | §6.1's new rung-4 headwind was attributed to lines that contain no cross-dataset comparison — §4.10 is a HateMM-only ladder whose three-entry column is per **seed** | attribution corrected (the four numbers restated as **HateMM 3-seed means**, with the ladder's own header quoted), and **the real cross-dataset sign flip cited where it lives**: `:784`'s ZH `F89_T4 −0.0063` against HateMM's `+0.0054`, and `:786-790`'s *"T2b/T4 are sub-0.007 and sign-unstable across the two head-space arenas"*, which `:902` books as *"same magnitude, opposite sign"* — a **stronger** headwind than v13 claimed | §6.1 |
| **I-4** | §5.2's reach bullet misattributed the MHC-ZH knife-edge leg: a one-item shortfall closes the branch on **power**, not reach | corrected with both arithmetics shown — HateMM closes on reach at a three-item shortfall (`0.0497`), MHC-ZH at **two** (`0.0484`), and a **one**-item shortfall there fires `p_w ≤ 29 < 30` while reach still clears at `0.05009` | §5.2 |
| **I-5** | the scope statement diverged between §0 and §12 again (an R11 I-2 regression), and both understated the diff | **one convention, written once above and copied verbatim into §0**, counted against the actual hunks | §0, §12 |

**What round 13 checked and left below its own line, left as it left it:** §10's
*"expected to be closed on power on MHC-ZH"* (survives, and now reads as an expectation
rather than an arithmetic certainty); §4.3's `|P_0|/n ≥ 0.10` remark (now `0.1062 /
0.1036`, **better** than under the retired figures); the `≈ 1.4 %` bank-channel magnitude
(`1/79 = 1.27 %`); `GATE-LEDGER`'s six trainlog reads of mixed files, safe on
`headspace_fidelity.py`'s hard `VAL_RE` filter; §5.6's *"30-epoch Adam"* where
`src/run_rac.py:684` uses `AdamW`; `ITEM-STRATUM` occupancy not being emitted the way the
row strata are (direction conservative); `queued.screening_arena_switch`; §2's
*"~25–60 s"* against a measured `33.2`; and the `(τ, k)`-cell wording where `k` is a
per-dataset function of `|P_τ|`.

---

*v14. No hash frozen, no config written, no code implemented, no namespace created, no
