...
**Status:** **FINAL.** GIR landed **DEAD** (F43, record `b64a85b`, orchestrator-spot-checked), so the wave-3 pool is empty and every in-box axis is closed by a binding verdict or a calibrated-zero gate; the terminus claim below is **unconditional**. Promoted to `TERMINUS_round3_mllm_plus3.md` (naming parallel to `TERMINUS_round2_mllm_plus3.md`) and marked FINAL for hand-off to the user (team-lead authorized).
...
**Terminus condition (now met).** After S2S (F37), CTF (F39), APX+AVC (F41), W2-A (F42), and now GIR (F43), the wave-2/wave-3 candidate pool is **empty** — six kills in one day, zero GPU-hours wasted (all closed at binding verdicts or calibrated-zero $0 gates). **Every axis reachable inside the frozen constraint box is closed** — the goal is unmet not for want of ideas but because the constraint box has no remaining opening. That is the claim this document supports and hands to the user. The loop now enters **monitoring mode** (no new GPU or probes on closed axes without a user ruling or a genuinely non-isomorphic structural idea that defeats the banked meta-patterns).
...
**Members:** W2-A (transcript-first grounded vision key), **GIR** (isolated grounded-incongruity residual, F43), C3-nontarget (dense reasoning-text fusion), C3-target (predicted-target one-hot), P4 (schema-field distillation), P5 (counterfactual twins).
...
**Meta-pattern:** the cross-modal **interaction term is already present** — `text_feats` is a *joint* frames+transcript forward (verified source-level), so re-pooling (W2-A) or generating text (C3) only **reshuffles information the pooled channels already carry**. **CLOSED.**

> **GIR — KILL both datasets (F43, record `b64a85b`, calibrated-zero $0 gate, orchestrator-spot-checked).** The isolated incongruity residual `r = grd − ungrd_vis` carries **zero** conditional label information over the strictest baseline `Z_best-incl-grd = concat(CLIP img+text, Qwen img+text, grd)` (12544-d, which already contains the full grounded key). Design-canonical `r_field`: HateMM **+0.0000** CI[−0.0054,+0.0056], MHC-EN **−0.0064** CI[−0.0159,+0.0029]; exact-subsumption `r_cache = grd − img_feats`: **+0.0012 / −0.0051**; HateMM covered-rows dilution control **−0.0035** — every decision-family estimate an order of magnitude under the +0.040 bar with bootstrap CI-lower ≤ 0, calibration accZA=1.0000 on all five cells (genuine null). Its live branch was correctly and non-vacuously fired (W2-A raw-FAIL + oracle-SURVIVE). *Mechanism:* `r_cache` is an **exact linear function of two baseline columns** (`grd` and `Qwen_img`, verified residual norm 0), so it is mathematically **subsumed by the W2-A K9 null** — isolating the interaction term recovers nothing, exactly as pre-declared (`WAVE3_CANDIDATES.md` §2b/f). **GIR was the last candidate in the pool; Axis F, and the wave-3 pool, are fully closed.**
...
4. **Cross-modal "grounding" is a CLIP-redundancy null, because the joint forward already banks the interaction (W2-A F42, `7228373`; GIR F43, `b64a85b`).** The transcript-conditioned vision key adds zero conditional info because `text_feats` is *already* a joint frames+transcript pool (verified source-level). *Mechanism:* the interaction term the mechanism claims to add is not missing from the pipeline. **GIR sharpens this from an empirical null to a mathematical identity:** the isolated incongruity residual `grd − ungrd_vis` is an *exact linear subset* of the baseline (verified residual norm 0), so even the interaction term *in isolation* carries zero conditional information — the grounding null is not a weak effect but an exact subsumption.
...
- Round-3 binding verdicts / gates (all LOCAL — SLURM or local CPU; cloud numbers are triage-only and never mixed): W2-A `7228373`/`688ef87` (F42), CTF `0eb6d33` (F39), APX `9c54faf` (F41), **GIR `b64a85b` (F43, non-binding calibrated-zero $0 gate, orchestrator-spot-checked)**, S2S `2c96ab6` (F37, Modal triage), W2-B `0f43bdd` (F27, Modal triage), B5 `50f01b9` (F34).
...
- **RESOLVED:** GIR verdict landed **DEAD** (F43, record `b64a85b`, `GIR_G0COND_GATE_OUT.json`, orchestrator-spot-checked) — folded into §0 (terminus condition now met), §1 Axis-F (KILL both datasets, exact subsumption), and §3 ledger item 4. The terminus claim is now unconditional; document promoted `TERMINUS_round3_DRAFT.md` → `TERMINUS_round3.md` → `TERMINUS_round3_mllm_plus3.md` (canonical, naming parallel to `TERMINUS_round2_mllm_plus3.md`) and marked **FINAL** (team-lead authorized) for hand-off to the user.
