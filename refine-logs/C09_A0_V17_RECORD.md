# C09 Stage-0 (A0) — preregistration **v17** (round-16 repairs)
...
> ## STATUS: `V17_DESIGN_GO_0C_0H_0I_ROUND_17 — READY TO HASH-FREEZE, NOT IMPLEMENTED, NOT FROZEN, NOT SUBMITTED`
>
> **Round 17 returned `GO (0 Critical / 0 High / 0 Important)`** — the design's first
> clean review — with the bottom line *"The science is finished and I could not move it
> — for the ninth round running… **The DESIGN is ready to hash-freeze.**"* Record:
> `refine-logs/C09_A0_V17_PREREG_REVIEW.md`. **This closes the design review; it
> authorizes nothing.**
>
> **What remains, in order:** implement the analysis script, the config and the CPU
> sbatch exactly as this document specifies; pass a **separate** independent
> code/resource review to `GO`; hash-freeze the frozen set into
> `refine-logs/C09_A0_RECORD.md` with a measured-claims register; then the three
> submission preconditions below plus the immediately-prior `squeue -u jehc223`
> empty-check, sha256 re-verification and namespace-absence check; then **one**
> 8-CPU / 32-GB / no-GPU / no-`--time` submission of ≈115 CPU-minutes.
>
...
> seventh round running… §8 and §9 are byte-identical to v8 by hash"*) → v16
> `C09_A0_V16_RECORD.md` → R16 `C09_A0_V16_PREREG_REVIEW.md` (`REVISE 0C/0H/3I` —
> *"The science is finished and I could not move it — for the eighth round running…
> Does any remaining finding change a verdict, a rule, a threshold, a gate, an operating
> point, or the scope of any conclusion? No — none of the three does"*) → **this file**.
>
> **`v1–v16 are superseded in full and must not be implemented.`** Each carries its own
> ledger; **the ledger of THIS version is always §12** (R16 I-3: this sentence froze at
> *"the v13 ledger"* when v14 forked and had been false in v14, v15 and v16). Round 12 confirmed the science intact once more —
...
## 0. What v17 changes, and how this design reached its present form

Round 16 returned **`0C / 0H / 3I`** and answered the question this loop now turns on
directly: *"Does any remaining finding change a verdict, a rule, a threshold, a gate, an
operating point, or the scope of any conclusion? **No — none of the three does.**"* It
re-verified `§8`+`§9` byte-identity to v8 by its own hash
(`md5 a17b56954ee6955013327f82a03904f7` across v8, v15 and v16), section-hashed v15 → v16
to confirm the six declared edit sites, re-derived the F88 transfer and all twelve `k`-grid
figures from the primary tables, and **reconstructed both confusion matrices exactly** from
`posrate_deployed` / `posrate_bank` — recovering `mF1 0.88378 = 0.8838` (HateMM) and
`0.87466 = 0.8747` (MHC-ZH) — confirming that the accuracy leg binds at `τ_hi` on both
datasets. It also confirmed that **no C09 namespace exists anywhere on disk**.

All three of its findings are statements this document makes about its own repair
history. All three are corrected here:

1. **§12's row I-4 claimed a corrected string that appears nowhere in v16 (R16 I-1).**
   The gate-count gloss was not "corrected to *its nine HALT gates and its three reporting
   instruments*" — both sentences were **deleted and rewritten** to describe the hashed
   span instead, which round 16 calls *"a better repair than the one claimed"*. **Row I-4
   now says what was done.**
2. **§12 misquoted round 15's bottom line (R16 I-2).** Round 15 ends *"…verdict or
   **scope**."*; v16 carried round 14's five-word *"the scope of any conclusion"* into
   round 15's mouth. **Quoted exactly.**
3. **The STATUS block still pointed at *"the v13 ledger"* (R16 I-3)** — a sentence that
   froze when v14 forked and had been false in v14, v15 and v16. **Restated as a
   version-independent pointer** so it cannot go stale again.

**Scope of v17, stated exactly** — one convention, copied into §12 character-identical
apart from the `§0`↔`§12` cross-reference. **Five edit sites:** the title; the STATUS
block; §0; §12; and the closing trailer. **§§1–11 are byte-identical to v16**, and
**§§8 and 9 are byte-identical to v8** (rounds 15 and 16, independently, by hash).
**No decision rule, threshold, feature set, null, gate, operating-point definition or
arithmetic rule has changed since v8.**

**What is NOT yet done, stated plainly** (round 16 names it and it is true): **the
analysis script and the sbatch driver do not exist.** This document is a design. Freezing
requires implementing them, hash-freezing the frozen set, and passing a **separate**
independent code/resource review before any `sbatch`.
...
## 12. Repair ledger — the 3 round-16 findings

Round 16's verdict was **`REVISE (0C / 0H / 3I)`**, and it answered the question this
loop now turns on in terms: *"Does any remaining finding change a verdict, a rule, a
threshold, a gate, an operating point, or the scope of any conclusion? **No — none of
the three does.** All three are statements the document makes about its own repair
history… §§1–11's governing text is untouched by all three, and for the fourth round
running the blockers sit entirely outside it."* Round 16 independently re-verified
`§8`+`§9` byte-identity to v8 by hash across three versions, section-hashed v15 → v16 to
confirm the declared edit sites, re-derived the F88 transfer and all twelve `k`-grid
figures, **reconstructed both confusion matrices exactly** from `posrate_deployed` /
`posrate_bank` (`mF1 0.88378` and `0.87466`, matching the banked `0.8838` / `0.8747`),
and confirmed that **no C09 namespace exists anywhere on disk**.

**Scope of v17, stated exactly** — one convention, copied into §0 character-identical
apart from the `§0`↔`§12` cross-reference. **Five edit sites:** the title; the STATUS
block; §0; §12; and the closing trailer. **§§1–11 are byte-identical to v16**, and
**§§8 and 9 are byte-identical to v8** (rounds 15 and 16, independently, by hash).
**No decision rule, threshold, feature set, null, gate, operating-point definition or
...
| # | round-16 finding | repair | where |
|---|---|---|---|
| **I-1** | §12's row I-4 claimed the gate-count gloss had been *"corrected in both copies to `its nine HALT gates and its three reporting instruments`"* — a string that appears nowhere in v16, one row after repairing exactly that defect class | **row restated to what was done**: both sentences carrying *"§8 (all ten gates)"* were **deleted and rewritten** to describe the hashed span (`## 8` through the line before `## 10`), which round 16 calls *"a better repair than the one claimed"*; the gate-count defect is genuinely gone and the byte-identity claim now rests on a hash rather than a gloss | §12 |
| **I-2** | §12 quoted round 15's bottom line with round 14's five-word tail spliced on: round 15 ends *"…verdict or **scope**."*, not *"…verdict or the scope of any conclusion"* | **quoted exactly**, with the ellipsis joining round 15's two separated sentences left as the legitimate elision it is | §12 |
| **I-3** | the STATUS block read *"the v13 ledger is §12"* — a pointer that froze when v14 forked and had been false in v14, v15 and v16, uncaught by three rounds | **restated version-independently** (*"the ledger of THIS version is always §12"*) so it cannot go stale again, with the history named | STATUS |

**What round 16 checked and left below its own line, left as it left it:** Band B's
quantifier not formally excluding Band C in one corner (§9 is two-valued and complete —
round 16 re-derived exhaustiveness itself); §5.6's *"30-epoch Adam"* against
`src/run_rac.py:684`'s `torch.optim.AdamW`; §2's *"~25–60 s"* against a measured minimum
of `33.2` and the budget's ≈116-vs-≈115 rounding; `ITEM-STRATUM` occupancy not being
emitted the way the row strata are (conservative — `K-FELDMAN` requires both families, so
the conditional null can only remove CONTINUEs); the `(τ, k)`-cell wording where `k` is a
per-dataset function of `|P_τ|`; §3's rendering of `LITSWEEP3:91`, inherited verbatim from
`label_use_legality.text_2` and running **against** C09;
`directions_tried.json::dead[Fxx]` not being a literally resolvable path (repository
convention); `ksweep_OUT.json`'s `dev` sub-key being `null` on exactly the four
`MHC_EN_ARM-V/*/valsel` curves; §6.1's *"`:644-647` … headed"* where the heading is at
`:625`; the reported `mF1 0.8831` reconstruction attributed to round 15 (round 16's exact
reconstruction lands on `0.8838` and confirms the same conclusion); and typographic drift
inside italic quotes.

---

*v17. No hash frozen, no config written, no code implemented, no namespace created, no
