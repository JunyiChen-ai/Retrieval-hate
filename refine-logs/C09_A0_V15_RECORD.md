# C09 Stage-0 (A0) — preregistration **v15** (round-14 repairs)
...
> ## STATUS: `V15_REPAIRED_READY_TO_FREEZE_PENDING_A_GO_REVIEW` — awaiting fresh independent review.
>
...
> text"*) → **this file**.
>
> **`v1–v14 are superseded in full and must not be implemented.`** Each carries its own
...
## 0. What v15 changes, and how this design reached its present form

Round 14 returned **`0C / 0H / 3I`** with the bottom line *"The science is finished and I
could not move it — for the sixth round running… for the third round running the
blockers are entirely outside §§1–11's governing text."* It verified the v13→v14 diff
site by site, re-derived §4.2's transfer and all twelve `k`-grid figures itself, and
additionally **diffed v8 → v14 to confirm that §8 (all ten gates) and §9 (the whole
decision rule) are byte-identical to v8** — the strongest form the "no rule has changed"
claim can take. All five of round 13's findings were discharged on substance. The three
residues are one-line text edits and are made here:

1. **§0's `K-DEG` history bullet was half-repaired (R14 I-1):** v14 re-based the
   numerator on `79` and left the figure it derives at the retired-`76` value.
   `2×79/744 = 0.212366`, so `ov = 0.95` gives `0.9894`, not `0.9898`. **Corrected**, and
   §6.2 already labelled `0.9898` as what the retired `76` gives.
2. **The MECHFIX cross-check locator was wrong by five lines, in three places (R14
   I-2).** `HEADSPACE_TRANSFER_PREGATE.md:786-790` is the ZH degeneracy block; the
   MECHFIX paragraph — the `T1/T2a/T2b/T4` test reads and the *"sign-unstable across the
   two head-space arenas"* conclusion — is at **`:791-796`**. Every quoted number was
   exact; only the locator was wrong. **Corrected in §0, §6.1 and §12.** *(Provenance
   worth recording: round 13 supplied `:786-790` and v14 adopted the reviewer's locator
   without re-opening the file — which is exactly how the defect round 13 was fixing
   arose. The rule this re-teaches is the repository's own: re-read the source, never
   transcribe a locator from a review.)*
3. **The unified scope statement miscounted its own diff (R14 I-3):** it said "nine
   hunks" while enumerating ten, and omitted the closing trailer. **Corrected to eleven
   edit sites, in both copies.**

**Scope of v15, stated exactly** — one convention, this paragraph, copied verbatim into
§12. **Six edit sites:** the title; the STATUS block; §0 (this section, including the
`K-DEG` history bullet's `0.9894` and the `:791-796` locator); one sentence of §6.1 (the
same locator); §12; and the closing trailer. **§§1–11 are byte-identical to v14 except
for that single §6.1 locator**, and **§§8 and 9 are byte-identical to v8**. **No decision
rule, threshold, feature set, null, gate, operating-point definition or arithmetic rule
has changed since v8.**
...
## 12. Repair ledger — the 3 round-14 findings

Round 14's verdict was **`REVISE (0C / 0H / 3I)`** with the bottom line *"The science is
finished and I could not move it — for the sixth round running… All three findings are
one-line text edits… None touches a rule, threshold, feature set, null, gate, operating
point, verdict or scope of any conclusion."* Round 14 additionally **diffed v8 → v14 and
confirmed that §8 (all ten gates) and §9 (the whole decision rule) are byte-identical to
v8**, which is the strongest form this document's standing "no rule has changed since v8"
claim can take, and it re-derived §4.2's transfer, all twelve `k`-grid figures, the caps,
the power rule, the band exhaustiveness and the three sha256 pins independently.

**Scope of v15, stated exactly** — one convention, this paragraph, copied verbatim into
§0. **Six edit sites:** the title; the STATUS block; §0 (including the `K-DEG` history
bullet's `0.9894` and the `:791-796` locator); one sentence of §6.1 (the same locator);
§12; and the closing trailer. **§§1–11 are byte-identical to v14 except for that single
§6.1 locator**, and **§§8 and 9 are byte-identical to v8**. **No decision rule, threshold,
feature set, null, gate, operating-point definition or arithmetic rule has changed since
v8.**

| # | round-14 finding | repair | where |
|---|---|---|---|
| **I-1** | §0's `K-DEG` history bullet was half-repaired: v14 re-based the numerator on `79` and left the derived figure at the retired-`76` value, so the document asserted two values of one derived quantity | **`0.9898 → 0.9894`** (`2×79/744 = 0.212366`; `1 − 0.212366 × 0.05 = 0.98938`), with the origin named. §6.2 already stated it correctly **and** labelled `0.9898` as the retired-`76` value, which is why the two now agree | §0 |
| **I-2** | the MECHFIX cross-check was cited to `:786-790` — the ZH degeneracy block — in three places; the paragraph is at **`:791-796`**. Every quoted number was exact | **locator corrected in all three places** (§0, §6.1, §12), and the provenance recorded: round 13 supplied the wrong locator and v14 adopted it without re-opening the file, which is the same failure the repair was fixing. The rule re-stated: **re-read the source; never transcribe a locator from a review** | §0, §6.1, §12 |
| **I-3** | the unified scope statement said *"Nine hunks"* while enumerating ten, and omitted the closing trailer | corrected to **eleven edit sites** in both copies of v14's paragraph, and v15's own scope paragraph states **six**, counted against the diff | §0, §12 |

**What round 14 checked and left below its own line, left as it left it:** §4.3's
`≈ 1.4 %` bank-channel magnitude (now `1/79 = 1.27 %`, disclosed); §2's `~25–60 s` lower
bound against a measured `33.2` and the ≈115-vs-116 budget rounding; §5.6's *"30-epoch
Adam"* where `run_rac.py:684` is `AdamW`; `ITEM-STRATUM` occupancy not being emitted the
way the row strata are (direction conservative, toward KILL);
`queued.screening_arena_switch` being a proposed-not-ruled practice change; the
`(τ, k)`-cell wording where `k` is a per-dataset function of `|P_τ|`; §6.2's F98 DEG-A
`0.9570` being the HateMM **seed-0** cell (the 3-seed mean is `0.9575`, a headwind either
way); §3's rendering of `LITSWEEP3:91` as *"at most +0.001-0.006"* against the source's
*"at most the +0.001–0.006 legal slice"*, a compression inherited verbatim from the
registry's own `label_use_legality.text_2` and running **against** C09; and §9's
raw-arena leg reporting `Δ` above `+0.010` with CAL-3 discharged by a comparator of
record rather than a strict ceiling, correctly labelled as such.

---

*v15. No hash frozen, no config written, no code implemented, no namespace created, no
