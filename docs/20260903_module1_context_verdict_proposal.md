...
## 8. 结果：第 3 节第 1 步（引出层面）HateMM 未通过，模块 1 本变体淘汰（2026-09-03 22:10）

来源：`runs/20260903_hier_evidence_mil/module1_elicitation/hatemm/cells_qwenctx.json`（与 `cells_qwen.json` 配对；val 109、test 215 个视频全部有上下文裁定）。

| HateMM | 原 K30（qwen） | 上下文 K30（qwenctx） | 预注册判据 |
|---|---|---|---|
| test K30 精度 / 召回 | .554 / .489 | **.495** / .519 | 精度须上升 → **不满足** |
| test "K30 是、K4 否" n / GT 率 | 603 / .158 | 1081 / **.167** | ≥ .30 → **不满足** |
| test "K30 是、K4 是" n / GT 率 | 5649 / .597 | 6349 / .551 | — |
| test "K30 否、K4 是" n / GT 率 | 5845 / .417 | 5145 / .450 | — |
| K30 触发窗中 K4 也触发比例 | .910 | .863 | 不得 ≥ .97 → 满足 |
| val K30 精度 / 召回 | .513 / .371 | .512 / .491 | — |
| val "K30 是、K4 否" n / GT 率 | 266 / .767 | 626 / .565 | — |

读法：给 K30 窗口加所属块的转录上下文后，VLM 触发更多（test 召回 +.03、val +.12），但新增触发的秒 GT 率与原来的误报格子一样低（.167），精度反而从 .554 降到 .495；块内落点也没有变准（"都触发"格子 GT 率 .597 → .551）。上下文让模型"更敢标"，没有让它分清模糊线索是否针对受保护群体。假设被证伪。按第 3 节第 4 条：模块 1 本变体淘汰，不进训练无关对照行、不进训练；论文模块 1 保持原裁定（修订 1）。HateMM train 与 HateClipSeg 的抽取让其跑完只作记录（`*_segscoreK30_qwenctx.jsonl` 保留、PROVENANCE 记录为已淘汰变体）。规则 9 修改轮次：本轮未进入训练，不计入。

后续可选方向（未开工、未预注册）：上下文引出没有降低误报，说明 HateMM 短窗误报的来源可能不在转录（视觉符号、手势、画面文字），或者 test 与 val 上 K30 可靠性本就相反（第 1 节）而不是引出方式的问题；在这两点弄清之前不再改模块 1。

### 8.1 HateClipSeg（记录，`runs/20260903_hier_evidence_mil/module1_elicitation/hateclipseg/cells_qwenctx.json`）

| HateClipSeg test | 原 K30 | 上下文 K30 | 预注册判据 |
|---|---|---|---|
| K30 精度 / 召回 | .756 / .360 | **.674** / .394 | 精度 ≥ .726 → **不满足** |
| "K30 是、K4 否" n / GT 率 | 873 / .732 | 1526 / .581 | — |
| "K30 是、K4 是" n / GT 率 | 3842 / .762 | 4265 / .708 | — |
| K30 触发窗中 K4 也触发比例 | .826 | .733 | 满足 |

两语料同一方向：加块上下文后 VLM 触发更多、精度下降（HateMM −.06、HateClipSeg −.08）。淘汰结论不变。抽取文件已回传本机 `data/MLLM_scores/`，`PROVENANCE.md` 已加"已淘汰变体"条目（HateMM 1068、HateClipSeg 393 视频，HateMM train 1 个 video_ok false）。
