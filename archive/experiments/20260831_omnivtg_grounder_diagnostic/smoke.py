#!/usr/bin/env python3
"""Technical smoke for the official OmniVTG temporal-grounding checkpoint."""
...
from protocol import MODEL_ID, QUERY, parse_interval
...
def calculate_timestamps(indices, video_fps, merge_size=2):
    ...
    start = indices[0]
    raw = [(index - start) / video_fps for index in indices]
    return [
...
def prepare_inputs(video_path: Path, processor):
    ...
    if len(timestamps) != frame_count:
        raise RuntimeError(f"timestamp/frame-grid mismatch: {len(timestamps)} vs {frame_count}")

    system = f"{IM_START}system\n{SYSTEM_MESSAGE}{IM_END}\n"
    user = "".join(
        f"<{timestamp:.1f} seconds>{VISION_START}{VIDEO_TOKEN}{VISION_END}"
        for timestamp in timestamps
    )
    user += PROMPT.format(query=QUERY)
    user = f"{IM_START}user\n{user}{IM_END}\n{IM_START}assistant\n"
    input_ids = torch.cat([
        processor.tokenizer(system, add_special_tokens=False, return_tensors="pt")[
            "input_ids"
        ][0],
        processor.tokenizer(user, add_special_tokens=False, return_tensors="pt")[
            "input_ids"
        ][0],
    ])[None]
...
def main():
    ...
    prompt_token_ids, mm_data = prepare_inputs(args.video, processor)
    ...
    if interval is None:
        raise RuntimeError("OmniVTG completion did not contain one valid final interval")
