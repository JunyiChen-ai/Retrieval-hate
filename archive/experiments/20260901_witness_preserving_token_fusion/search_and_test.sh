...
SEARCH="$ROOT/runs/20260901_witness_preserving_token_fusion/val_search/$corpus"
FORMAL="$ROOT/runs/20260901_witness_preserving_token_fusion/formal_val_selected_seed234/$corpus"
...
for lr in "$lr_low" "$lr_high"; do
  trial=$((trial + 1)); out="$SEARCH/$(printf 'trial_%02d' "$trial")"
  mkdir -p "$out"; echo $$ > "$out/run.pid"
  "$PY" "$EXP/train.py" --corpus "$corpus" --arm anchor \
    --alpha-fusion 0 --lr "$lr" --output-dir "$out" > "$out/run.log" 2>&1
  anchor_logs+=("$out/train_log.json")
done

declare -A shifted_logs
lr_index=0
for lr in "$lr_low" "$lr_high"; do
  anchor_reference="${anchor_logs[$lr_index]}"; lr_index=$((lr_index + 1))
  for alpha in .25 .5 .75; do
    trial=$((trial + 1)); out="$SEARCH/$(printf 'trial_%02d' "$trial")"
    mkdir -p "$out"; echo $$ > "$out/run.pid"
    "$PY" "$EXP/train.py" --corpus "$corpus" --arm shifted \
      --alpha-fusion "$alpha" --lr "$lr" --reference-log "$anchor_reference" \
      --output-dir "$out" > "$out/run.log" 2>&1
    shifted_logs["$lr,$alpha"]="$out/train_log.json"
  done
done
...
for lr in "$lr_low" "$lr_high"; do
  for alpha in .25 .5 .75; do
    trial=$((trial + 1)); out="$SEARCH/$(printf 'trial_%02d' "$trial")"
    mkdir -p "$out"; echo $$ > "$out/run.pid"
    "$PY" "$EXP/train.py" --corpus "$corpus" --arm aligned \
      --alpha-fusion "$alpha" --lr "$lr" \
      --reference-log "${shifted_logs[$lr,$alpha]}" --output-dir "$out" \
      > "$out/run.log" 2>&1
  done
done
...
for role in anchor aligned shifted; do
  out="$FORMAL/$role"; mkdir -p "$out"; echo $$ > "$out/run.pid"
