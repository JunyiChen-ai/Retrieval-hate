...
from method import ARMS, TemporalCoalitionCreditMIL  # noqa: E402
...
def run_epoch(model, data_loader, optimizer, args):
    ...
    for feats, labels, lengths, mask, _ in data_loader:
        ...
        responsibility, aligned, trained, witness_count = (
            model.responsibility_terms(output, mask, lengths, labels))
        loss = (mil + args.lambda_smooth * smooth +
                args.lambda_contrast * contrast + args.alpha * responsibility)
        optimizer.zero_grad(set_to_none=True)
        loss.backward()
        optimizer.step()
        terms = {
            "loss": loss, "mil": mil, "smooth": smooth,
            "contrast": contrast, "responsibility": responsibility,
            "aligned_agreement": aligned, "training_target_agreement": trained,
        }
        ...
        witness_total += witness_count
        seen += batch_size
    result = {name: value / max(seen, 1) for name, value in totals.items()}
    result["responsibility_witness_count"] = witness_total
    return result


@torch.no_grad()
def predict(model, data_loader, device, include_diagnostics):
    model.eval()
    frame_records, video_records, labels_out = {}, {}, {}
    diagnostic_sum = {"aligned": 0.0, "trained": 0.0, "count": 0}
    for feats, labels, lengths, mask, video_ids in data_loader:
        ...
        video_scores = model.video_scores(output["probs"], mask_device,
                                          lengths_device)
        if include_diagnostics and model.alpha > 0:
            _, aligned, trained, count = model.responsibility_terms(
                output, mask_device, lengths_device, labels_device)
            diagnostic_sum["aligned"] += float(aligned) * count
            diagnostic_sum["trained"] += float(trained) * count
            diagnostic_sum["count"] += count
    ...
    count = diagnostic_sum["count"]
    diagnostics = {
        "aligned_credit_agreement": (diagnostic_sum["aligned"] / count
                                     if count else None),
        "training_target_agreement": (diagnostic_sum["trained"] / count
                                      if count else None),
        "responsibility_witness_count": count,
    }
    return frame_records, video_records, labels_out, diagnostics
...
def reference_metrics(path):
    ...
    if payload["arm"] != "anchor" or payload["alpha"] != 0.0:
        raise ValueError("reference must be an alpha=0 anchor log")
    return payload["selected_validation_metrics"]
...
def selection_key(metrics, reference):
    ...
    if feasible:
        return (2, float(within), -max(ap_drop, roc_drop))
    return (1, -violation, float(within))
...
def main():
    ...
    parser.add_argument("--max-epoch", type=int,
                        help="smoke override; formal runs omit this")
    ...
    config = {
        "date": "2026-09-01",
        "method": "temporal_coalition_credit_mil",
        "code_version_description": (
            "Anchor-compatible MultiHateLoc plus exact three-modality "
            "witness-level coalition credit and a shared local router; no hash."),
        "evaluation_split": "validation_only",
        "args": vars(args), "official_fixed_config": fixed,
    }
    ...
    if args.arm == "anchor" and reference is not None:
        raise ValueError("anchor cannot have a reference")
    if args.arm != "anchor" and reference is None:
        raise ValueError("result-relevant arm requires matched anchor reference")

    best_key, best_epoch, best_state, best_metrics = None, None, None, None
    history, started = [], time.time()
    max_epoch = args.max_epoch if args.max_epoch is not None else fixed["max_epoch"]
    if max_epoch <= 0:
    ...
    for epoch in range(1, max_epoch + 1):
        ...
        if epoch == 1 or epoch % 10 == 0:
            print(f"{args.corpus}/{args.arm}/a={args.alpha:g} epoch={epoch:03d} "
                  f"loss={stats['loss']:.4f} within={metrics['within_roc']:.4f} "
    ...
    payload = {
        "corpus": args.corpus, "arm": args.arm, "alpha": args.alpha,
        "lr": args.lr, "selection_rule": (
            "within primary; pooled AP/ROC max drop .01 against same-lr anchor; "
            "if infeasible minimize maximum violation then maximize within"),
