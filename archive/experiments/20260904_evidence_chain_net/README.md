...
**HateMM seed 234（uoa-lab1，2026-09-04 02:15–04:45，20 trial，每 trial 520–800 s；`runs/20260904_evidence_chain_net/hatemm/seed234/study_summary.json`）**：19/20 trial 被 within 下限（.632）剪掉；全部 trial 的范围 test AP .435–.563、ROC .761–.823、within .492–.651。

| 选法 | trial | 超参 | test AP / ROC / within | val AP / ROC | 选中 epoch |
|---|---|---|---|---|---|
| 唯一未剪的 trial | 8 | lr 7.98e-4, dropout .2, max_seqlen 150 | .464 / .761 / .651 | .789 / .898 | 5 |
| test 目标最高（被剪） | 9 | lr 3.14e-4, dropout .3, max_seqlen 150 | .563 / .823 / .519 | .741 / .861 | 1 |
| val 选中（规则 7 报告口径，被剪） | 13 | lr 9.77e-4, dropout .3, max_seqlen 150 | .524 / .799 / .577 | .853 / .923 | 7 |

对照 hier_evidence_mil 修订 1 HateMM seed 234：.661 / .841 / .646；固定链 .546 / .813 / .575。**结论：修订 1 HateMM 全面低于对照，AP 差 .10–.20；test 目标最高的 trial 选中 epoch 1，即几乎不训练最好，训练越久 test 越差而 val 越好（val AP 到 .85），说明模型在 val 上学到的东西不迁移到 test（5.1 诊断 2、3 的密度反转与门捷径）。** 淘汰，进入修订 2（5.3）。
...
**HateMM seed 234（uoa-lab1，2026-09-04 05:47 起）**：全部 trial 在第 2–10 个 epoch 崩溃（评测器 `ZeroDivisionError`，原因是 val 分数为 NaN）。搜索已停止，不作记录；原因与修复见 5.6。

### 5.6 修订 2 HateMM 崩溃的原因与修复（2026-09-04 06:00）
`ref/nan_probe.py`（用 trial 1 的超参训练几个 epoch，每步检查损失、梯度范数、权重，逐项归因）：第 4 个 epoch 某一步损失有限（video .067）但梯度范数 NaN，该 batch 里有一个正例视频 log(Z0/Z) = −6e−8，即链认为"这个视频没有任何仇恨"的概率 ≈ 1。正例项 −log(1 − Z0/Z) 对 log(Z0/Z) 的梯度是 1/(1 − Z0/Z) ≈ 1.7e7，反传过链后溢出，权重变 NaN。修订 1 没触发是因为 u_t 不读裁定上下文、不会把整个视频压到那么低；HateClipSeg 没触发是因为其正例的裁定证据更密（README 0：train 正例 ρ 中位数 .70 对 HateMM .012——但 HateMM 有 43% 正例 ρ < 1e−3，另一端是 ρ → 1 的正例）。

修复（`train.py` `video_loss`，常数 `LABEL_NOISE_EPS = 1e-3`）：正例项用 log(Z0/Z) 截断到 log(1 − ε) 再算 log(1 − Z0/Z)，即 P(y=1 | 证据) ≥ ε。截断处梯度为 0：证据说"完全没有仇恨"的正例视频不再从边际似然拿梯度（视作标签噪声），它们仍由块级 MIL 和蒸馏项训练。修复后 probe 8 个 epoch 权重、val 输出全部有限。修订 3 两语料搜索都用修复后的代码（HateClipSeg 在 lab3 重启，之前跑的 2 个 trial 删除）。

### 5.5 修订 3（规则 9 第 2/3 次修改）：链后验蒸馏，网络证据作输出（2026-09-04 05:55 起）
