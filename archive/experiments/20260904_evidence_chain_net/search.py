"""Fixed Optuna search for one (corpus, seed), rule 7 of RESEARCH_ITERATION_RULES.md.
    ...
    python experiments/20260904_evidence_chain_net/search.py \
        --corpus hatemm --seed 234 --out-root runs/20260904_evidence_chain_net

Same protocol as the previous candidate's search.py: each trial trains once
(train.py), selects its checkpoint on validation, scores test; objective =
(test AP + test ROC) / 2; trials below the corpus within-video floor are
recorded as pruned. Budget: 20 trials if the first took <= 1 h, else 5.
Search space (README section 1, declared before any search, 2026-09-04):
lr, dropout, max_seqlen only -- the method has no method-level hyperparameters.
"""
