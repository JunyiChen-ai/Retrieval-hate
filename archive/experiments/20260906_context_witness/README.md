...
**已归档：2026-09-06 06:10 NZST用户叫停。每窗口四次VLM推理、预处理成本不可接受，且尚无实验证据支持足够大的收益；不是已验证的性能失败。不恢复本候选抽取或训练。**
...
06:10两机正式抽取进程组及对应本机monitor均已停止，GPU已释放。HateMM完成302/1068视频，HCS完成316/393视频；已完成JSON、日志和配置全部保留回传，不删除缓存，不把缺失部分当完成。未启动任何候选7训练，无test性能结果。最终输出与停止说明见 `runs/20260906_context_witness/cancellation_audit.json`。

工程上四模式含可复用的全空观测；更根本的问题是，在没有量化定位收益前选择了每视频120次VLM调用的高成本方案。本轮按用户成本判断停止，不据此声称模块无效。后续优先基于已完整缓存设计低成本方法；若需新VLM输入，必须说明增量成本及相对廉价输入的必要性，不把冻结/离线当成成本消失。
