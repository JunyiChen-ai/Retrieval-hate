...
from method import ARMS, training_mil_loss, witness_failure_loss  # noqa: E402
...
def run_epoch(model, data_loader, optimizer, args):
    ...
    for feats, labels, lengths, mask, _ in data_loader:
        ...
        mil, per_branch = training_mil_loss(
            model, output, mask, lengths, labels, args.arm, args.gce_q)
        smooth = model.smoothness_loss(output["probs"], mask)
        contrast = model.contrastive_loss(output["embeds"], mask)
        failure, failure_diag = witness_failure_loss(
            model, output, mask, lengths, labels, args.arm)
        loss = (mil + args.lambda_smooth * smooth +
                args.lambda_contrast * contrast +
                args.lambda_failure * failure)
        optimizer.zero_grad(set_to_none=True)
        loss.backward()
        optimizer.step()
        terms = {"loss": loss, "mil": mil, "smooth": smooth,
                 "contrast": contrast, "failure": failure}
        ...
        for name in ("positive_videos", "negative_videos"):
            totals["failure_" + name] = totals.get(
                "failure_" + name, 0.0) + int(failure_diag[name])
        seen += batch_size
...
def reference_metrics(path):
    if path is None:
        return None
    return json.loads(Path(path).read_text())["selected_validation_metrics"]
...
def main():
    ...
    parser.add_argument("--lambda-failure", type=float, required=True)
    parser.add_argument("--lr", type=float, required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--reference-log")
    parser.add_argument("--gce-q", type=float, default=.7)
    parser.add_argument("--seed", type=int, default=234)
    parser.add_argument("--batch-size", type=int, default=32)
    parser.add_argument("--device", default="cuda")
    args = parser.parse_args()
    if args.arm == "anchor" and args.lambda_failure != 0.0:
        raise ValueError("anchor requires lambda_failure=0")
    if args.arm != "anchor" and args.lambda_failure <= 0.0:
        raise ValueError("result-relevant arms require lambda_failure>0")
    ...
    config = {
        ...
        "code_version_description": (
            "MultiHateLoc with GCE unimodal shortcut experts and LfF "
            "relative weighting on fused latent witnesses."),
    ...
    if args.arm != "anchor" and reference is None:
        raise ValueError("non-anchor requires matched reference")
    ...
    for epoch in range(1, fixed["max_epoch"] + 1):
        ...
        frames, videos, observed = predict(model, val_data, args.device)
        ...
        if epoch == 1 or epoch % 10 == 0:
            print(f"{args.corpus}/{args.arm}/l={args.lambda_failure:g} "
    ...
    payload = {
        ...
        "lambda_failure": args.lambda_failure, "gce_q": args.gce_q,
        "lr": args.lr,
        "selection_rule": (
