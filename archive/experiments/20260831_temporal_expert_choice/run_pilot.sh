...
PILOT="${PILOT_DIR:-$ROOT/runs/20260831_temporal_expert_choice/pilot_seed234}"
mkdir -p "$PILOT"
...
for corpus in hatemm hateclipseg; do
  ...
  for arm in token_choice expert_choice; do
    run_dir="$PILOT/$corpus/$arm"
    mkdir -p "$run_dir"
    echo $$ > "$run_dir/run.pid"
    "$PY" "$ROOT/experiments/20260831_temporal_expert_choice/train.py" \
      --corpus "$corpus" --arm "$arm" --output-dir "$run_dir" \
      "${corpus_args[@]}" > "$run_dir/run.log" 2>&1
    "$PY" "$ROOT/experiments/20260831_temporal_expert_choice/evaluate.py" \
      --corpus "$corpus" --run-dir "$run_dir" >> "$run_dir/run.log" 2>&1
  done
done

"$PY" "$ROOT/experiments/20260831_temporal_expert_choice/summarize.py" \
