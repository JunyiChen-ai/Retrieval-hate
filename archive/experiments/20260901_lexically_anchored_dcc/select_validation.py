#!/usr/bin/env python
"""Select one core trial using validation only."""
...
def main():
    ...
    ap.add_argument("--pooled-tolerance", type=float, default=0.005)
    args = ap.parse_args()
    ...
    core = [row for row in rows if row["arm"] == "aligned"]
    if len(anchors) != 2 or len(core) != 12:
        raise RuntimeError(
            f"incomplete validation search: anchors={len(anchors)} core={len(core)}")
    for row in core:
        anchor = anchors[row["lr"]]
        val, base = row["validation"], anchor["validation"]
        row["delta_vs_anchor"] = {
    ...
    if feasible:
        selected = max(feasible, key=lambda row: (
            ...
            -row["contrast_weight"], -row["support_quantile"]))
    ...
    payload = {
        ...
        "selected": selected,
        "matched_anchor": anchors[selected["lr"]],
        "trials": rows,
