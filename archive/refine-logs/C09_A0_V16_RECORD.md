# C09 Stage-0 (A0) — preregistration **v16** (round-15 repairs)
...
> ## STATUS: `V16_REPAIRED_READY_TO_FREEZE_PENDING_A_GO_REVIEW` — awaiting fresh independent review.
>
...
> what blocks the freeze is not science but the document's front matter"*) → v9
> `C09_A0_V9_RECORD.md` → R9 `C09_A0_V9_PREREG_REVIEW.md` (**`REVISE 0C/0H/3I` — the
> first round with no Critical and no High**) → v10 `C09_A0_V10_RECORD.md` → R10
> `C09_A0_V10_PREREG_REVIEW.md` (`REVISE 0C/0H/2I`) → v11 `C09_A0_V11_RECORD.md` → R11
> `C09_A0_V11_PREREG_REVIEW.md` (`REVISE 0C/0H/2I`) → v12 `C09_A0_V12_RECORD.md` → R12
> `C09_A0_V12_PREREG_REVIEW.md` (`REVISE 0C/1H/2I`) → v13 `C09_A0_V13_RECORD.md` → R13
> `C09_A0_V13_PREREG_REVIEW.md` (`REVISE 0C/0H/5I` — *"The science is finished and I
> could not move it — for the fifth round running… Round 12's High is discharged"*) →
> v14 `C09_A0_V14_RECORD.md` → R14 `C09_A0_V14_PREREG_REVIEW.md` (`REVISE 0C/0H/3I` —
> *"The science is finished and I could not move it — for the sixth round running…
> for the third round running the blockers are entirely outside §§1–11's governing
> text"*) → v15 `C09_A0_V15_RECORD.md` → R15 `C09_A0_V15_PREREG_REVIEW.md`
> (`REVISE 0C/0H/5I` — *"The science is finished and I could not move it — for the
> seventh round running… §8 and §9 are byte-identical to v8 by hash"*) → **this file**.
>
> **`v1–v15 are superseded in full and must not be implemented.`** Each carries its own
> ledger; **the v13 ledger is §12**. Round 12 confirmed the science intact once more —
> *"The instrument, the legality spine, the executability, the budget and the inference
> are all intact, and I could not move any of them"* — and then found that **v12's own
> headline repair corrected the wrong term**: round 11 proved `n_unstable ≈ 7–9` and
> `|P_0| ≈ 76 / 55` could not coexist, and v12 kept `76 / 55`; the arithmetic says the
> opposite. **v13 re-derives the whole F88 transfer from the primary tables, shows the
> derivation in §4.2 so a reviewer can check it in two minutes, and restates every
> pre-declaration that stood on the old figures.**
>
...
## 0. What v16 changes, and how this design reached its present form

Round 15 returned **`0C / 0H / 5I`** with the bottom line *"The science is finished and I
could not move it — for the seventh round running."* It re-verified round 14's byte-identity
claim by hashing `## 8` through the line before `## 10` in v8 and in v15 — **`md5
a17b56954ee6955013327f82a03904f7` on both** — and re-derived the F88 transfer, all twelve
`k`-grid figures, the caps, the Hanley–McNeil algebra and the three sha256 pins itself. It
additionally reconstructed both confusion matrices from the banked floors and the ERRPAT
FP/FN splits and confirmed that **the accuracy leg, not the macro-F1 leg, is the binding
one at `τ_hi` on both datasets** — so §5.2's accuracy-only knife edge is correct rather
than incomplete. All three of round 14's findings were discharged at source.

Its five findings are one-line text edits; four sit in §0/§12 and one in §5.5:

1. **§12 asserted a measured quantity that exists in no artifact (R15 I-1).**
   `aggnet_pregate.py:534` computes `A_agree_threshold_shift` **once**, from the
   `NET_SEED = 0` primary arm; the stability arms `C3_net_s1/s2` never enter the `DEG`
   block and their per-item vectors are not banked, so there is no per-seed or 3-seed
   DEG-A and `0.9575` is not recomputable. v15 transcribed it from round 14's
   below-the-line note — **the exact failure §0's own round-14 note names as
   forbidden**. **Struck.**
2. **§0 and §12 claimed a repair whose text does not exist here (R15 I-2).** v15 said
   v14's scope paragraph had been *"corrected to eleven edit sites, in both copies"*;
   v14's paragraph was **deleted and replaced**, not corrected, and nothing in v15 states
   eleven. **Restated to what actually happened.**
3. **§5.5 presented a composite paraphrase as a verbatim quotation (R15 I-3).** The
   reopen's sentence at `:1050-1051` reads *"Part of the reported ZH floor rests on a
   marker of how the corpus was collected rather than on video content"*; v3–v15 spliced
   `0.8537` in from `:1047-1048`, dropped *"a marker of"* and read *"harvested"*.
   **Quoted verbatim at its locator.** This is the one finding inside §§1–11, so v16's
   scope statement is restated rather than carried.
4. **"§8 (all ten gates)" contradicts §8.1's own count (R15 I-4).** §8 names twelve
   `GATE-*` objects — nine HALT gates and three reporting instruments; "ten" is §8.1's
   count of *publication conditions*, one of which (`SHUFFLE-POP`'s band) is not in §8.
   **Corrected in both copies.** The substantive byte-identity claim is unaffected and
   was independently re-verified by hash.
5. **The two copies of the scope paragraph were not verbatim identical (R15 I-5)** —
   they differed by two words beyond the intentional cross-reference. **Made
   character-identical apart from that cross-reference.**

**Scope of v16, stated exactly** — one convention, copied into §12 character-identical
apart from the `§0`↔`§12` cross-reference. **Six edit sites:** the title; the STATUS
block; §0; one sentence of §5.5; §12; and the closing trailer. **§§1–4, §§6–11 are
byte-identical to v15**, and **§§8 and 9 are byte-identical to v8** (round 15, by hash).
**No decision rule, threshold, feature set, null, gate, operating-point definition or
arithmetic rule has changed since v8.**
...
- **`K-DEG`'s statistic (R4 C-1).** F96 and F98 measure **prediction-vector agreement
  ...
  `k/n ≈ 79/744` (§4.2's corrected `|P_0|`) the two differ by
  `pred_agree = 1 − 2k(1−ov)/n`, so v4's `ov ≥ 0.95`
  demanded `pred_agree ≥ 0.9894` (R14 I-1: v14 re-based the numerator on `79` and left
  the derived figure at the retired-`76` value `0.9898`) and would have let an `S`
  three-quarters identical to a bare threshold shift through the campaign's only
  degeneracy screen. **Since v5,
...
- **The symmetric correction, at its true weight.** F47's train-supervised leg is
  *"memorization-degenerate target, CLIP LOO 0.998"*; **F114**
  (`autoresearch/goal_mllm_plus3/state/findings.jsonl:115` — the CLIP-LOO erratum;
  locator given because the bare number also names an unrelated C04-lineage finding at
  `TARGET_FINDINGS.md:79`, R12 I-2) rules that premise a **CLIP** number while the
  deployed Qwen heads sit at `0.9406 / 0.8915 / 0.8154`, so
...
> **The transfer, derived rather than asserted (R12 H-1).** Every version up to v12
> quoted `|P_0|` and `n_unstable` as bare "transferred F88 rates" with no derivation, and
> took them from *different* sentences using *different* denominators — which is why
> rounds 11 and 12 found them mutually incompatible. They are two ratios of the **same**
> table and must be transferred jointly, denominated on the only quantity this arena
> measures: the per-seed error count.
>
> Let `E_s` be the seed-`s` inversion set, `a` = #(wrong in exactly 2 seeds), `b` =
> #(exactly 1). Then `Σ_s |E_s| = 3|P_0| + 2a + b` and `n_unstable = a + b`.
>
> | | HateMM (per-seed counts from `ERRPAT_HateMM §1`'s cell table; consensus from `§1.1`, **final** row) | MHC-ZH (**`ERRPAT_MHC-ZH §2`**, *"ERROR INVENTORY (Tier 2, final-epoch protocol)"*) |
> |---|---|---|
> | per-seed errors | `28 / 26 / 26` ⇒ `Σ = 80`, mean `26.6667` | `23 / 24 / 22` ⇒ `Σ = 69`, mean `23` |
> | `3/3` = `\|P_0\|` | `25` | `22` |
> | `2/3` = `a` | `2` | `0` |
> | `1/3` = `b` | `1` | `3` |
> | identity check | `3(25) + 2(2) + 1 = 80` ✓ | `3(22) + 0 + 3 = 69` ✓ |
> | `\|P_0\|` / per-seed mean | `25 / 26.6667 = 0.9375` | `22 / 23 = 0.9565` |
> | `n_unstable` / per-seed mean | `3 / 26.6667 = 0.1125` | `3 / 23 = 0.1304` |
>
> This arena's **measured** per-seed error counts are `83 / 85 / 85` (HateMM,
> `744 × (1 − 0.8884 / 0.8858 / 0.8858)`) and `62 / 64 / 61` (MHC-ZH,
> `579 × (1 − 0.8929 / 0.8895 / 0.8946)`), i.e. means `253/3 = 84.3333` and
> `187/3 = 62.3333`. Scaling both ratios:
>
> > **`|P_0| ≈ 79` (HateMM) / `≈ 60` (MHC-ZH)**; **`n_unstable ≈ 9` / `≈ 8`**;
> > **`|P_{τ_hi}| ≈ |P_0| / 2 ≈ 40` / `≈ 30`.**
>
> Both reproduce the measured total exactly: `3(79.06) + 15.81 = 253.0` and
> `3(59.62) + 8.13 = 187.0`.
>
> **What was wrong before, named.** v6–v12 carried `|P_0| ≈ 76 / 55`. The ZH figure was
> `0.88 × 62.3333`, applying F88's **union**-denominated rate (*"22 of the 25-item
> union"*) to a **per-seed** base; the per-seed rate is `22/23 = 0.9565`. The HateMM
> figure took the low end of the `"≈89-93 %"` band against a fuzzy `"~26-28"`; the
> protocol-matched rate is `25/26.6667 = 0.9375`. v11 correctly proved that
> `n_unstable ≈ 7–9` and `|P_0| ≈ 76 / 55` could not coexist; v12 resolved it by keeping
> the wrong term. **`7–9` was approximately right; `76 / 55` was not.**
>
> **Protocol note.** The HateMM row is the **final-epoch** row of §1.1's two-protocol
> table, because final-epoch is what this arena's mint produces (30 fixed epochs, no
> checkpoint selection — §5.6); MHC-ZH's §2 inventory is itself headed *"final-epoch
> protocol"*, so both legs match. The val-sel alternative, in this block's own
> `(|P_0|, a, b)` order, is `(24, 2, 3)` with per-seed errors `26 / 27 / 26`, `Σ = 79`,
> mean `26.3333`, giving `24 / 26.3333 = 0.9114` and **`|P_0| ≈ 77`** (R13 I-1: v13 wrote
> `78`). Nothing in the decision rule reads either.
...
with the **first set the positives and the second the negatives — the negative class is
what this document calls `CONFIG-MATCHED-CORRECT`, and that name is defined here, at its
point of definition (R10 I-2; earlier versions used the name in §7 while §5.3 marked the
phrasing containing it superseded, and the object itself changed twice under it, being
confidence-restricted by R5's H-3 and made fold-dependent by R6's H-1)** — so the class
...
medians of overlapping 4/5 subsamples of the same `≈ 79` values (§4.2; R13 I-2 —
v6–v13 wrote `≈76`, the retired figure) — but the composition of a
...
>
> **Pre-declared consequence, multiplied out on the corrected transfer (§4.2; R12 H-1
> — v6–v12 stood this paragraph on `|P_{τ_hi}| ≈ 38 / 28`, which came from a mis-derived
> `|P_0|`).** On the corrected transfer `|P_{τ_hi}| ≈ 40 (HateMM) / 30 (MHC-ZH)`, and the
> co-primary is **not** arithmetically dead — **it sits on a knife edge on both legs, on
> both datasets:**
>
> - **Reach.** `40 / 744 = 0.0538` and `30 / 579 = 0.0518`, both just above the `+0.050`
>   bar. **On HateMM a shortfall of three items closes the branch on reach**
>   (`37 / 744 = 0.0497 < 0.050`; two still clears, at `0.0511`). **On MHC-ZH the reach
>   bar is `|P| ≥ 28.95`, i.e. `≥ 29`, so a shortfall of two closes it on reach**
>   (`28 / 579 = 0.0484`) — **and a shortfall of one closes it on POWER, not reach**
>   (`29 / 579 = 0.05009` still clears, while `p_w ≤ 29 < 30` fires the tag below).
>   *(R13 I-4: v13 attributed the one-item MHC-ZH case to reach.)*
> - **Power.** `p_w ≤ |P_τ|`, and the rule fires at `p_w < 30`. On HateMM that needs
>   `30` of `≈ 40` positives — **≥ 75 %** — in two-class strata; on **MHC-ZH it needs all
>   `≈ 30`**, i.e. **not one positive in a single-class stratum**, against this section's
>   own statement that most strata may be single-class and that it is worst at `τ_hi`.
>
> **Declared now, in advance:** on the corrected transfer the `τ_hi` co-primary is
> **live but marginal on both datasets and on both legs**, most likely to close on the
> power leg on MHC-ZH, and a tagged KILL there is a statement about power, not about
> geometry. v12's stronger claim — that the branch *"cannot produce a CONTINUE at all"* —
> **is withdrawn**: it rested on `28 < 30`, and the corrected figure is `30`. The `τ_0` branch is where this A0's identifiability question is
...
1. **unstable errors** — on §4.2's derived transfer, `≈ 9` (HateMM) / `≈ 8` (MHC-ZH)
   items (R12 H-1; v12's `13–25 / 11–22` stood on a mis-derived `|P_0|`) — at both `τ`;
   and
...
score over all `n`, where **`R(x) ≡ int(numpy.floor(x + 0.5))`** — half-up, pinned here
(R5 I-3) because Python's and NumPy's default `round` is banker's rounding. On §4.2's
corrected transfer the live hazard is HateMM's `1.5 × 79 = 118.5`, which banker's
rounding sends to `118` and `R(x)` sends to `119` (R13 I-2 — v5–v13's example used
`1.5 × 55 = 82.5`, a ZH figure now superseded by the integer `1.5 × 60 = 90`, which
carries no hazard). `k` enters the caps below, `K-NET`'s bar and `K-DEG`'s agreement
mapping.
...
`k > 2·mean_s|wrong_s| − 37.2 = 2 × (253/3) − 37.2 = 131.4667` (HateMM) — i.e.
`k ≥ 132` — or for `k > 2·mean_s|wrong_s| − 29.0 = 2 × (187/3) − 29.0 = 95.6667`
(MHC-ZH) — i.e. `k ≥ 96` — and for
...
`[28.95, 29.0)`. Both figures are reported. On the **corrected** transfer
(§4.2: `|P_0| ≈ 79 / 60`, `|P_{τ_hi}| ≈ 40 / 30`; R12 H-1) that means, at `τ_0`:
`k = R(2|P_0|) = 158 / 120` is **dead on both datasets**; `k = R(1.5|P_0|) = 119 / 90`
survives only under both of, re-derived in place — **HateMM**
`|S ∩ wrong| ≥ (119 + 37.2)/2 = 78.1` ⇒ recall `≥ 78.1/84.3333 = 0.926` and precision
`≥ 78.1/119 = 0.656`; **MHC-ZH** `|S ∩ wrong| ≥ (90 + 29.0)/2 = 59.5` ⇒ recall
`≥ 59.5/62.3333 = 0.955` and precision `≥ 59.5/90 = 0.661` — and `k = |P_0| = 79 / 60`
needs recall `≥ 0.689 / 0.714` at precision `≥ 0.735 / 0.742`. **At `τ_hi` all three `k`
clear the net caps on both datasets** (`40 / 60 / 80` and `30 / 45 / 60`, all inside
`[37.2, 131.4667]` and `[29.0, 95.6667]`), at required precisions of
`0.965 / 0.810 / 0.733` (HateMM) and `0.983 / 0.822 / 0.742` (MHC-ZH) — so the co-primary
is **not** closed on the net cap, and v12's *"`k = |P_{τ_hi}|` is strictly impossible on
MHC-ZH whenever `|P_{τ_hi}| < 29`"* is withdrawn as a live expectation: the corrected
`30` clears.
The run computes the caps from the realised `|wrong_s|` and `|P_τ|`, and — since the
caps are **per dataset** — marks all **twelve** `(dataset, τ, k)` cells, six per dataset
(R10 corrected v10's "six"), **`LIVE_ON_NET`** or **`ARITHMETICALLY_DEAD_ON_NET`** — the caps
...
choice is adjudicated rather than inherited. *(v4 said "~7.5 points", inherited from the
round-3 finding without re-derivation; at `k = 100` the true gap is **7.45** points —
`(1 + 37.2/100)/2 − (1 + 22.3/100)/2 = 0.0745`. R4 I-1; figure corrected R10.)*
...
- **MHC-ZH `<em class="keyword">` markup.** The reopen records markup-bearing rows
  ...
  `0.1161` (39/336) without, against a `0.3109` base. The reopen's own conclusion,
  quoted verbatim at its locator (`GATE0_REOPEN_2026-07-31.md:1050-1051`; R15 I-3 — v3–v15
  carried a composite paraphrase that spliced `0.8537` in from `:1047-1048`, dropped the
  source's *"a marker of"* hedge and read *"harvested"* for *"collected"*):
  *"Part of the reported ZH floor rests on a marker of how the corpus was collected
  rather than on video content"*.
...
`mlp_mean3`, and the λ-oracle ceiling). **They are not one lineage** — which is the point
v4's "all nine descending from F105/VSW" got wrong — and the precise statement is this
(R12 I-1; v10–v12's *"the count is the source's four"* equivocated between two different
fours): `HEADSPACE_TRANSFER_PREGATE.md:623` heads the ladder *"### 4.10 The transfer
ladder (§2.10) — four operator families, one arena swap"*, and its rung column numbers
them **rung 1 = VSW** (including `ORACLE_lambda_pow` *"(hindsight ceiling)"* and
`CTRL_cos_pow` *"(DEG-D, no verifier)"*, VSW's own oracle and control arms), **rung 2 =
F95**, **rung 3 = FIXK**, **rung 4 = F89**, with `THRESH_best` deliberately **un-runged**
(`:643`, *"(global recalibration)"*). **The nine raw positives occupy rungs 1–3 plus the
un-runged arm and contain no rung-4 member at all** — F89's RAW column is `—`. So the
ladder's four and the nine's four are different sets of the same cardinality, and `:623`
does not license the count on its own.

**And rung 4 is an independent headwind this design had not registered (R12 I-1; the
attribution corrected R13 I-3).** F89's four arms are closed-form **geometric transforms
of the head keys** — the family nearest to `FULL`'s structural block. **On HateMM**,
`:644-647` of §4.10 — headed *"3-seed means, head arena. RAW column re-read from
`vsw_main_hatemm_OUT.json`"*, i.e. a **HateMM-only** ladder whose three-entry column is
**per seed**, not per dataset — gives head-space 3-seed means `F89_T1 +0.0000`,
`F89_T2a −0.0004`, `F89_T2b +0.0027`, `F89_T4 +0.0054`: **null to sub-noise**.
*(v13 called these "per-dataset first entries" and read a cross-dataset sign flip off
them; no cross-dataset comparison exists at those lines.)* **The cross-dataset sign flip
is real and lives elsewhere, and it is stronger than what v13 claimed:** the ZH ladder
at `:784` gives `F89_T4 −0.0063` against HateMM's `+0.0054`, and `:791-796` records that
the MECHFIX head-space **test** reads are `T1 +0.0000, T2a +0.0000, T2b +0.0000,
T4 +0.0067` against this arena's `−0.0006 / +0.0000 / −0.0040 / −0.0063`, concluding
that *"T2b/T4 are sub-0.007 and sign-unstable across the two head-space arenas"* — the
same disagreement `:902` books as *"same magnitude, opposite sign."* All of it is
adverse to C09 and is carried here rather than left out. *(The "at least six" v6–v9
carried from R5 I-5b, and its "not re-derivable" clause, remain withdrawn — R9 I-2.)* The record concludes at `:863-864` — the clause begins on `:863`
(R6 I-7e) — *"9 of 9 raw positives fail to transfer; the median shrink is >7×; three
invert sign."*
...
entry, not in `HEADSPACE_TRANSFER_PREGATE.md`, whose `:871-873` phrases it at **3** dp — `−0.064 / −0.129` against
`+0.157 / +0.230` — and without the fold count, which is at `:847`/`:901`; v9 said 2 dp,
R9 I-3): *"any FITTED relation score over head keys memorises the bank
...
so at HateMM's `n = 744` with `k ≈ 79` (§4.2's corrected `|P_0|`; R13 I-2 — v4–v13 used
the retired `76`), `pred_agree = 1 − 0.2124(1 − ov)`: F98's `0.95` line corresponds to
`ov ≈ 0.765`, while v4's `ov ≥ 0.95` demanded `pred_agree ≥ 0.9894`. *(The retired `76`
gave `0.204`, `ov ≈ 0.755` and `0.9898` — the same conclusion at the same margin; the
gate reads realised `pred_agree`, not either figure.)* An `S` three-quarters identical to a bare threshold shift — the
...
- **`UNSTABLE-POP`.** `D-FELDMAN` re-run with the target redefined as unstable errors,
  ...
  **Pre-declared expectation, on §4.2's derived transfer (R3 I-9; arithmetic corrected
  twice — R11 I-1 identified the incompatibility, R12 H-1 identified which term was
  wrong).** §4.2 derives `n_unstable ≈ 9` (HateMM) and `≈ 8` (MHC-ZH) by transferring
  `(a + b) / mean_s|E_s|` — `3/26.6667` and `3/23` in the primary tables — onto this
  arena's measured per-seed means `84.3333` and `62.3333`. Both are **far under the `20`
  trigger**.

  **So `CONTROL_UNDERPOWERED` is the expected outcome on both datasets** — which is what
  v11 declared, on a figure that happened to be right for the wrong reason, and which v12
  wrongly retracted after resolving round 11's contradiction in favour of the mis-derived
  `|P_0|`. **That is declared now, is not an artefact, and means the registry claim's own
  stability premise is expected NOT to be tested by this A0.** The determination is still
  made by the **realised** tag, not by this expectation: the rule is unchanged,
  data-independent, and applied identically to both datasets, and §10 reports which leg,
  if either, fired.
...
**Break exposure — the registry claim's second conjunct — is measured by §5.3, and is
named here rather than asserted by cross-reference (R10 I-2).** v10's summary attributed
it to a "break-exposure stratification" supplied by `CONFIG-MATCHED-CORRECT`, a name §5.3
had marked superseded and §5.2 had stopped using; §5.2 now carries the alias at its point
of definition, but the instrumentation is elsewhere: **every selected item is costed at
every seed** by `net_s = 2·|S ∩ wrong_s| − k` (§5.3); **`GATE-SELFTEST` ties that
identically to `n · Δacc_s`** (§8.1); and **the composition of `S` by class — stable
inversion / unstable error / always correct — together with the fraction of `S` lying
outside `A^{(f)}`, is reported at every operating point** (§5.3). Those three are what
make *"while explicitly constraining break exposure"* measured rather than asserted.
...
- **This A0 does not separate H-DATA-DEFECT from H-TOPOLOGY** (§5.5); it only prices
  the overlap. **And it is expected not to test the registry claim's stability premise**:
  §4.2's derived transfer gives `n_unstable ≈ 9 / 8`, far under the `20` trigger, so
  `CONTROL_UNDERPOWERED` is expected on both datasets (§6.3). **The determination is made
  by the realised tag, not by this expectation**, and the verdict record must report
  which leg, if either, fired. *(v11 declared this correctly; v12 retracted it on a
  mis-derived `|P_0|`; R12 H-1 restored it with the derivation shown.)*
...
ban-scope adjudication against **F75, F66, F98 and F99**. Two of those four carry a
named burden rather than a general one, and both are spelled out so Stage-1 entry cannot
treat them as noted-and-passed. **F99** (added R6 H-2) is the only banked finding written
on the successor's own object class, and **its novelty leg (GAP-7/D7) and its
`+0.0279 / +0.0470` permutation cap must both be defeated**. **F66**'s own `ban_scope`
closes *"Re-open requires an operator that converts symmetric structure — the beta
decomposition proves none of the tried classes can"* — the successor §11 names **is** a
symmetric operator, so **that sentence is its precondition, not its context** (R10,
below-the-line item, adopted). **If no such operator can be named at
Stage-1 entry, the CONTINUE is void and C09 closes with no further spend.**

---

## 12. Repair ledger — the 5 round-15 findings

Round 15's verdict was **`REVISE (0C / 0H / 5I)`** with the bottom line *"The science is
finished and I could not move it — for the seventh round running… All five findings are
one-line text edits and none touches a rule, threshold, feature set, null, gate,
operating point, verdict or the scope of any conclusion."* Round 15 **re-verified round
14's byte-identity claim by hash** (`## 8` through the line before `## 10`:
`md5 a17b56954ee6955013327f82a03904f7` in both v8 and v15), re-derived the F88 transfer
and all twelve `k`-grid figures independently, and additionally **reconstructed both
confusion matrices from the banked floors and the ERRPAT FP/FN splits** — recovering
`mF1 0.8831` against banked `0.8838` (HateMM) and `0.8747` exactly (MHC-ZH) — to confirm
that **the accuracy leg binds at `τ_hi` on both datasets**, so §5.2's accuracy-only knife
edge is correct rather than incomplete.

**Scope of v16, stated exactly** — one convention, copied into §0 character-identical
apart from the `§0`↔`§12` cross-reference. **Six edit sites:** the title; the STATUS
block; §0; one sentence of §5.5; §12; and the closing trailer. **§§1–4, §§6–11 are
byte-identical to v15**, and **§§8 and 9 are byte-identical to v8** (round 15, by hash).
**No decision rule, threshold, feature set, null, gate, operating-point definition or
...
| # | round-15 finding | repair | where |
|---|---|---|---|
| **I-1** | §12 asserted `0.9575` as *"the 3-seed mean"* of F98's DEG-A. `aggnet_pregate.py:534` computes `A_agree_threshold_shift` **once**, from the `NET_SEED = 0` primary arm; the stability arms `C3_net_s1/s2` never enter the `DEG` block and their per-item vectors are not banked, so no per-seed or 3-seed DEG-A exists or is recomputable, and the string appears nowhere in the repository outside round 14's review. v15 transcribed it from that review — **the exact failure §0's own round-14 note names as forbidden** | the parenthetical is **struck**; the `0.9570` figure is re-attributed to its single emission, the non-existence of a per-seed family is stated, and the `0.95` line is left where it is correctly sourced (`RESTRANS_PREGATE_RECORD.md:409`) | §12 |
| **I-2** | §0 and §12 claimed v14's scope paragraph had been *"corrected to eleven edit sites, in both copies"*; that paragraph was **deleted and replaced**, not corrected, and nothing in v15 states eleven — so a reader auditing the ledger would search for text that does not exist | **restated to what happened**: v14's paragraph was superseded, v15 stated its own count of six, and eleven was round 14's number for v14's diff | §0, §12 |
| **I-3** | §5.5 presented a composite paraphrase as a verbatim quotation from the reopen: `0.8537` spliced in from `:1047-1048`, the source's *"a marker of"* hedge dropped, *"collected"* read as *"harvested"* — carried unlocated since v3 | **quoted verbatim at its locator** (`GATE0_REOPEN_2026-07-31.md:1050-1051`) with the three deviations named. Every surrounding number was already exact and re-measured; the distortion ran **against** C09 | §5.5 |
| **I-4** | *"§8 (all ten gates)"* contradicts §8.1's own count: §8 names **twelve** `GATE-*` objects — nine HALT gates and three reporting instruments — and "ten" is §8.1's count of *publication conditions*, one of which is `SHUFFLE-POP`'s band, not a §8 gate | corrected in **both** copies to *"its nine HALT gates and its three reporting instruments"*. The substantive byte-identity claim is unaffected and round 15 re-verified it by hash | §0, §12 |
| **I-5** | the two copies of the scope paragraph differed by two words beyond the intentional cross-reference, against the paragraph's own *"copied verbatim"* claim | made **character-identical apart from the `§0`↔`§12` cross-reference**, and the claim reworded to say exactly that | §0, §12 |

**What round 15 checked and left below its own line, left as it left it:** Band B's
quantifier not formally excluding Band C in one corner (§9 is two-valued, complete and
unambiguous, so no verdict is at risk); §5.6's *"30-epoch Adam"* against `run_rac.py:684`'s
`AdamW`; §2's *"~25–60 s"* against a measured `33.2` and the ≈115-vs-116 budget rounding;
`ITEM-STRATUM` occupancy not being emitted the way the row strata are (round 15 confirms
the direction is conservative — `K-FELDMAN` requires **both** families to reject, so the
conditional null can only remove CONTINUEs); the `(τ, k)`-cell wording where `k` is a
per-dataset function of `|P_τ|`; §3's rendering of `LITSWEEP3:91`, inherited verbatim from
`label_use_legality.text_2` and running **against** C09; `directions_tried.json::dead[Fxx]`
not being a literally resolvable path (repository convention, used by the reopen too);
`ksweep_OUT.json`'s `dev` sub-key being `null` on the four `MHC_EN_ARM-V/*/valsel` curves
(the parenthetical it supports concerns the two in-scope datasets, where it is populated);
and minor typographic drift inside italic quotes.

---

*v16. No hash frozen, no config written, no code implemented, no namespace created, no
