# C09 Stage-0 (A0) — preregistration **v8** (round-7 repairs)
...
> ## STATUS: `V8_REPAIRED_NOT_FROZEN_NOT_SUBMITTED` — awaiting fresh independent review.
>
...
Round 7 returned **no Critical** and one High: v7's two round-6 repairs contradict each
other at the co-primary.

**R6's H-1 repair made the `D-FELDMAN` analysis set fold-dependent (`A^{(f)}`); R6's I-3
repair then pinned the permutation pool to that same fold-dependent set — and asserted a
single permutation `π` per draw.** At `τ_0` the two agree, because `A^{(f)}` is the same
set for every `f`. At `τ_hi` they cannot both hold: the five `A^{(f)}` differ by a
handful of items, and one permutation of items is not a permutation of five different
pools. The literal reading forces **five independent `π` per draw**, which averages the
permuted noise over five independent re-assignments, narrows the null and makes `p` too
small — anti-conservative, in the CONTINUE direction, by the same mechanism round 1
rated Critical for the bootstrap. **v8 names the union pool `P^{(τ)} ≡ ∪_f A^{(f)}` and
draws one `π` over it**, with only `A^{(f)}`'s rows used inside the fold-`f` refit.

Four precision items follow: `PERM-STRUCT-COND`'s exactness claim is restated for the
null it is actually exact for; §5.3's out-of-support declaration is brought up to date
with the per-fold analysis set and extended to the `c`-restricted population at `τ_hi`;
`ksweep_OUT.json`'s `n_test` is given per dataset; and `THRESH-SYM`'s per-seed scale is
disambiguated so the two-scale maximum cannot silently collapse to one.

**Rounds 3–6's repairs, retained unchanged in v8:**
...
## 12. Repair ledger — the 5 round-7 findings

| # | round-7 finding | repair | where |
|---|---|---|---|
| **H-1** | R6's H-1 repair (per-fold analysis set `A^{(f)}`) and R6's I-3 repair (pool = `A^{(f)}`) contradict each other at `τ_hi`, where the five `A^{(f)}` differ; the literal reading forces five independent `π` per draw, narrowing the null in the CONTINUE direction | **the union pool `P^{(τ)} ≡ ∪_f A^{(f)}` is named and pinned**, with **one** `π` per `(dataset, τ, draw)` over it and only `A^{(f)}`'s rows used inside the fold-`f` refit; the same wording applied to `PERM-STRUCT-COND` and to `SHUFFLE-POP`; the domain stays the estimator's own, never all `n` | §5.2, §6.3 |
| **I-1** | `PERM-STRUCT-COND`'s exactness claim names `struct ⊥ target \| ITEM-STRATUM`, but `ΔAUC` reads `BASE`, so validity needs the joint form | claim restated as **exact for `struct ⊥ (target, BASE) \| ITEM-STRATUM`**, with an explicit statement of what follows if only the weaker independence holds — the `[0.95, 1.0]` bucket does not isolate `pred_purity = 1.0`, so a `p_cond` rejection may reflect within-stratum non-linear re-encoding of `BASE`; scoped throughout and carried into §10 | §5.2, §10 |
| **I-2** | §5.3's out-of-support declaration is stale against the per-fold fit set and omits the `c`-restricted population at `τ_hi` | fit set restated as **`A^{(f)}`**; the out-of-support population extended to **every item with `c_j < τ_hi^{(f)}`** at the co-primary — roughly half of `P_0` plus the low-`c` tail of the correct class — with the reason each alternative was rejected; **the fraction of `S` lying outside `A^{(f)}` is now reported** at every operating point | §5.3 |
| **I-3** | `n_test = 215` quoted for all three CAL-2(2) arms | corrected **per dataset**: `215` (HateMM), `149` (MHC-ZH), `161` (MHC-EN ARM-V), with the note that the ZH figure is load-bearing elsewhere (F99's `+0.0470 = 7/149`) | §2 |
| **I-4** | `THRESH-SYM`'s per-seed scale is ambiguous; one reading collapses the two-scale maximum | disambiguated: the per-seed twin is built **within each seed separately** and **`pred_agree` is averaged over seeds**, not the scores — so the two-scale maximum cannot silently become one scale | §6.2 |

---

*v8. No hash frozen, no config written, no code implemented, no namespace created, no
