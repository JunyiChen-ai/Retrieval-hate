# C06 `$0` CPU falsifier — preregistration **DRAFT v10** (2026-08-04)

**SUPERSESSION.** Supersedes `C06_FALSIFIER_PREREG_DRAFT_V9.md` → V8 → V7 → V6 → V5 → V4 → V3 → V2
→ v1. All nine remain on disk **unmodified** as the record of what each round reviewed. Reviews of
record: `C06_FALSIFIER_PREREG_REVIEW{,_R2,…,_R9}.md` — REVISE 3C/6H/10I+4M, 3C/3H/7I+3M,
...
**Disposition: 8 round-9 findings, dispositioned at LIMB level — every limb quoted VERBATIM from
round 9's review with its location there, so the enumeration is checkable against a source rather
than trusted (§14, §14.1). No blanket adoption claim is made anywhere in this document.**

**Round 9 was the first round with zero Criticals**, and it confirmed the science layer closed for
the third time independently: all thirteen arms rebuilt from §3.4's prose at the first attempt with
nothing silently supplied, every constructible wrong reading caught, all 37 digests, all 26 `ρ` at
6 dp under the frozen float64 reduction, the Holm table cell for cell, and *"no gate can fire on a
warranted CLOSE, all twenty, re-derived."* It also verified round-8 C-1 the only way that settles
it: v9's embedded transcript is **byte-identical** to round 9's own run of the §14.2 script against
the finished on-disk v9.

**What round 9 found, and it is the reason this version exists.** The record failure has changed
shape — *"it is no longer **absence** … it is **narrowing**: two limbs land a weaker repair than the
one prescribed, and the limb table's paraphrase is where the weakening happens."* Two demonstrations:

* **§7.9's bolded headline** still said *"Five CPU head mints were trained in v7"*, a **spend**
  statement applying this document's own reported-versus-spend rule **backwards**, contradicting the
  parenthetical directly beneath it, §7.9's own sum, and §7.8 — for the third consecutive version in
  the section `rule_1` and the F118 lesson bind by name.
* **§14.1's self-exclusion asserted rather than checked.** Round 9 demonstrated by construction that
  a draft whose §14.1 is byte-identical to its predecessor's still printed `CHANGED §14.1` and still
  rated all three §14.1-citing rows and five §14.1-citing limbs `OK`. **The instrument built to end
  unverified repair claims contained a branch that certified a repair claim without checking it.**

**What v10 changes.**

* **H-1** — §7.9's headline is rewritten in round 9's own words; the now-redundant reconciliation
  sentence is dropped; and v7's `≈ 5 / ≈ 25` is re-derived at `≈ 4 / ≈ 21` with four mints, the
  discharge mint's share moving to the v6 column where it was spent.
* **H-2** — the self-exclusion now **checks**: §14.1 is printed `CHANGED` / `UNCHANGED` / `ADDED`
  from an actual comparison and only added to `touched` when it genuinely differs, so a row citing
  §14.1 fails exactly when §14.1 did not change. Self-exclusion covers the **size only**, never the
  changed/unchanged fact.
* **I-1** — the eighth uncounted loop (the arena's materialisation of the 60 per-cell head-key
  matrices) is priced as Phase 1f from a measured unit.
* **I-2** — `U_tie` is measured (`2.0e-05 s`/item) and the `GATE-ZEROOP` row's cell count corrected
  to §6.5's own aggregation (`12`, not the fold-level `30`), which **reduces** that row.
* **M-4** — every limb in §14's table is now a **verbatim quotation** of the prescription with its
  location in round 9's review, because round 9 showed the paraphrase is where narrowing enters.
...
printed or recorded at any point in v1–v9.** Rounds 4 and 5 audited this by grepping every decimal
...
published C01 dev-arena accuracy, a majority/band constant or a unit-time string. **v6–v9 add exactly one
...
`C01_A0_OUT.json` cells of round-9 M-3, and the audit re-runs. **v8, v9 and v10 each trained no
heads.**
...
tail cells) + 0 (v8) + 0 (v9) + 0 (v10) = ` **12 CPU head mints**; time
`≈ 22 (v1–v6, incl. the discharge mint) + 4 (v7) + 2 (v8) + 1 (v9) + 1 (v10) = ` **≈ 30
wall-minutes**, `≈ 89 + 21 + 6 + 3 + 3 = ` **≈ 122 CPU-minutes**. All `$0`, zero GPU, no
...
unchanged by the move.)* The CPU-cap conflict was knowable from C09's
...
| **1f** the arena's materialisation of the per-cell head-key matrices from the banked mint `.npz` (**round-9 I-1**, the eighth uncounted loop — Phase 1c prices the arena's *ro cache* load, Phase 1e a *metadata-only* read, Phase 2b a build on *in-memory* arrays; none prices `np.load(...)['K_*']`) | `60` cells × 2 matrices (`h_std`, `h_ow`) | **measured `0.0041 s` per matrix ⇒ `0.0083 s`/cell**; timed region = `np.load(...)` **plus `np.asarray` on the named array**, warm cache | `1.0 s` (measured product `0.50 s`; carried at the conservative bound for a cold cache) |
...
**Total, re-multiplied:** `2927.6 + 1.0 + 0.7 + 0.1 (Phase 7z) + 1.0 (Phase 1f) = `
**`2930.4 s = 48.8 min`** corroborating; **`× 1.25 = 3663.0 s = 61.1 min`** conservative. (Round-9
I-1 adds Phase 1f at `1.0 s`; round-9 I-2 reduces the `GATE-ZEROOP` row from `0.3` to `0.1 s` now
that `U_tie` is measured and the cell count follows §6.5.)
...
`9.3 %`; a 2× miss moves the total to `3204.1 s = 53.4 min`, a 5× miss to `4025.2 s = 67.1 min`.
...
## 14. Cumulative disposition — all nine rounds

**Every row cites the v9→v10 section(s) implementing it; every limb is quoted VERBATIM from round
9's review with its location there (round-9 M-4); §14.1 prints the scripted audit that checks both.**
No blanket adoption claim is made anywhere in this document.

### Round 9 (4 findings + 4 Minor) — row level

| finding | disposition | §(diffed) |
|---|---|---|
| **H-1** §7.9's headline assigns the discharge mint to v7, contradicting its own parenthetical, its own sum and §7.8 | **ADOPTED** — headline rewritten in round 9's own words; the reconciliation sentence dropped; v7 re-derived at `≈ 4 / ≈ 21` with four mints and the discharge mint's share moved to v6 | §7.9 |
| **H-2** §14.1's self-exclusion asserts rather than checks | **ADOPTED** — the `changed.add(SELF)` fallback is deleted; §14.1's status is printed from an actual comparison and only enters `touched` when it differs; §14.1 states that self-exclusion covers the **size only** | §14.1, §14.2 |
| **I-1** eighth uncounted loop: arena-side `.npz` key materialisation | **ADOPTED** — Phase 1f, measured unit `0.0083 s`/cell with the timing boundary stated, carried at the cold-cache bound `1.0 s`; total `2930.4`/`3663.0` | §8 |
| **I-2** Phase 7z's tie row has no measured unit and a cell count contradicting §6.5 | **ADOPTED** — `U_tie` measured at `2.0e-05 s`/item on one synthetic near-tie group; cells corrected to `12` per §6.5's aggregation (`72` items worst case, not `360`); row **reduced** `0.3 → 0.1 s` | §8 |
| **M-1** `:2049` is `"source_rows"`, not `"registered_null_rows_excluded"` (`:2050`) | **ADOPTED** | §5.2.2 |
| **M-2** the `§13 item N` scan misses `item 10` in §14's own I-2 row, and is the one reference line not naming its scope | **ADOPTED** — pattern widened to bare `item N`, and the line now names its scope | §14.1, §14.2 |
| **M-3** §1's table prints `—` for two cells that exist in `C01_A0_OUT.json` | **ADOPTED** — both filled: ZH `common` `0.8718`/`+1`, HateMM `endpoint_concat` `0.8598`/`+2`, read from the OUT file | §1 |
| **M-4** the limb table is the designer's paraphrase, not a transcription | **ADOPTED** — every limb below is a **verbatim quotation** with its location in round 9's review | §14 |

### Round 9 — LIMB level, quoted verbatim (round-9 M-4)

Each limb is round 9's own sentence, in quotation marks, with the finding it came from. Where round
9 stated a repair as a single sentence with clauses, each clause is a limb.

LIMB-TABLE-BEGIN

| finding | limb, quoted verbatim from round 9 | landed in |
|---|---|---|
| **H-1** | *"Rewrite the headline as 'Five CPU head mints are attributable to the v6–v7 rounds: one trained in v6 for §7.8's GATE-FLOOR discharge (33.5 s) and four trained in v7 for its four-cell displacement-tail table'"* | §7.9 |
| **H-1** | *"and drop 'Both are now stated so the two cannot be read as contradicting'"* | §7.9 |
| **H-1** | *"Then check the ≈ 5 wall-minutes / ≈ 25 CPU-minutes attributed to v7 still holds with four mints rather than five, or say what else is inside it"* | §7.9 |
| **H-2** | *"Delete the `changed.add(SELF)` fallback"* | §14.2 |
| **H-2** | *"and print §14.1's status honestly without its size"* (round 9's three-line replacement, adopted as written) | §14.2 |
| **H-2** | *"so a row citing §14.1 fails exactly when §14.1 did not change, and the printed line is a measurement rather than a convention"* | §14.2 |
| **H-2** | *"Then say in §14.1 that self-exclusion covers the size only, never the changed/unchanged fact"* | §14.1 |
| **I-1** | *"One Phase 1f row: 60 arena-side cell loads (or 66, if the full-fold mints are also read)"* | §8 |
| **I-1** | *"unit measured on one banked `.npz` with the key arrays materialised"* | §8 |
| **I-1** | *"state whether the timed region includes `np.load` alone or `np.asarray` on the arrays too, per the timing-boundary lesson §8 has just institutionalised"* | §8 |
| **I-1** | *"and re-multiply"* | §8 |
| **I-2** | *"Measure `U_tie` on one synthetic near-tie group of realistic size (a few milliseconds of CPU, no mint)"* | §8 |
| **I-2** | *"price the row as `≤ cap × cells × U_tie` with the cell count taken from §6.5's aggregation (6 per dataset, 12 total)"* | §8 |
| **I-2** | *"state the timing boundary"* | §8 |
| **I-2** | *"and re-multiply"* | §8 |
| **M-1** | *"`:2049` → `:2050`, or drop the description and keep the line number"* | §5.2.2 |
| **M-2** | *"name the scope on that line too"* | §14.2 |
| **M-2** | *"or widen the pattern to bare `item N`"* — both adopted | §14.2 |
| **M-3** | *"add the two cells, or one clause saying what the dash means"* | §1 |
| **M-4** | *"quote each prescribed limb verbatim from the previous round's review, with its location in that review, so the enumeration is checkable against a source rather than trusted"* | §14 |

LIMB-TABLE-END

**Round-9 rulings accepted without change.** The science layer is closed — bit-exact rebuild at
first attempt, every constructible wrong reading caught, no gate able to fire on a warranted CLOSE
(all twenty, re-derived), the verdict-path enumeration total and mutually exclusive. Round-8 C-1
and C-2 are genuinely closed. §13.1's items 15 and 19 carry what round 7 asked for and round 8 found
missing. The four-cell tail evidence is adequate because §13 item 25 is the run-time control.

**Round-9's method ruling, adopted into this document's practice.** *"It is no longer absence … it
is narrowing, and the limb table's paraphrase is where the weakening happens."* The response is
verbatim quotation with provenance (above), not a longer paraphrase. Round 9 also showed that
nothing in the machinery could have caught I-2's narrowing — *"I caught it by reading round 8's
sentence"* — which is why §15 and the round-10 request keep that obligation with the reviewer.

### Rounds 1–8 — carried

Round 9 audited v9's round-8 block at limb level against round 8's prescription text and found **no
unlanded limb**; the two findings above are narrowings, not absences. Earlier rounds stand as
recorded in v3/v4/v6/v7/v8/v9 §14; where a later round refined an earlier disposition, the later
section governs.
...
**The convention, and what it does and does not cover (round-9 H-2).** §14.1 is **self-excluding for
its SIZE ONLY**: the audit never prints §14.1's byte count, because pasting the transcript changes
the length of the section being measured. **It does not exempt §14.1 from the changed/unchanged
check.** v9's script contained a fallback that added §14.1 to the touched set when it was
*unchanged*, so a row citing §14.1 could pass without §14.1 having been edited — round 9
demonstrated it by construction. That branch is deleted; §14.1's status is now computed from an
actual comparison and printed as `CHANGED`, `UNCHANGED` or `ADDED`, and only a genuine change adds
it to the touched set. **A row or limb citing §14.1 now fails exactly when §14.1 did not change — verified by the same
construction round 9 used to break v9.** I built a counterfactual identical to v10 except that its
§14.1 is v9's verbatim, and ran the corrected script on it: it prints `UNCHANGED §14.1 (self, size
not reported)`, **fails** the two §14.1-citing rows and the one §14.1-citing limb, and step (6)
reports `named by a row but unchanged: ['14.1']`. Under v9's script the same counterfactual printed
`CHANGED` and passed all of them.

**How the transcript below was produced.** The corrected script was executed against the
**finished, on-disk v10** as the last action before this document was closed, then re-executed
against the document with the transcript already embedded; the two outputs are byte-identical, so
the transcript is a **verified** fixed point. v10's drafting also writes after every edit and
verifies each write.

**What the audit does.** (1) Splits v9 and v10 into sections and diffs them, printing §14.1's status
without its size. (2) For every §14 row, checks each cited section appears in the diff. (3) Flags
any row whose cited section shows no diff. (4) Resolves references — emphasis-tolerant and
bare-`item N`-tolerant for §13 items, with **every reference line naming its scope** (round-9 M-2) —
reporting dotted `§N.N` and bare `§N` separately. (5) **Limb level**: parses the `LIMB-TABLE` and
checks each limb's landing section diffed. (6) **Coverage both ways**: sections that changed but no
row or limb cites, and sections a row names that did not change.

**What the audit still cannot do, stated because round 9 proved it by example.** It verifies limbs
against **this document's transcription** of the prescriptions. v10 makes that transcription
verbatim with provenance, which makes the narrowing round 9 found *visible*, but the check that the
quotation is faithful and complete remains a **reading** obligation and belongs to the reviewer, not
to the script.

**Output, verbatim:**

```
=== (1) SECTION DIFF v9 -> v10 ===
  CHANGED  §1        +28 chars
  CHANGED  §8        +1335 chars
  CHANGED  §14       -84 chars
  CHANGED  §15       +103 chars
  CHANGED  §7.9      +822 chars
  CHANGED  §14.2     +419 chars
  CHANGED  §5.2.2    +90 chars
  CHANGED  §14.1     (self, size not reported)
  CHANGED  header    +943 chars
  UNCHANGED: 49 sections

=== (2)+(3) DISPOSITION ROWS vs DIFF ===
  OK    H-1   cites §7.9
  OK    H-2   cites §14.1, §14.2
  OK    I-1   cites §8
  OK    I-2   cites §8
  OK    M-1   cites §5.2.2
  OK    M-2   cites §14.1, §14.2
  OK    M-3   cites §1
  OK    M-4   cites §14
  rows verified against diff hunks: 8 ; rows failing: 0
...
=== (4) REFERENCE RESOLUTION (emphasis-tolerant; dotted AND bare) ===
  ...
  §13 item refs (scope: '§13 item N' or bare '**item N**'): [5, 7, 8, 19, 23, 25, 26] ; defined 1..26 ; unresolved: NONE

=== (5) LIMB-LEVEL DISPOSITION (round-8 prescription) ===
  OK    H-1   *"Rewrite the headline as 'Five CPU head mints are a -> §7.9
  OK    H-1   *"and drop 'Both are now stated so the two cannot be -> §7.9
  OK    H-1   *"Then check the ≈ 5 wall-minutes / ≈ 25 CPU-minutes -> §7.9
  OK    H-2   *"Delete the `changed.add(SELF)` fallback"*          -> §14.2
  OK    H-2   *"and print §14.1's status honestly without its size -> §14.2
  OK    H-2   *"so a row citing §14.1 fails exactly when §14.1 did -> §14.2
  OK    H-2   *"Then say in §14.1 that self-exclusion covers the s -> §14.1
  OK    I-1   *"One Phase 1f row: 60 arena-side cell loads (or 66, -> §8
  OK    I-1   *"unit measured on one banked `.npz` with the key ar -> §8
  OK    I-1   *"state whether the timed region includes `np.load`  -> §8
  OK    I-1   *"and re-multiply"*                                  -> §8
  OK    I-2   *"Measure `U_tie` on one synthetic near-tie group of -> §8
  OK    I-2   *"price the row as `≤ cap × cells × U_tie` with the  -> §8
  OK    I-2   *"state the timing boundary"*                        -> §8
  OK    I-2   *"and re-multiply"*                                  -> §8
  OK    M-1   *"`:2049` → `:2050`, or drop the description and kee -> §5.2.2
  OK    M-2   *"name the scope on that line too"*                  -> §14.2
  OK    M-2   *"or widen the pattern to bare `item N`"* — both ado -> §14.2
  OK    M-3   *"add the two cells, or one clause saying what the d -> §1
  OK    M-4   *"quote each prescribed limb verbatim from the previ -> §14
  limbs landed: 20 ; limbs open/failing: 0

=== (6) CHANGED-BUT-UNCITED / NAMED-BUT-UNCHANGED ===
  changed but cited by no row/limb: ['15', 'header']
  named by a row but unchanged:    NONE
```

**Reading it.** Every round-9 row cites a section the diff shows as changed, and every one of the 20
limbs names a section that diffed. Step (6)'s *changed-but-uncited* list returns **§15 and the
header** — the open-issues list and the supersession/disposition preamble, which necessarily change
every round and which no round-9 finding is about; that is structural, not a finding. Step (6)'s
*named-but-unchanged* list is **empty**, which is the check that would have surfaced §13 in v8 and
which round-9 H-2's counterfactual shows now works for §14.1 too. The row scan excludes the
limb-table region, so the two counts (**8 rows**, **20 limbs**) are disjoint.

*(One consequence of rebuilding §14 this round: the `§13 item` scan now returns `[5, 7, 8, 19, 23,
25, 26]` where v9 returned `[…, 15, …]`. Items 10 and 15 are still **defined** in §13 and still
carry their round-7/round-8 repairs; what disappeared are the *disposition-row references* to them,
because v9's round-5 and round-8 blocks were compressed into the "Rounds 1–8 — carried" paragraph.
`unresolved: NONE` holds throughout, and no §13 item lost content — verify against §13 directly.)*
...
```python
#!/usr/bin/env python3
"""Mechanical disposition verification, C06 falsifier v9.
(1) section diff v8->v9   (2) every §14 row cites a diffed section   (3) rows failing that are flagged
(4) reference resolution (emphasis-tolerant, dotted AND bare)        (5) LIMB-LEVEL disposition
(6) changed-but-uncited / named-but-unchanged                        §14.1 is SELF-EXCLUDING.
"""
import re, sys
V_OLD='/data/jehc223/RGCL/refine-logs/C06_FALSIFIER_PREREG_DRAFT_V9.md'
V_NEW='/data/jehc223/RGCL/refine-logs/C06_FALSIFIER_PREREG_DRAFT_V10.md'
...
items=[int(m.group(1)) for m in re.finditer(r'§?13\s*(?:\*\*)?\s*items?\s*(?:\*\*)?\s*(\d+)', b)]
items+= [int(m.group(1)) for m in re.finditer(r'(?<!§13 )\*\*item (\d+)\*\*', b)]
items+= [int(m.group(1)) for m in re.finditer(r'(?<![§\w.])item (\d+) (?:requires|carries|now|must|is|reads)', b)]
s13=SB.get('13.1','')+SB.get('13','')
defined={int(x) for x in re.findall(r'\*\*\((\d+)\)', s13)}
un_i=sorted({i for i in items if i not in defined})
print('  §13 item refs (scope: \'§13 item N\' or bare \'**item N**\'): %s ; defined 1..%d ; unresolved: %s'
      %(sorted(set(items)), max(defined) if defined else 0, un_i or 'NONE'))

print()
print('=== (5) LIMB-LEVEL DISPOSITION (round-8 prescription) ===')
...
## 15. Open issues for round 10

1. **Narrowing, not absence (round-9's diagnosis).** Round 9 showed that the limb machinery cannot
   catch a limb whose *paraphrase* drops a clause — it caught I-2 *"by reading round 8's
   sentence"*. v10's answer is verbatim quotation with provenance. **Round 10 should test it the
   only way that works: read round 9's prescription text and check each quoted limb is complete and
   faithful, not merely present.** If a quotation is truncated or a clause is missing, that is the
   finding.
2. **The corrected self-exclusion (§14.1, §14.2).** Round 9 broke v9's version by construction.
   Round 10 should do the same to v10's: build a counterfactual whose §14.1 is byte-identical to
   v10's and confirm the script now prints `UNCHANGED` and **fails** the §14.1-citing rows and
   limbs.
3. **Phase 1f and the corrected Phase 7z (§8).** Two rows moved this round on measured units. Round
   10 should re-derive both, check the timing boundaries are stated, and rule whether carrying
   Phase 1f at the cold-cache bound (`1.0 s` against a measured `0.50 s`) is the right convention or
   an inconsistency with how the other measured rows are carried.
4. **The ninth uncounted loop.** Nine rounds, eight found. Hunt again.
5. **§1's table after M-3.** Two cells were filled from `C01_A0_OUT.json`. Round 10 should confirm
   the table now has no unexplained `—`, and that the added values do not disturb §1's *"load-bearing
   twice over"* role or §6.2's argument.
6. **Is the record now sound?** Round 9 answered *no* and named two narrowings, both closed above.
   Nine rounds is evidence of nothing in either direction — round 9 said so explicitly and held the
   line. If round 10 finds the design and the record clean, say **GO** plainly; if not, name the
   specific defect.

---

*No GPU, SLURM, Modal, teacher call, cache write, test-split access, job submission or commit
occurred in producing this document. **v10 trained no heads**; its measurements were the arena-side
`.npz` key-materialisation unit (round-9 I-1), `U_tie` (round-9 I-2), the two `C01_A0_OUT.json`
cells (round-9 M-3), and the audit re-runs — all read-only on banked train-split artifacts and on
the drafts themselves. The **twelve** CPU head mints across v1–v10 (§7.9's sum) wrote nothing
outside the session scratchpad and computed no battery-arm accuracy. `TARGET_STATE.json` was read
and not modified. v1–v9 are unmodified.*
