...
- **Original bottleneck:** a dense, assignment-specific, train-only MLLM signal must move shared full-video query/key geometry and final kNN.
- **Why still solved:** the same single ordinal tangent loss remains; only its validated path is now frozen rather than silently reselected.
- **Drift rejected:** no class output, rationale, segment/timestamp/span, teacher key/embedding, extra encoder/head/router, test view, rerank, scale or SSR/EDCM action.

## Simplicity Check

- **Dominant contribution:** label-blind withholding relations, after class-conditional transfer validation, supervise a frozen-anchor supported response of the exact epoch-refreshed full-bank true-class margin.
- **Removed ambiguity:** one fixed medoid ID, one fixed adjacent radius pair, one effective-sample formula, one A1 high-noise gate, one bootstrap p-value rule.
- **No added component:** all changes are frozen protocol definitions and stop rules.

## Changes Made

1. **Frozen validated tangent:** A0 freezes each dataset/fold medoid video ID and adjacent radii before teacher calls. Refresh re-encodes that ID but never replaces it or the radii. Support and anchor-direction drift can only stop, not trigger reselection.
2. **Exact A0 rotation:** fit on A, select on B, refit on A∪B, predict C; rotate. Strict OOF A0 targets are cached before pilot selection and reused by A1 label-only.
3. **Exact A1 cells/statistics:** defined video-clustered effective sample size per dataset×label×relation; fixed high noise for A1 and both rates for A2/final; clarified assignment-free/mask-free gradient matching.
4. **Exact endpoint inference:** deterministic +.030 is checked against the max historical/paired bar; paired centered-null inference uses same-seed predictions only and never pretends to pair with a historical scalar.
...
For current normalized projected modality features `p_i^m`, define exactly one prototype candidate: the spherical medoid ID minimizing summed cosine distance at the A0-selected checkpoint. For `a∈{.05,.10,.20,.30}`,

\[
\tilde p_i^m(a)=norm((1-a)p_i^m+a p_{anchor}^m).
\]
...
**Freeze before any teacher call:** for every strict A0 fold/dataset and the post-A0 full-train dataset checkpoint, hash the medoid video ID and `(a1,a2)`. Never reselect either. At later refreshes, re-encode the fixed medoid ID with the current shared encoder and recompute only the support mask at both fixed radii. Every control uses the identical ID/radii/mask rule.
...
For relation cell `r`, combine a video's modality weights as `w_vr=min(1,sum_{m:r_vm=r}rho_vm)` and define `n_eff=(sum_v w_vr)^2/sum_v w_vr^2`. Require `n_eff>=10` for every dataset×label×relation, preserve `|T|<=d0`, ordered `D_preserve<D_weaken<D_reverse`, and class-specific ordinal slope 95% lower bound >0 at both radii. Pooled evidence cannot rescue a class.
...
Three claim blocks suffice: A0 action capacity; A1 <=128 class-conditional assignment value; A2/final actual +3/+3 kNN attribution. Estimated A0 is 20–40 GPU-hours pending a SLURM microbenchmark; A1 <=2048 calls; A2/final only after gates. All compute uses SLURM/HateVideo. Gold annotation cost is zero; pseudo-relations are not annotations.
