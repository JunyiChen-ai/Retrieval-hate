# C09 Stage-0 (A0) — preregistration **v10** (round-9 repairs)
...
> ## STATUS: `V10_REPAIRED_NOT_FROZEN_NOT_SUBMITTED` — awaiting fresh independent review.
>
...
> first round with no Critical and no High**) → **this file**.
>
> **`v1–v9 are superseded in full and must not be implemented.`** Each carries its own
> ledger; **the v10 ledger is §12**. Round 9 diffed v8 → v9 line by line, re-verified
> R1–R8 against v9 itself, and found the science unmovable — *"§§2–9 and §11's
> pre-existing text are byte-identical to v8. No rule, threshold, feature set, null,
> gate or arithmetic changed."* Its three Importants were all statements **about** the
> document's or the record's own contents; all three are closed here.
>
...
## 0. What v10 changes, and how this design reached its present form

Round 9 returned **`0C / 0H / 3I`** — the first round with no Critical and no High — and
its bottom line was *"The science is finished and I could not move it… Fix them and
freeze."* All three findings are statements **about** the document's or the record's own
contents, not about the design:

1. **v9's own change summary understated v9's own changes**, in §0 and again in §12: both
   said nothing outside §0 / STATUS / §1 / §11 had moved, while §10 had gained a scope
   bullet that §0's item 4 and §12's own table both name. **Corrected in both places
   (R9 I-1).**
2. **§6.1 called F113's nine non-transferring arms *"at least six distinct operator
   families"* and said the source's "four" was not re-derivable.** Re-opening the source:
   `HEADSPACE_TRANSFER_PREGATE.md:623` heads the ladder *"### 4.10 The transfer ladder
   (§2.10) — **four operator families**, one arena swap"*, and its rung column groups the
   nine positives as VSW (with its λ-oracle and DEG-D control), F95, FIXK and the
   un-runged `THRESH_best`. The error entered at round 5 and four rounds re-attested it.
   **Restored to the source's own taxonomy (R9 I-2).**
3. **§6.1 said `HEADSPACE_TRANSFER_PREGATE.md:871-873` phrases the fitted-relation-score
   sentence "at 2 dp".** It is at **3** (`−0.064 / −0.129`, `+0.157 / +0.230`).
   **Corrected (R9 I-3).**

**Nothing in §§2–9 changed in v10, and nothing in §§1–11 changed in substance.** The
edits are three sentences in §0/§6.1, the STATUS block, and §12.
...
withdrawn (R9 I-2). Concluding at `:863-864` *"9 of 9 raw positives fail to transfer; the median shrink is
...
## 12. Repair ledger — the 3 round-9 findings

Round 9's verdict was **`REVISE (0C / 0H / 3I)`** — the first round with neither a
Critical nor a High — with the bottom line *"The science is finished and I could not
move it… These are edits to three sentences. They need no re-measurement, no GPU and no
re-derivation of anything in §§2–11. Fix them and freeze."*

**Scope of v10, stated exactly (this is R9 I-1's own repair):** the edits are the title,
the STATUS block, §0, two sentences of §6.1, and §12. **§§2–5, §7, §8, §9, §10 and §11
are byte-identical to v9**, and **no decision rule, threshold, feature set, null, gate or
arithmetic has changed since v8.**

| # | round-9 finding | repair | where |
|---|---|---|---|
| **I-1** | v9's change summary understated v9's own changes in both places that state them: §0 said *"v9 changes nothing in §§1–11's substance"* and §12 said *"Nothing in §§2–10's substance changed in v9"*, while §10 had gained a scope bullet that §0's item 4 and §12's own I-3 row both name — the same class of defect R8 rated High, pointing the other way | **both sentences rewritten**: v9's true surface is named (§0, STATUS, §1, **§10** and §11), and v10's own scope is stated in the same terms above, with the invariant that matters spelled out — no rule, threshold, feature set, null, gate or arithmetic has moved since v8 | §0, §12 |
| **I-2** | §6.1's *"at least six distinct operator families"* contradicts the cited source's own heading, and its *"v5's 'four' was not re-derivable"* clause is false | **restored to the source's taxonomy**: `HEADSPACE_TRANSFER_PREGATE.md:623` heads the ladder *"four operator families, one arena swap"*, and the rung column groups the nine positives as VSW (with its λ-oracle and DEG-D control arms), F95, FIXK and the un-runged `THRESH_best`; the "not re-derivable" clause is **withdrawn**; the separate and still-correct point — that the nine are **not one lineage**, which is what v4 got wrong — is retained | §6.1 |
| **I-3** | §6.1 said `HEADSPACE_TRANSFER_PREGATE.md:871-873` phrases the fitted-relation-score sentence *"at 2 dp"* | corrected to **3 dp** (`−0.064 / −0.129` against `+0.157 / +0.230`), with the fold count's real location (`:847` / `:901`) named | §6.1 |

---

*v10. No hash frozen, no config written, no code implemented, no namespace created, no
