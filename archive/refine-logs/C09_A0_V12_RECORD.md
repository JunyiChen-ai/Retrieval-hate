# C09 Stage-0 (A0) — preregistration **v12** (round-11 repairs)
...
> ## STATUS: `V12_REPAIRED_NOT_FROZEN_NOT_SUBMITTED` — awaiting fresh independent review.
>
...
> `C09_A0_V11_PREREG_REVIEW.md` (`REVISE 0C/0H/2I`) → **this file**.
>
> **`v1–v11 are superseded in full and must not be implemented.`** Each carries its own
> ledger; **the v12 ledger is §12**. Rounds 9, 10 and 11 all found the science unmovable
> — round 11: *"I could not move the science, and neither of my findings is a defect in
> it."* Round 11's first Important is nonetheless **the first arithmetic error found in
> this document since round 6**: the pre-declared `n_unstable ≈ 7–9` was a *per-seed*
> quantity presented as an *item-level* population, and it is arithmetically
> incompatible with the same section's `|P_0| ≈ 76 / 55` and the banked per-seed error
> counts. It is corrected here, together with the scope statement.
>
...
## 0. What v12 changes, and how this design reached its present form

Round 11 returned **`0C / 0H / 2I`** with the bottom line *"I could not move the science,
and neither of my findings is a defect in it."* Its first finding is nonetheless the
first **arithmetic** error found in this document since round 6, and it is corrected in
full.

1. **The pre-declared `n_unstable ≈ 7–9` was a per-seed quantity presented as an
   item-level population, and it cannot coexist with `|P_0| ≈ 76 / 55` (R11 I-1).**
   §4.2 defines an unstable error at the **item** level, so with `a =` #(wrong in exactly
   2 seeds) and `b =` #(exactly 1), `Σ_s |E_s| = 3|P_0| + 2a + b` and
   `n_unstable = a + b`. The per-seed error counts are **measured, not transferred**:
   `83 + 85 + 85 = 253` (HateMM) and `62 + 64 + 61 = 187` (MHC-ZH). At `|P_0| = 76 / 55`
   that gives `2a + b = 25 / 22`, hence **`n_unstable ∈ [13, 25]` (HateMM) and
   `[11, 22]` (MHC-ZH)** — `7–9` is not merely wrong on HateMM, it is **unreachable**
   (minimum 13). Worse for the claim it was used to make: the very F88 datum §4.2 quotes
   (*"NOTHING at exactly 2/3"*) forces `a = 0` on MHC-ZH, i.e. **`n_unstable = 22`, above
   the `20` trigger**, so `CONTROL_UNDERPOWERED` would **not** fire on that leg. The `7–9`
   figure was the per-seed excess `84.33 − 76 = 8.33` and `62.33 − 55 = 7.33`.
   **§6.3's expectation, §5.3's out-of-support figure and §10's stability-premise bullet
   are all restated on the corrected arithmetic**, with the ZH consequence made explicit.
2. **The scope statement mis-described its own diff for the fourth round running
   (R11 I-2).** v11 said "two sentences of §6.1" (one changed), "§7's two
   `CONFIG-MATCHED-CORRECT` sentences" (one changed; the controls enumeration is
   untouched), and §0 and §12 used different words for the same §5.2 edit. **Corrected in
   both places, with one convention.**

**Scope of v12, stated exactly.** The edits are the title, the STATUS block, §0, one
sentence of §5.3, three sentences of §6.3, one bullet of §10, and §12. **§§1, 2, 3, 4,
5.1–5.2, 5.4–5.6, 6.1, 6.2, 7, 8, 9 and 11 are byte-identical to v11, and no decision
rule, threshold, feature set, null, gate or arithmetic has changed since v8.**
...
1. **unstable errors** — on the transferred population, `13–25` (HateMM) / `11–22`
   (MHC-ZH) items, §6.3 (R11 I-1: v11 wrote `≈ 7–9`, a per-seed figure) — at both `τ`;
   and
...
`mlp_mean3`, and the λ-oracle ceiling), spanning **four operator families** on that
record's own taxonomy — `:623` heads the ladder *"### 4.10 The transfer ladder (§2.10) —
four operator families, one arena swap"*, and its rung column groups the nine as
**VSW** (rung 1, including `ORACLE_lambda_pow` *"(hindsight ceiling)"* and
`CTRL_cos_pow` *"(DEG-D, no verifier)"*, which are VSW's own oracle and control arms),
**F95** (rung 2), **FIXK** (rung 3) and the un-runged `THRESH_best`
*"(global recalibration)"*. **They are not one lineage** — which is the point v4's
"all nine descending from F105/VSW" got wrong — but the count is the source's four, not
the "at least six" v6–v9 carried from R5 I-5b, whose "not re-derivable" clause is
withdrawn (R9 I-2). The record concludes at `:863-864` — the clause begins on `:863`
...
- **`UNSTABLE-POP`.** `D-FELDMAN` re-run with the target redefined as unstable errors,
  ...
  **Pre-declared expectation, re-derived correctly (R3 I-9; arithmetic corrected R11
  I-1).** The banked floors give **measured** per-seed error counts of `83 / 85 / 85`
  (HateMM, from `744 × (1 − 0.8884 / 0.8858 / 0.8858)`) and `62 / 64 / 61` (MHC-ZH, from
  `579 × (1 − 0.8929 / 0.8895 / 0.8946)`), summing to `253` and `187`. `n_unstable` is an
  **item** count (§4.2), so with `a =` #(wrong in exactly 2 seeds) and `b =` #(exactly
  1), `Σ_s |E_s| = 3|P_0| + 2a + b` and `n_unstable = a + b`. On the transferred F88
  population `|P_0| ≈ 76 / 55` this gives `2a + b = 25 / 22`, hence

  > **`n_unstable ∈ [13, 25]` (HateMM) and `[11, 22]` (MHC-ZH)** — not `≈ 7–9`, which was
  > the *per-seed* excess (`84.33 − 76 = 8.33`, `62.33 − 55 = 7.33`) and is
  > **arithmetically unreachable** on HateMM, where the minimum is 13.

  **Consequences, both directions, declared now.** On **MHC-ZH** the F88 datum §4.2
  quotes — *"NOTHING at exactly 2/3"* — forces `a = 0`, i.e. `n_unstable = 22`, **above**
  the `20` trigger, so the count leg of `CONTROL_UNDERPOWERED` would **not** fire there
  and the control may be genuinely powered on that leg; whether it fires would then turn
  on the CI-width leg alone. On **HateMM** the transferred interval straddles the
  trigger. **The honest pre-declaration is therefore that `CONTROL_UNDERPOWERED` is
  plausible but not expected on either dataset, and that whether this A0 tests the
  registry claim's stability premise is decided by the realised tag, not in advance.**
  The rule itself is unchanged, data-independent, and applied identically to both
  datasets.
...
- **This A0 does not separate H-DATA-DEFECT from H-TOPOLOGY** (§5.5); it only prices
  the overlap. **Whether it tests the registry claim's stability premise is decided by
  the realised `CONTROL_UNDERPOWERED` tag, not in advance** (§6.3, corrected R11 I-1):
  on the transferred population `n_unstable ∈ [13, 25] / [11, 22]`, and on MHC-ZH the
  F88 transfer (`a = 0`) puts it at `22`, **above** the `20` trigger. v11 declared the
  control expected-underpowered on both datasets on a per-seed figure that cannot hold;
  the corrected statement is that it is plausible but not expected, and the verdict
  record must report which leg, if either, fired.
...
## 12. Repair ledger — the 2 round-11 findings

Round 11's verdict was **`REVISE (0C / 0H / 2I)`**, with the bottom line *"I could not
move the science, and neither of my findings is a defect in it… Both repairs are text…
Neither needs re-measurement, GPU, or any change to a rule, threshold, feature set, null,
gate or arithmetic."* Round 11 also re-derived, from scratch and independently, both
permutation nulls' exactness arguments, the Hanley–McNeil algebra, the macro-F1
monotonicity, and **both** of the mint's run-time sha256 assertions.
...
| # | round-11 finding | repair | where |
|---|---|---|---|
| **I-1** | `n_unstable ≈ 7–9` is a **per-seed** quantity presented as an **item-level** population; it is arithmetically incompatible with the same section's `\|P_0\| ≈ 76 / 55` and the banked per-seed error counts, and on MHC-ZH the document's own F88 transfer puts the true figure at **22 — above the `20` trigger** the scope bullet relies on. **The first arithmetic error found in this document since round 6.** | **re-derived from the identity `Σ_s \|E_s\| = 3\|P_0\| + 2a + b`**: measured per-seed counts `83/85/85` and `62/64/61` sum to `253 / 187`, giving `2a + b = 25 / 22` at `\|P_0\| ≈ 76 / 55`, hence **`n_unstable ∈ [13, 25]` / `[11, 22]`**, with `7–9` identified as the per-seed excess and **unreachable** on HateMM. **Both consequences declared:** on MHC-ZH `a = 0` (F88's *"nothing at exactly 2/3"*) gives `n_unstable = 22`, so the count leg would **not** fire; on HateMM the interval straddles the trigger. §6.3's expectation is restated as *plausible but not expected*, §5.3's out-of-support figure is corrected, and **§10's stability-premise bullet now depends on the realised tag rather than on an expectation** | §5.3, §6.3, §10 |
| **I-2** | the scope statement mis-described its own diff for the fourth round running: "two sentences of §6.1" (one changed), "§7's two `CONFIG-MATCHED-CORRECT` sentences" (one changed; the controls enumeration is untouched), and §0 and §12 used different words for the same §5.2 edit | **v12's scope is stated once, in one convention, in both places**, and enumerates the byte-identical sections explicitly so the claim is checkable against a diff | §0, §12 |

**What round 11 checked and left below its own line, left as it left it:** §5.3's
"superseded phrasing" note (still true after the alias insertion, since `P_τ` and
`A^{(f)}` remain different objects); §11's F66 paragraph sitting in mild tension with the
NCA counterweight (a tension that runs **against** the candidate, Stage-1-scoped, and
already covered by precondition (d)); F75's semicolon truncation (nothing adverse
hidden); typographic normalisation inside quotations; §4.3's `≈ 1.4 %` for a withdrawn
v4 construction; §2's budget summing to 116 against "≈ 115"; §2's `ksweep` parenthetical
erring against the design; and §5.3's R10 attribution slack.

---

*v12. No hash frozen, no config written, no code implemented, no namespace created, no
