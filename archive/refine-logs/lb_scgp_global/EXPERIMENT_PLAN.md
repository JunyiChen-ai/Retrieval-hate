...
| M1 | B1 | Build and seal train-only label-blind cache | cache MHC, cache MHC-ZH, seal decision | Merkle seal, zero forbidden counters, schema/QC pass | 24 CPU-h, 16 GPU-h (2x8, local Qwen2.5-VL-7B inference), 4512 base API calls, 9024 retry cap | invalid cache becomes unresolved or STOP, not rescue |
...
- M1 GPU-hour erratum (M1_CACHE_PLAN_AMENDMENT.md, 2026-07-13; rollback per M1_CACHE_CODE_REVIEW.md ruling (1)): the two M1 cache runs perform local Qwen2.5-VL-7B inference and each need 1 GPU / 8 GPU-h (seal is CPU-only). These 16 GPU-h were omitted (0) from the run-row-summing GPU-hour aggregates; forward-looking views are corrected +16 GPU-h (684 -> 700). The one historical snapshot (`original_approved_r2_envelope_before_v2`) is intentionally RETAINED at 684 — the omission was genuinely present at approval time, so the snapshot keeps its as-approved value plus a superseded annotation. CPU-h, storage, and API figures are unchanged everywhere.
- Original approved R2 paper-plan envelope before the v2 supplement (HISTORICAL snapshot, retained at 684; forward views corrected to 700): 64 MUST plus 1 NICE; MUST 704 CPU-h / 684 GPU-h / 4512 base API calls / 9024 retry-cap API calls / 786 GB; total with NICE 720 CPU-h / 684 GPU-h / 4512 base API calls / 9024 retry-cap API calls / 791 GB.
- Substitution paper-plan view (forward): v2 replaces exhausted v1 as the synthetic KKT scientific slot; CPU-h/API/storage remain as original R2 and GPU-h carries the +16 M1 correction: MUST 704 CPU-h / 700 GPU-h / 4512 base API calls / 9024 retry-cap API calls / 786 GB; total with NICE 720 CPU-h / 700 GPU-h / 4512 base API calls / 9024 retry-cap API calls / 791 GB.
- Lifetime lineage envelope: retain v1 historical planned 32 CPU-h / 0 GPU-h / 0 API / 5 GB and add v2 planned 32 CPU-h / 0 GPU-h / 0 API / 5 GB. Machine run-row totals therefore become MUST 736 CPU-h / 700 GPU-h / 4512 base API calls / 9024 retry-cap API calls / 791 GB; total with NICE 752 CPU-h / 700 GPU-h / 4512 base API calls / 9024 retry-cap API calls / 796 GB.
- Remaining prospective budget after Run1 FROZEN and v1 FAIL_STOP: MUST 700 CPU-h / 700 GPU-h / 4512 base API calls / 9024 retry-cap API calls / 785 GB; total with NICE 716 CPU-h / 700 GPU-h / 4512 base API calls / 9024 retry-cap API calls / 790 GB. Execution-authorized remaining budget now is 0.
...
- Wave 1: run the two M1 cache jobs (MHC, MHC-ZH) as at most two concurrent 1-GPU jobs (4 CPU / 32 GB / 1 GPU each; 2x1 GPU is within the 2-GPU cap), if API/CPU policy allows; then run the CPU-only seal decision.
