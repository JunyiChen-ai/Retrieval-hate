# C06 `$0` CPU falsifier — preregistration **DRAFT v6** (2026-08-04)

**SUPERSESSION.** Supersedes `C06_FALSIFIER_PREREG_DRAFT_V5.md` → V4 → V3 → V2 → v1. All five
remain on disk **unmodified** as the record of what each round reviewed. Reviews of record:
`C06_FALSIFIER_PREREG_REVIEW{,_R2,_R3,_R4,_R5}.md` — REVISE 3C/6H/10I+4M, 3C/3H/7I+3M,
2C/1H/6I+4M, 3C/3H/8I+4M, 3C/3H/6I+5M. Complete standalone document.
...
**Disposition: all 17 round-5 findings ADOPTED, 0 rebutted, plus one defect the designer found
that no round flagged (D-1, §5.2).** Round 5's audit was **18 of 18 VERIFIED ADOPTED** — the first
round with no failed adoption. Cumulative table in **§14**.
...
inside the frozen `2e-6` bar (§7.9).
...
`required_halt_only_validity_guards` (seven entries) does **not** contain it. Its **arm scope is
...
non-rejecting. **Measured, this makes the bar invariant to a drop:** executing `holm_adjust`, the
witness's 24 hypotheses reject `24/24` identically under `m = 92` padded with `0.5`, under `m = 92`
padded with `1.0`, and under `m = 46` with the dropped hypotheses removed. So the choice cannot be
gamed in either direction — engineering a drop changes S4's difficulty by nothing at all — and
freezing rather than recomputing is what makes that guarantee auditable. §8 Phase 4 continues to
price 92 comparison-cells, which is now an exact count rather than an upper bound.
...
* An **absent** decision or gate quantity HALTs on the same footing as a non-finite one —
  **scoped, per round-5 C-2a, to the lineage(s) that passed their per-lineage gates.** v5 stated
  ...
  > required**, and are excluded from every decision rule and from the S5 family. **Absence by
  > declared drop is lawful; absence by computation failure in a surviving lineage still HALTs.**
...
4. **S6 binds, and v4's vacuity claim was false (round-4 C-2).** With S3 and S6 now measured
   against the **same** reference arm (§5.2.1), `mean_s net_s = n_D · (acc(A) − acc(reference))`
   exactly, so the two are statements about one quantity. S3 is defined on the **seed
...
**The residual is now measured on a trained head, and it is the head-space value that binds**
(§7.9). Measured at `θ = 45`: **`2.384e-07`**, giving a similarity window of `1.079e-05`. v5
...
Only the trained figure is quoted as load-bearing; the untrained ones are recorded in §7.9 as the
...
printed or recorded at any point in v1–v6.** Rounds 4 and 5 audited this by grepping every decimal
...
published C01 dev-arena accuracy, a majority/band constant or a unit-time string. **v6 adds two
measured accuracies and neither is a battery arm:** §7.9's `0.8725` is the native deployed vote,
the instrument anchor already published in §6.
...
### 7.9 `GATE-FLOOR` discharged, and the trained-head algebra residual
...
**The head-space `GATE-ALGEBRA` residual, on a trained head.** Round 5 measured this on *untrained*
heads and obtained `1.863e-07`–`1.974e-07`; measuring the same quantity here on untrained heads at
three seeds on both datasets gave `1.175e-06`–`1.362e-06`. Two honest measurements of one quantity,
`6×` apart. **The discrepancy is the finding:** at an untrained head the two orbit endpoints are
near-collinear (median `‖l2(h_ow) − l2(h_std)‖ = 0.0032`), so the `θ = 45` residual is dominated by
the `cos45 − sin45 = 1.11e-16` asymmetry acting on a near-zero difference, and it is not a stable
estimate of anything. On the **trained** head above:

| quantity | trained head | untrained heads |
|---|---|---|
| `θ = 0` residual | `1.490e-08` (`134×` inside the `2e-6` bar) | `1.118e-08` |
| `θ = 45` residual | **`2.384e-07`** (**`8.4×`** inside the bar) | `1.175e-06`–`1.974e-07`, unstable |
| median `‖l2(h_ow) − l2(h_std)‖` | **`0.2301`** | `0.0032` |

Two consequences. **`GATE-ALGEBRA`'s frozen `2e-6` bar transfers to head space with `8.4×`
headroom** — the first evidence that C01's constant is usable here at all, and the answer to
whether the gate carries a false-HALT risk on a correct run. And **the trained head does not
collapse the prompt orbit**: median displacement `0.2301` against `0.0032` untrained, a `72×`
difference, which independently corroborates §6.1's trained-head `ρ` result and finally retires the
last remnant of v1's "219× contraction" worry — that figure was an initialisation artifact, exactly
as round 1's I-10 warned. No accuracy of any battery arm was computed in producing either row.

### 7.8 Dry-check cost

v6's measurements added ≈ **3 wall-minutes / ≈ 8 CPU-minutes** — one real head mint for §7.9's
...
## 15. Open issues for round 6

1. **D-1, the reference arm (§5.2.1).** The one defect no review round found, and which three
   rounds affirmatively verified in its wrong form. Round 6 should check it from the source —
   C01's two `fix_break` sites at `:1725` and `:2702-2714`, and the executed `net_fixes.reference`
   values `common` / `endpoint_concat` — and rule whether inheriting C01's accuracy-based selector
   is pre-registration-safe (the argument is that it is a deterministic *rule*, not a researcher
   choice, so it introduces no degree of freedom even though the arm it selects is unknowable in
   advance).
2. **The dataset axis (§5.6).** Does the conservative reading close C-1 completely — is there any
   remaining path on which a lineage clean on one dataset only reaches a verdict? And does the
   `(dataset, lineage)` cross appear everywhere it must: §5.2, §5.3, §6's scope column, §10.2,
   §13 item 20?
3. **The frozen 92-family (§5.5).** The invariance is measured, but round 6 should re-derive it
   and rule whether padding untested hypotheses at `p = 1` is the right convention, or whether
   recomputing the family at 46 would be more honest despite being gameable in principle.
4. **S7's head-space statistic (§5.2.2).** A **declared departure** from C01's
   `min(‖d_img‖, ‖d_text‖)`, forced by the head emitting one fused block. Is the departure the
   right one, and does the arena-is-train-split structural difference — the small set is the
   bottom decile of the scored population by construction — need more than a recorded note?
5. **`GATE-FLOOR`'s discharge (§7.9).** One cell of 30 was measured and matched exactly. Is one
   exact match sufficient evidence for a global gate demanding 42 banked quantities at 4 dp, or
   should more cells be checked before freeze?
6. **The enumeration (§8).** Five rounds have each found one uncounted loop. Hunt again, and rule
   whether Phase 7's *"sub-`0.1 s` class"* bucket is still an honest home for the items now in
   it.

---

*No GPU, SLURM, Modal, teacher call, cache write, test-split access, job submission or commit
occurred in producing this document. One CPU head mint was trained on the login node for §7.9's
`GATE-FLOOR` discharge; it wrote nothing outside the session scratchpad and computed no
battery-arm accuracy. Login-node dry-check processes only (§7.8). `TARGET_STATE.json` was read and
not modified. v1–v5 are unmodified.*
