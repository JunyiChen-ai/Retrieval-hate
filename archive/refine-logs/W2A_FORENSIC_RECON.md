...
**r1 amendment note (2026-07-15):** the independent pre-registration review (`refine-logs/W2A_PREREG_REVIEW.md`) verified both load-bearing findings TRUE and returned APPROVED-WITH-AMENDMENTS / GO. §5 (control gates) and §8 (recommendation) below are updated in place for the applied amendments (Z_best binding conditional-info baseline; kNN CONCAT demoted to advisory; cross-video placebo; τ_live/empty-branch demoted to diagnostic; vision-pad contiguity assert). The HEADLINE + §1–§4/§6–§7 findings are unchanged.
...
2. **Grid gate + vision-pad mask assert (MANDATORY, HALT; r1 Amdt 7).** `n_vis == grid_t·(grid_h//2)·(grid_w//2)` from the model's own `video_grid_thw` + `spatial_merge_size=2` (S2S A1), in **both** forwards; PLUS assert the vision-pad positions (`input_ids==video_token_id`) are a **single contiguous block** of that count, with identical mask logic in both forwards — because G-recon-IMG validates the *prefix* span, not the vision-pad pool that actually produces `grd`/`ungrd_vis`, so the pool span is pinned independently.
3. **Grounding-LIVE positive control (MANDATORY — the analog of S2S's temporal control; r1 Amdt 4/6).** The **BINDING no-op VOID** is a fixed pre-declared numeric: **present-transcript-set median** `cos(grd, ungrd_vis) ≥ 0.999` → silent no-op → **probe VOID** (the exact trap §3.2 warns about). The per-video `cos < τ_live` check and the **empty-transcript branch** (expecting `cos ≈ 1`) are **demoted to logged diagnostics, NOT HALT gates**: τ_live self-calibrates (cannot fail on a weak-but-nonzero effect), and the empty branch is confounded by the M-RoPE position-shift + attention to the `"(none)"` block, so exact ≥0.999 is not guaranteed. The binding content check is the placebo (item 4).
4. **Placebo-transcript control (MANDATORY, subset ≥50 videos; r1 Amdt 3).** PRIMARY: `grd` recomputed with a **cross-video MISMATCHED** transcript (video *j*'s transcript on video *i*'s frames, length-comparable partner) must differ from true-`grd` (`cos < 0.999`) — matching the probe's cross-video permutation null, isolating transcript *content* from *position/length*. SECONDARY diagnostic: the within-video token-shuffle (order only). The extraction-time analog of the permutation null.
...
2. **The redundancy arm is the entire verdict — and the prereg review (r1) tightened its baseline.** The binding test is now the weighting-invariant **C3-template conditional-info probe with `Z_best` = concat(CLIP img+text, Qwen img+text) (8960-d)**, +0.040 triple rule (r1 Amdt 1) — a grounded key that beats Qwen-only concat but is **CLIP-encoder-redundant** (dies against `Z_best`) is DEAD, the exact C3-nontarget lesson. The kNN `GROUNDED − CONCAT` comparison is retained but **DEMOTED TO ADVISORY** (r1 Amdt 2, option (b)): the fixed-50/50 kNN CONCAT geometry can false-PASS, so it corroborates but does not gate. (See `exp-w2a-grounded.md` §5/§6, `refine-logs/W2A_PREREG_REVIEW.md`.)
