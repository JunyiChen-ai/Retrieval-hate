...
> **⚠ Multi-seed correction (2026-07-26, F88 — `refine-logs/ERRPAT_MHC-EN_2026-07-26.md` §6.5,
> commit `ad56a62`), binding on how this whole document is read.** The manual repair this experiment
> tries to automate is **single-seed**: an exact replay of the same deletion on all four deployed seeds'
> banked top-60 neighbour lists gives **+0.0124 on seed 0 and exactly zero vote flips on seeds 1, 2 and
> 3** (four-seed mean **+0.0031**), and the two items seed 0 flips are low-margin false positives from
> the seed-flip noise band, not hard errors. The 14-id rule-hit list is strictly stronger
> (**+0.0093 acc / +0.0089 mF1, 3/4 seeds, 6 fixed / 0 broken**) but sub-bar and now test-consumed.
> This does **not** change any verdict below — the AND rule's C − A = +0.0000 stands, and so does the
> guard-rail finding — but it does resize the *target*: what the automatic rule failed to reproduce is a
> single-seed, two-video effect. Cite as **"human-in-the-loop capability demonstration, single-seed; not
> an accuracy claim."**
...
manual gain was a **human-precision, 2-entry surgical edit** (and, per the F88 correction at the
top of this document, a **single-seed** one worth 2 videos on seed 0 and nothing on seeds 1–3),
and the semantic vote at full
...
band, so we make no accuracy claim for it. The manual editability demo stands **as a capability
demonstration (single-seed; F88 correction above)**; automating it with this specific two-vote rule
does not close the human-in-the-loop gap.
