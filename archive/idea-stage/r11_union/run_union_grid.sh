#!/usr/bin/env bash
# R11-UNION -- fork of idea-stage/r10_combo/run_combo_grid.sh.
...
#   ARMSPEC entry = TAG:MODEL:FUSION:TEACHER:LAMBDA
...
for DS in "${DSS[@]}"; do
  for SPEC in "${ARMSPECS[@]}"; do
    IFS=':' read -r ARM MODEL FUSION TEACHER LAMBDA <<< "$SPEC"
    FUSION="${FUSION:-align}"
    TEACHER="${TEACHER:--}"
    LAMBDA="${LAMBDA:-0}"
    ANCHOR_ARGS=()
