...
|---|---|---|---|
| 0 | 初版（1.1–1.3）；2026-09-08 06:15 开跑（trial 0 曾因长视频反事实前向 OOM 失败，改按视频长度分块后重跑） | HCS（best trial 7，`runs/20260908_adaptive_vlm_query/hateclipseg/seed234/`）：12 次 .6793 / .6816 / .5436；τ_val = .005 → 11.8 次 .6843 / .6817；同模型 34 次 .6936 / .6924，4 次 .6395 / .6343；uniform 12 次 .6727 / .6782。HateMM：搜索中。 | HCS：过规则 8；E2 未过（AP 差 .015，ROC 差 .003）；E3 未达（+.007 / +.004）。训练级对照（`ablations/hateclipseg/seed234/`，12 次 AP / ROC）：no_missing_state .6646 / .6663，train34 .6634 / .6637，no_dropout .6442 / .6309，round0_only .6560 / .6558，coarse4_train（4 次）.6050 / .5913 → 四个部件在 HCS 都成立（≥ .015；策略观测集上重训 +.023）；full 模型只看 4 粗块 .6395 也高于只用粗块训练的 .6050；同模型 34 次 = 修订 4 seed 234（.6936），损失来自裁定数量。缓存裁定只有 0/3 两档，无法用分级信息补。HateMM 出来后定第 1 轮改动。 |
