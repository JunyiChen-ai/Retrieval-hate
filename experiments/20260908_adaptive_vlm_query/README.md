...
| 1 | 训练期策略起点改为只从 4 粗块起跑（`policy_start=coarse`，与测试期同起点；round-1 前缀 = 策略原始 pick 顺序；训练期调用仍 = 8 + seed 集外的新 pick）。改动前预注册（2026-09-08）：改动理由是第 0 轮训练期策略从 seed 窗起跑而测试期从粗块起跑，round-1 的前缀分布与测试轨迹起点不一致；预期 HCS（策略有效，E3 +.011）小幅上升，HateMM（策略无效）不变。输出 `runs/20260908_adaptive_vlm_query_it1/`；规则 6 复核 `REVIEW_RULE6_IT1.md`（一处 must-fix：配置缺省回退写法无效，已改为 DEFAULTS 显式记录并拒绝未知键）。工程记录：两语料 seed 234 于 uoa-lab3 14:43 开跑；同日 lab1 两个 HateMM 第 0 轮搜索同跑时 EOC 反事实前向 CUDA OOM（单进程缓存占 24 GB），分块预算 3e7 → 1e7 并逐视频释放缓存（commit 200db33，只改内存不改数值），lab1 两 seed 重启，lab3 搜索中途拉取该 commit（trial 2 起）。 | HateMM（best trial 11，`runs/20260908_adaptive_vlm_query_it1/hatemm/seed234/`）：12 次 .6532 / .8419 / within .6089；τ_val = 0（12 次）；同模型 34 次 .6355 / .8346，4 次 .6532 / .8379；uniform 12 次 .6440 / .8339；训练期 15.3 次/视频。HCS（best trial 12）：12 次 .6854 / .6844；τ_val = .01 → 11.25 次 .6969 / .6841；同模型 34 次 .6982 / .6937，4 次 .6367 / .6305；uniform 12 次 .6768 / .6784；训练期 15.3 次。HateMM 三 seed（2025 best 11、3407 best 11；`runs/20260908_adaptive_vlm_query_it1/hatemm/seed*/`）：12 次 .6532 / .6785 / .6706，均值 .6674 ± .0130，ROC .8419 / .8554 / .8489，均值 .8487；uniform 12 次均值 .6587 / .8444；同模型 34 次均值 .6526 / .8455，4 次均值 .6649 / .8459；τ_val = 0 / .08 / 0（seed 2025 停在 4.0 次 .6759 / .8559），停止点均值 9.3 次 .6666 / .8489；训练期 15.3–15.4 次。HCS 三 seed（2025 best 17、3407 best 15）：τ = 0（12 次）.6854 / .6926 / .6929，均值 .6903 ± .0043，ROC 均值 .6873；validation 选的停止点（τ_val .01 / .005 / .01，平均 11.5 次）.6969 / .6983 / .6997，均值 .6983，ROC .6841 / .6913 / .6860，均值 .6871；同模型 34 次均值 .6995 / .6939；uniform 12 次均值 .6801 / .6825；训练期 15.2–15.3 次。 | HateMM seed 234 过 E2（AP +.0075、ROC +.003，都在 ±.01 内）；E3 差 .001（+.009 / +.008）。HCS seed 234 τ = 0 点 AP 差 .009，τ_val 点 AP 过、ROC 差 .0002。逐 trial 配对（同 sampler seed，前 10 个 trial 超参相同）：it1 − it0 均值 HateMM +.004 AP / +.002 ROC（10/20 胜），HCS +.003 / +.002（11/20 胜）；即改动本身是小幅正向，best-trial 的提升含搜索波动。HCS 三 seed 判定（2026-09-09 06:40）：固定预算点（τ = 0，12 次）AP .6903 对下限 .6944 差 .004、ROC 过；E3 过（比 uniform +.010 AP）；**validation 选的停止点（平均 11.5 次）AP .6983、ROC .6871，两项都过 E2 下限**，且比固定 12 次少问 0.5 次、AP 高 .008 —— 停止规则在 HCS 上有用（三 seed 每个 seed 都是停止点 AP 更高）。两个操作点都是搜索前预注册的（第 2 节），τ 只用 validation 选；哪一个作主操作点写论文由用户裁定：按固定预算点 HCS 差 .004（在 seed std .004–.008 内），按停止点两语料候选都过。按 2.4 进入三 seed：HCS 2025/3407（lab3，22:53 起）、HateMM 2025（lab3，23:00 起）、HateMM 3407（lab1，第 0 轮两 seed 跑完后接上）。第 0 轮 HateMM seed 3407 只有 19 个有效 trial（trial 0 因两搜索同时启动 OOM），第 0 轮只作记录不再补跑。HateMM 三 seed 判定（2026-09-09 09:10）：**E1 过、E2 过**（AP .6674 ≥ .6457，+.022；ROC .8487 ≥ .8389，+.010）；E3 未达（比 uniform +.009 AP / +.004 ROC，差 .001）；三 seed 全部落在修订 4 三 seed 均值（.6630 / .8486）附近，即 12 次调用达到 34 次的修订 4 水平。第 1 轮汇总：HateMM 两操作点都过 E2；HCS 停止点过、固定 12 次点 AP 差 .004。HCS 第 1 轮训练级对照三 seed（`runs/20260908_adaptive_vlm_query_it1/ablations/hateclipseg/seed*/`，各 seed best-trial 超参，12 次 AP / ROC 均值；full − arm）：no_missing_state .6829 / .6774（+.007 / +.010），no_dropout .6632 / .6498（+.027 / +.037），train34 .6699 / .6609（+.020 / +.026），round0_only .6699 / .6775（+.020 / +.010），coarse4_train（4 次）.6155 / .6056（+.075 / +.082）。HateMM 第 1 轮训练级对照三 seed（`runs/20260908_adaptive_vlm_query_it1/ablations/hatemm/seed*/`，seed 234/3407 在 uoa-lab1、2025 在 uoa-lab3；12 次 AP / ROC 均值；full − arm）：no_missing_state .6485 / .8425（+.019 / +.006），no_dropout .6002 / .8221（+.067 / +.027），train34 .6435 / .8459（+.024 / +.003），round0_only .6412 / .8460（+.026 / +.003），coarse4_train（4 次）.6230 / .8425（+.044 / +.006）。HateMM 上五个部件在 AP 都 ≥ .019；ROC 上只有 no_dropout、coarse4_train ≥ .01（其余 +.003–.006，HateMM ROC 本身各策略各调用数都在 .844–.851 内）。汇总见第 6 节。与第 0 轮的差别：no_missing_state 的 AP 差距从 +.014 降到 +.007，低于 .01 门，ROC 恰 +.0099；即改为同起点后显式缺失状态在 HCS 上不再按规则 14(g) 可主张，其余四个部件仍过。 |

## 6. 第 1 轮结论（2026-09-09 10:30，三 seed 234/2025/3407，两语料；来源 `runs/20260908_adaptive_vlm_query_it1/<corpus>/seed*/trial<best>/summary.json` 与 `.../ablations/`）

**终止判定。** E1（测试期平均调用 ≤ 12）两语料过。E2 按操作点分：
- 固定预算点（τ = 0，12 次）：HateMM .6674 / .8487（下限 .6457 / .8389，过）；HCS .6903 / .6873（下限 .6944 / .6843，AP 差 .004，ROC 过）。
- validation 选的停止点（τ_val，第 2 节规则）：HateMM 平均 9.3 次 .6666 / .8489（过）；HCS 平均 11.5 次 .6983 / .6871（过）。
- E3（同调用数比 uniform 高 ≥ .01 AP）：HCS 过（+.010），HateMM 差 .001（+.009）。

即按停止点两语料 E1、E2 都过，本阶段目标达成；按固定 12 次点 HCS AP 差 .004（在 seed std .004–.008 内）。主操作点由用户裁定；论文写法建议以停止点为主（自适应查询本来就是逐视频可变调用数，平均调用数 ≤ 12 是操作点定义），固定 12 次点作为曲线上的一个读数并列报告。参考：修订 4 用全 34 条裁定的三 seed 为 HateMM .6630 / .8486、HCS .7024 / .6972；本模块 12 次 = 修订 4 34 次的 HateMM 水平、HCS 差 .004–.012。

**调用数曲线（三 seed 均值 AP，六种策略，同一模型；34 = 全部裁定）。**

| 语料 | 策略 | 4 | 6 | 8 | 12 | 16 | 22 | 34 |
|---|---|---|---|---|---|---|---|---|
| HateMM | eoc | .6649 | .6675 | .6687 | .6674 | .6624 | .6562 | .6526 |
| HateMM | uniform | .6649 | .6616 | .6612 | .6587 | .6549 | .6517 | .6526 |
| HateMM | conflict | .6649 | .6606 | .6603 | .6566 | .6617 | .6626 | .6526 |
| HateMM | entropy | .6649 | .6684 | .6699 | .6702 | .6592 | .6542 | .6526 |
| HateMM | localization | .6649 | .6633 | .6614 | .6598 | .6523 | .6553 | .6526 |
| HateMM | random | .6649 | .6685 | .6660 | .6655 | .6577 | .6553 | .6526 |
| HCS | eoc | .6456 | .6600 | .6754 | .6903 | .6957 | .6968 | .6995 |
| HCS | uniform | .6456 | .6555 | .6668 | .6801 | .6892 | .6948 | .6995 |
| HCS | conflict | .6456 | .6599 | .6718 | .6826 | .6924 | .6984 | .6995 |
| HCS | entropy | .6456 | .6487 | .6564 | .6730 | .6809 | .6855 | .6995 |
| HCS | localization | .6456 | .6679 | .6781 | .6880 | .6914 | .6938 | .6995 |
| HCS | random | .6456 | .6648 | .6702 | .6732 | .6820 | .6886 | .6995 |

ROC 同表：HateMM 各策略各调用数都在 .8429–.8507（eoc 12 次 .8487，uniform .8444）；HCS eoc 12 次 .6873，uniform .6825，conflict .6796，entropy .6687，localization .6770，random .6694，34 次 .6939。读法：HCS 上曲线随调用数单调上升，eoc 在 8–16 次都是最高或并列最高，12 次比 uniform 高 .010，比 conflict（无反事实前向、无 HMM 加权）高 .008；HateMM 上曲线平（4 次 = 34 次 ≈ .65–.67，dropout 训练后的模型对细窗裁定不敏感），eoc 12 次比 uniform 高 .009，entropy 与 eoc 相当。

**七臂表（三 seed 均值 full − arm，AP / ROC；规则 14(g) 两语料 ≥ .01 才可主张）。**

| arm | HateMM | HCS | 可主张 |
|---|---|---|---|
| uniform_at_matched（选窗） | +.009 / +.004 | +.010 / +.005 | AP：HCS 过、HateMM 差 .001；ROC 两语料不过 |
| no_stop（停止规则；τ_val 点 − τ = 0 点，调用数） | −.001 / +.000（9.3 vs 12 次） | +.008 / −.000（11.5 vs 12 次） | 停止规则减少调用而不降指标（HateMM 省 2.7 次，HCS 省 0.5 次且 AP +.008）；按第 2 节，可主张"少问不降"，不主张涨点 |
| conflict（反事实前向 + HMM 加权） | +.011 / +.002 | +.008 / +.008 | AP 各差一语料（HateMM 过、HCS 差 .002）；不可主张 |
| no_missing_state | +.019 / +.006 | +.007 / +.010 | AP：HateMM 过、HCS 差 .003；不可主张（第 0 轮 HCS +.014 可主张） |
| no_dropout（vs train34：HateMM +.043 / +.024，HCS +.007 / +.011） | +.067 / +.027 | +.027 / +.037 | vs full 两语料 AP、ROC 都过；vs train34 HateMM 过、HCS AP 差 .003 |
| train34 | +.024 / +.003 | +.020 / +.026 | AP 两语料过；ROC HateMM 不过 |
| round0_only | +.026 / +.003 | +.020 / +.010 | AP 两语料过；ROC HateMM 不过 |
| coarse4_train | +.044 / +.006 | +.075 / +.082 | AP 两语料过；ROC HateMM 不过 |

按 AP（SOTA 比较的主指标）两语料 ≥ .01 成立的部件：no_dropout、train34、round0_only、coarse4_train 四个；ROC 上只有 no_dropout 两语料成立（HateMM ROC 对任何训练方案都在 .82–.85，区分度低）。选窗（uniform）、反事实前向（conflict）、显式缺失状态三项在 HCS 或 HateMM 单侧差 .001–.003，不可主张，论文按"与 uniform 相当或略高"写。

**训练期调用。** 两语料 15.2–15.4 次/视频（8 次 round 0 + 约 7.3 次策略 pick），比全 34 少 55%；测试期 12 次或停止点 9.3 / 11.5 次，比 34 少 65–73%。

**去向。** 达成停止点下的 E1 + E2；HCS 固定 12 次点差 .004 未达。如用户要求固定预算点也过，第 2 轮候选（单项）：(a) 在 validation 上按"平均调用 ≤ 12"选 (cap, τ)（第 2 节已预注册的停止规则推广到预算约束）；(b) 一致性正则：训练期让 12 次视图的输出逼近同模型 34 次视图（自蒸馏，不加新调用）。
