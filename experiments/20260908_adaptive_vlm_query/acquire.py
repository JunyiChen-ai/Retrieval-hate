"""Acquisition policies over the cached VLM verdicts, scored through a trained
backbone (adaptive VLM query module, 2026-09-08). Simulation only: revealing a
window = un-masking its cached verdict; 0 new VLM calls.
...
with MISSING (-1) entries. Every policy reveals one fine window per step:
  eoc          expected output change of the backbone (two counterfactual
               forward passes per candidate window, weighted by the interval
               HMM's predictive probability of the verdict)
  conflict     |mean over the window of sigmoid(content logit) - P(h_w | E)|
               (backbone vs HMM disagreement, no counterfactual forward)
...
the full per-second logit), so one greedy run gives the whole budget curve;
for eoc the max gain at each step is kept so any stopping threshold tau can be
...
class Acquirer:
    def __init__(self, model, hmm, cache, corpus, binary, device):
        self.model = model
        self.hmm = hmm
        self.cache = cache
        self.corpus = corpus
    ...
    def forward(self, f_v5, fa_list):
        ...
        for i in range(0, len(fa_list), rows):
            ...
            csig = torch.sigmoid(self.model.last_content_logit.squeeze(-1)).view(n, align.N_CROPS, -1).mean(1)
            outs.append(sig.cpu().numpy())
            couts.append(csig.cpu().numpy())
        return np.concatenate(outs, 0), np.concatenate(couts, 0)
    ...
    def run_video(self, vid, policy, n_steps, rng, initial=None):
        ...
        for _ in range(n_steps):
            ...
            if policy in ("uniform", "random"):
                ...
                pred = self.hmm.predictive_fine(g)
                ...
                for c in cands:
                    ...
                    for b, pb in ((1, pred[c]), (0, 1 - pred[c])):
                        ...
                        bf2 = bf.copy()
                        bf2[c] = b
                        p2 = self.p_s(self.gamma(bf2, bc))
                        exp_u += pb * float(np.sum(p2 * (1 - p2)))
                    if u_now - exp_u > best:
                        best, w = u_now - exp_u, c
            elif policy == "conflict":
                _, p_hf, _ = self.hmm.summarize_gamma(self.gamma(bf, bc))
                cur_c = csig[0]
                u = []
                for c in cands:
                ...
                sc, _ = self.forward(f_v5, fa_list)
