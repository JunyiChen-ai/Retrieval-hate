...
离线验证（不训练，裁定单独作为分数；`scratchpad/hier_hmm.py`，参数只用 train 视频标签）：

| 裁定单独分数 | HateMM test AP / ROC / within | HateMM val | HateClipSeg test | HateClipSeg val |
|---|---|---|---|---|
| 两粒度平均等级（修订 4 先验） | .500 / .801 / .572 | .444 / .705 / .503 | .628 / .634 / .526 | .618 / .713 / .568 |
| 分层证据 HMM 后验 | .546 / .819 / .572 | .485 / .742 / .533 | .699 / .661 / .554 | .727 / .757 / .597 |
| 同模型只用 K30 | .522 / .790 / .547 | .465 / .713 / .492 | .698 / .662 / .563 | .709 / .727 / .586 |
| 同模型只用 K4 | .591 / .851 / .666 | .504 / .740 / .595 | .671 / .625 / .475 | .673 / .747 / .550 |
| 对照：裁定简单邻域平滑（w=2）+ K4 | .536 / .820 / .580 | — | .657 / .653 / .542 | — |

两语料 test 与 val 同向提升；HateMM 上模型只用 K4 更好（EM 把 K30 的可靠性估高：test 上 K30 判仇恨而 K4 判非仇恨的秒阳性率只有 .158），这是要如实报告的模型误设，处理方式是把 K30 证据温度 w30 ∈ [0,1] 作为标量超参数进搜索（规则 13 允许）。估出的参数：HateMM q30 .955 / r30 .088 / q4 .975 / r4 .233，A 对角 .958/.924，p0(s=0) .649；HateClipSeg q30 .858 / r30 .104 / q4 .920 / r4 .323，A 对角 .985/.935，p0(s=0) .818。
...
## 4. 进度
- 2026-09-03：提案；离线验证完成（第 1 节表）；prior_only 消融在 lab1/lab3 运行中。
