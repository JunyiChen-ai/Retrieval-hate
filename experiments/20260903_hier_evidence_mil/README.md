...
HateMM（uoa-lab3，2026-09-04）：

| 去掉 | seed 234 / 2025 / 3407 AP/ROC | 三 seed 均值 AP / ROC / within | 对 full（.657/.842/.646） | AP 下降 seed 数 |
|---|---|---|---|---|
| no_ema（MACIL-SD 参数 EMA 自蒸馏） | .648/.837 .619/.836 .647/.840 | .638 / .838 / .646 | -0.020 / -0.005 / -0.001 | 3/3 |
| no_cmal（MACIL-SD 跨模态对比损失） | .576/.807 .623/.823 .602/.806 | .600 / .812 / .621 | -0.057 / -0.030 / -0.025 | 3/3 |
| no_text（BERT 文本列置零） | .693/.871 .644/.871 .667/.865 | .668 / .869 / .610 | +0.010 / +0.027 / -0.036 | 1/3 |
| no_visual（I3D 置零） | .624/.843 .577/.826 .619/.844 | .607 / .838 / .614 | -0.051 / -0.005 / -0.033 | 3/3 |
| no_audio（VGGish 置零） | .586/.814 .620/.816 .627/.819 | .611 / .816 / .614 | -0.046 / -0.026 / -0.033 | 3/3 |
| no_ps（输入去 P(s) 列，留 ℓ） | .607/.821 .631/.828 .616/.826 | .618 / .825 / .634 | -0.040 / -0.018 / -0.012 | 3/3 |
| no_hmm_input（输入去 ℓ、P(s) 两列，留原始两列） | .654/.838 .644/.831 .605/.816 | .634 / .829 / .634 | -0.023 / -0.014 / -0.012 | 2/3 |

读法（HateMM）：音频、视觉、CMAL 各贡献约 .05 AP（HateClipSeg 上几乎为 0）；EMA 自蒸馏 −.020 AP（3/3，HateClipSeg ≈ 0）；文本去掉后 pooled 反而升（+.010/+.027）但 within 掉 .036（HateMM within 下限是主要约束，文本两语料统一保留）；P(s) 列在 HateMM 上有 .040（HateClipSeg 冗余），HMM 两列合起来 −.023（2/3）。
...
读法：HateClipSeg 上 AVCE 结构里能确认的只有"共享一层跨模态注意力"值 .01–.02 ROC（与 CMAL 对比同量级）；自注意力没有用（不如不要注意力）；注意力层整体的贡献 ≤ .012 ROC。**骨干在 HateClipSeg 上几乎是"投影 + 头"在工作，时间建模贡献很小**，与 9.3（z 方差 79% 在视频之间）一致。

**HateMM 结构臂结果（2026-09-04 09:36，同格式）**：

| 臂 | 三 seed 均值 | 差 | 说明 |
|---|---|---|---|
| full | .657 / .842 / .646 | | |
| self_attn | .623 / .833 / .613 | −.035 / −.009 / −.034（3/3、2/3） | 换自注意力：AP、within 三 seed 都明显降 |
| no_attn | .603 / .806 / .643 | −.054 / −.037 / −.004（3/3、3/3） | 去掉注意力：pooled 大降，within 不变 |
| unshared_cma | .620 / .837 / .622 | −.038 / −.005 / −.024（3/3、1/3） | 各方向一层：AP 与 within 降，ROC 基本不变 |
| 参照 no_ema（9.6） | .659 / .844 / .646 | +.002 / +.002 / .000 | |
| 参照 no_cmal（9.6） | .636 / .834 / .636 | −.021 / −.008 / −.010 | |

读法：HateMM 与 HateClipSeg 相反，注意力层贡献很大（no_attn −.054 AP / −.037 ROC，三 seed 全降），而且必须是**跨模态**（自注意力只挽回 .020 AP）且**共享**（不共享 −.038 AP，只有 ROC 持平）。within 三臂里只有 no_attn 不动：注意力层带来的是 pooled 增益（跨视频的分数尺度校准），不是视频内排序。两语料合起来：共享跨模态注意力是 AVCE 里唯一在两语料都确认的结构部件（HateClipSeg −.010/−.021、HateMM −.035/−.009，self_attn 臂 pooled 3/3 + 3/3 与 3/3 + 2/3）。

**链蒸馏臂 HateClipSeg 结果（2026-09-04 09:25）**：

| 臂 | 三 seed 均值 | 差 | 逐 seed（AP/ROC/within 差） |
|---|---|---|---|
| chain_distill | .682 / .666 / .543 | −.017 / −.015 / −.010（3/3、2/3） | 234 −.008/+.001/−.012；2025 −.033/−.042/−.016；3407 −.009/−.005/−.002 |
| chain_distill_ema | .693 / .673 / .553 | −.006 / −.008 / +.001（2/3、1/3） | 234 −.001/+.004/−.003；2025 −.027/−.040/−.009；3407 +.011/+.012/+.014 |

**链蒸馏臂 HateMM 结果（2026-09-04 10:45）**：

| 臂 | 三 seed 均值 | 差 | 逐 seed（AP/ROC/within 差） |
|---|---|---|---|
| chain_distill | .619 / .817 / .627 | −.038 / −.025 / −.020（3/3、3/3） | 234 −.031/−.019/−.030；2025 −.040/−.027/−.027；3407 −.045/−.029/−.003 |
| chain_distill_ema | .608 / .812 / .625 | −.050 / −.030 / −.025（3/3、3/3） | 234 −.041/−.017/−.016；2025 −.051/−.031/−.017；3407 −.058/−.043/−.041 |

HateMM 上链蒸馏三 seed 全降，幅度比 HateClipSeg 更大。判定：预注册要求"两语料三 seed 均值不低于 full 且 within 不低于 full"，HateClipSeg 已不满足（两臂 pooled 均值都低于 full）。**链后验蒸馏在本骨干上不成立**：候选 2 里它有效是因为那个骨干没有 top-k MIL 和原始裁定列输入，蒸馏目标是唯一的定位监督；本骨干已经从 top-k MIL + 裁定列学到了超过链的视频内排序（9.6 no_ema ≈ 0 同理，任何自蒸馏类目标在这里都是冗余）。HateMM 链臂只作记录（运行中，lab1）。

**9.7 结论（骨干改动方向）**：(1) 有可主张作用的结构部件 = 共享跨模态注意力（两语料确认）+ CMAL（9.6 两语料确认）+ 裁定列输入与先验（9.6）；(2) 没用的 = EMA 自蒸馏、任何链/自蒸馏目标、自注意力；(3) 注意力的作用在 HateMM 是 pooled 校准而非视频内排序（within 不受影响），HateClipSeg 上几乎没作用。改结构的方向应当是把"跨模态注意力"这个确认有效的部件改成我们能主张的形式，同时把无效的 EMA 拿掉，不再往骨干上加新的训练目标。
...
### 8.4 修订 2 HateMM 三 seed 消融（只作记录；`runs/20260903_hier_evidence_mil_rev2/ablations/hatemm/`）

| 消融 | 均值 AP / ROC | 对 full（.634/.830） | AP 下降 seed 数 |
|---|---|---|---|
| mean_prior | .618 / .831 | −.016 / +.001 | 2/3 |
| indep_hmm | .606 / .814 | −.028 / −.016 | 3/3 |
| flat_coarse | .607 / .806 | −.027 / −.024 | 3/3 |
| no_block | .619 / .820 | −.015 / −.010 | 2/3 |
| raw_block_label | .608 / .821 | −.026 / −.008 | 2/3 |
| no_input | .613 / .811 | −.022 / −.019 | 3/3 |
| no_prior | .616 / .815 | −.019 / −.015 | 2/3 |
| no_verdict | .569 / .786 | −.065 / −.043 | 3/3 |

去掉原始列后 HMM 后验成为骨干唯一的裁定入口，时间耦合与块 OR 的消融幅度比修订 1 大（−.028 / −.027），但整体低于修订 1，淘汰结论不变。
