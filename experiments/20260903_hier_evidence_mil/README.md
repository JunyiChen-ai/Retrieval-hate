...
判定：预注册要求"两语料三 seed 均值不低于 full 且 within 不低于 full"，HateClipSeg 已不满足（两臂 pooled 均值都低于 full）。**链后验蒸馏在本骨干上不成立**：候选 2 里它有效是因为那个骨干没有 top-k MIL 和原始裁定列输入，蒸馏目标是唯一的定位监督；本骨干已经从 top-k MIL + 裁定列学到了超过链的视频内排序（9.6 no_ema ≈ 0 同理，任何自蒸馏类目标在这里都是冗余）。HateMM 链臂只作记录（运行中，lab1）。
