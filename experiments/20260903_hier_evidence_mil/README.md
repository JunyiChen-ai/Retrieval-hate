...
HateClipSeg 三 seed：.693 ± .001 / .673 ± .001 / .543 ± .003（修订 1 .699/.681/.553），在下限之内。HateMM seed 234 低于下限 .019 AP / .013 ROC，与修订 1 no_input 消融（−.052）同方向：去掉原始裁定列，HateMM 明显掉点。**按预注册第 3 条，修订 2 淘汰，论文方法回到修订 1**。结论：HMM 后验在 HateMM 上不是原始裁定的充分统计量，证据模型存在失配（与 4.9 节一致：test 上应当更不信 K30，而 EM 与 val 都给 K30 高可靠性）。HateMM seed 2025/3407 与消融链继续跑完，只作记录（`runs/20260903_hier_evidence_mil_rev2/`）。
