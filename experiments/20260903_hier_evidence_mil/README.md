...
读法（HateMM）：音频、视觉、CMAL 各贡献约 .05 AP（HateClipSeg 上几乎为 0）；EMA 自蒸馏 −.019 AP（2/3）；文本去掉后 pooled 反而升（+.017/+.031）但 within 掉 .034（HateMM within 下限是主要约束，文本两语料统一保留）；P(s) 列在 HateMM 上有 .040（HateClipSeg 冗余），HMM 两列合起来 ≈ 0（1/3）——单列消融在这个语料上噪声大。
