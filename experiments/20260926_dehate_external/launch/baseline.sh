#!/usr/bin/env bash
# One baseline on DeHate (README section 2): the 5-trial validation search at seed 234 (user, 2026-09-26; 40 on
# HateMM / HateClipSeg), then the three-seed
...
if $PY -u scripts/reproduction_baselines/tune_official_val.py --method "$m" --corpus dehate --trials 5 \
       --root "$B/tuning" \
