...
### 8.2 随机数流重复：三臂各 5 次（HateClipSeg seed 234，trial 15 超参；`diag/hateclipseg/seed234/stream_noise/<arm>_burn<k>/metrics.json`，2026-09-05 06:30）

| 臂 | 原单次 | 流 1 | 流 2 | 流 3 | 流 4 | 5 次均值（std） |
|---|---|---|---|---|---|---|
| full | .702/.687/.550 | .688/.679/.553 | .694/.692/.555 | .701/.696/.543 | .695/.681/.553 | .696 / .687 / .551（.006/.007） |
| no_token_unmasked（候选 1 骨干） | .672/.660/.548 | .676/.672/.541 | .685/.678/.553 | .696/.689/.536 | .681/.671/.530 | .682 / .674 / .542（.009/.011） |
| no_token_masked | .673/.661/.548 | .677/.672/.543 | .686/.680/.552 | .695/.688/.535 | .681/.670/.528 | .682 / .674 / .541（.009/.010） |

读数：
1. **单次换臂比较的噪声约 ±.01**（同臂同超参只换随机数流，AP std .006–.009，极差 .013–.024）。搜索选出的 full 单次 .702 比同臂 5 次均值高 .006（test 选 best trial 的选择偏差）；对照臂原单次 .672 比其 5 次均值低 .010。原来记录的 +.030 里有 .016 是这两种噪声。
2. 去掉噪声后空 token 在候选 4 训练设置里的作用：**+.014 AP / +.013 ROC**（5 次均值差；两臂 std ≈ .008，n=5，差值标准误约 .005）。作用是真的，但只有单次读数的一半，并且（8.1）测试时空 token 只拿 1/T 的注意力，作用机制不是"注意力放到空 token 上"，具体发生在训练过程中的什么环节未查明。
3. masked 与 unmasked 5 次均值相同：HateClipSeg 上 padding 屏蔽无作用，确认。
