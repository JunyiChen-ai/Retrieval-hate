"""One training trial of candidate 3 revision 3 (evidence-routed cross-modal
attention + interval evidence HMM). Training = revision 2: video-level top-k
MIL on z~ = z + c(E) + prior_scale * ell / ELL_SCALE, MACIL-SD's cross-modal
contrastive loss CMAL on a_out / v_out (content only), the verdict-block MIL
on the content logit z (no calibration, no prior), checkpoint on the official
...
positive_constraint; gate A1 of the README fixed these before the search).
    ...
    python experiments/20260907_c3_rev3_interval_evidence/train.py \
        --corpus hatemm --seed 234 --out-dir runs/.../trial0 --config cfg.json

--ablation (README section 3):
  structure  full | avce | no_qk_enc | no_cell | no_bias | key_bias | shared_bias | no_context | ctx_in_rep
