...
| no_cell（四格嵌入换成四列线性映射） | .648 / .837 / .635 | .020 / .014（3/3） | .015 / .027（2/3、3/3） | 是 |
...
3. HateMM seed 3407 的 no_bias / scalar_bias 是 −.003 / .000（不降），另两 seed 降 .03 以上，均值 .020 / .025；HCS seed 234 上 7 个臂 AP 不降（.001–.004 的反向差）但另两 seed 都降 ≥ .01（外部审稿核对后修正，原写 8）。按 09-06 裁定只看均值。
...
Brief `REVIEW_NOVELTY_REQUEST.md`，审稿原文 `REVIEW_NOVELTY_GPT6ASTRA.md`（GPT6-Astra，reasoning ultra，只读核对了 78 个 metrics.json）。结论：拒稿 4/10（上一版候选 1 精简版 3/10）。回答了的旧批评：骨干不再是 MACIL-SD 原样，同证据 avce 对照成立。没回答的：融合仍是 Snorkel/Dugong/CHMM 一类标签模型；HCS 上 HMM-only（.698/.661）已到 full 的 AP；30/4 固定网格；"内容表示始终纯内容"的机制表述与代码不符（c 加进两个流后才进 CMAL 和块 MIL，最终 logit 含视频级证据项）。审稿人的两处事实修正已并入 8.1。四个改进方向见审稿文第 4 节，排序：① 不训练的机制检验（打乱证据时间对应、视频标量对照）；② 隔离逐头偏置与四格交互项的对照臂；③ 自适应查询（先粗块后按定位不确定性选细窗，未观测裁定要边缘化）；④ 真实区间连续时间 HMM 替代索引层次。
