...
## 12. HateMM v2 三 seed 确认（2026-09-06 16:20，lab1，seed 2025/3407 并行；`runs/20260906_hier_evidence_clean_v2/hatemm/seed<seed>/`）

| seed | best trial（epoch） | AP / ROC / within | α / λ_block / a2b / a2n / cof / lr | validation 选 trial 的 test |
|---|---|---|---|---|
| 234 | 1（8） | .633476 / .839300 / .618039 | 1.21 / .272 / 1.72 / 1.94 / .020 / 6.3e-4 | trial 17 .5979/.8285/.6168 |
| 2025 | 5（8） | .629419 / .832696 / .630482 | .83 / .255 / 1.51 / 1.48 / .051 / 1e-4 | trial 10 .6091/.8207/.6391 |
| 3407 | 6（6） | .634278 / .833758 / .626328 | 3.37 / .124 / 1.37 / 1.94 / .076 / 1e-3 | trial 8 .6256/.8354/.6364 |
| **均值 ± std** | | **.6324 ± .0026 / .8353 ± .0035 / .6249 ± .0063** | | |

每 seed 20 trial 全部完整、无剪枝。规则 8 确认：门 .573 ± .033 / .807 ± .019（MACIL-SD 三 seed），要求边距 ≥ max(候选 std, baseline std, .005) = .033 / .019，实际边距 .059 / .028，**通过**。within 均值 .625，低于 MultiHateLoc 参考 .632（只报告）。

对候选 1 三 seed .657 ± .013 / .842 ± .005 / .646 ± .004：**AP −.025、ROC −.007、within −.021**。AP 差超过候选 1 的标准差，是真实差距，不是噪声（三个 seed 都在 .629–.634，std .003）。预注册第 2 条（AP ≥ .644）在 HateMM **不成立**；第 3 条查过的 lamda_cof 不是原因（第 8 节）。剩下的候选 1 与本版的差别只有两个：

1. **w_fine 固定为 1**。候选 1 HateMM 三个 best trial 的 w_fine 为 .74 / .83 / .46，都小于 1；候选 1 README 4.9 节：HateMM test 上越不信 K30 越好（HMM 后验单独 w_fine 1 → .1：AP .541 → .582），而 validation 选 w_fine = 1。候选 1 的搜索目标是 test，所以 w_fine 在 test 上被调到了合适的值；本版把它固定在 validation 选的 1。这个量是我们引入的方法标量（K30 证据的可靠性调温），加回去就是第三个方法超参数。
2. **within 剪枝**。候选 1 的 HateMM 搜索里 11–17/20 个 trial 被剪，TPE 在剩余空间里采样；本版不剪。两者 best 的 within 差 .02，方向与"剪枝把搜索推向 within 高的区域"一致，但对 pooled AP 的影响没法从现有数据分离。

两语料合看：HCS 本版高于候选 1（+.007 / +.008），HateMM 低 .025 AP。SOTA 门两语料三 seed 都过。是否为 HateMM 的 .025 AP 加回 w_fine（第三个方法标量），交用户裁定；本目录不自行加回。HateMM seed 2025（lab1）/ 3407（lab3）九臂消融 16:21 启动。

## 13. 两语料三 seed 消融合并与规则 14 核对（2026-09-06 17:10；HateMM seed 2025 在 lab1、3407 在 lab3；汇总 `runs/20260906_hier_evidence_clean_v2/ablations/three_seed_summary.json`，每臂原评测 `runs/20260906_hier_evidence_clean_v2/ablations/<corpus>/seed<seed>/<arm>/metrics.json`）

full 三 seed 均值：HateMM .6324 / .8353 / .6249；HCS .7060 / .6895 / .5649。

| 去掉 | HateMM 三 seed 均值 AP/ROC（对 full） | AP 降 seed 数 | HCS 三 seed 均值 AP/ROC（对 full） | AP 降 seed 数 | 14(g)：两语料均值降 ≥ .01 |
|---|---|---|---|---|---|
| VLM 裁定整体 no_verdict | .5261/.7772（−.106/−.058） | 3/3 | .6045/.5757（−.102/−.114） | 3/3 | 成立 |
| 先验项 no_prior | .5840/.8034（−.048/−.032） | 3/3 | .6344/.6257（−.072/−.064） | 3/3 | 成立 |
| HMM 后验换平均等级 mean_prior | .6188/.8239（−.014/−.011） | 3/3 | .6917/.6735（−.014/−.016） | 3/3 | 成立 |
| HMM 时间耦合 indep_hmm | .6071/.8171（−.025/−.018） | 3/3 | .6759/.6601（−.030/−.029） | 3/3 | 成立 |
| 块 OR 层次 flat_coarse | .6157/.8270（−.017/−.008） | 3/3 | .7047/.6906（−.001/+.001） | 2/3 | **不成立**（HCS 无差） |
| 块级 MIL no_block | .5697/.8115（−.063/−.024） | 3/3 | .6722/.6481（−.034/−.041） | 3/3 | 成立 |
| 裁定拼输入 no_input | .5989/.8065（−.034/−.029） | 3/3 | .6882/.6729（−.018/−.017） | 3/3 | 成立 |
| MACIL-SD EMA no_ema | .6202/.8253（−.012/−.010） | 3/3 | .6909/.6707（−.015/−.019） | 3/3 | 成立 |
| MACIL-SD CMAL no_cmal | .5956/.8129（−.037/−.022） | 3/3 | .6689/.6489（−.037/−.041） | 3/3 | 成立 |

读法：
- 我们加的部件（模块 3 融合：先验项、HMM 后验优于平均等级、时间耦合；模块 2 监督：块级 MIL；裁定拼输入）**两语料三 seed 全部成立**，且几乎每个 seed 都同向（唯一例外 flat_coarse）。候选 1 里 HateMM 上不稳定的 mean_prior（1/3）这里 3/3 成立，原因可能是 w_fine 固定后 HMM 后验与原始列的分工更清楚。
- 块 OR 层次在 HCS 上没有差别，与候选 1、离线验证一致，**不进主张**；模型陈述里保留分层形式，论文写明该结构无独立增益。
- 从 MACIL-SD 继承的 EMA、CMAL 两语料都有 ≥ .01 贡献，但它们是别人的部件，只作为"骨干保留原样"的依据，不作 novelty。

规则 14 逐项：
- (a) 两语料三 seed 确认全过，每 seed 数字来自该 seed 完整 20 trial 搜索的 best trial（第 9、12 节）。
- (b) 标准差已报；对门的边距 HateMM .059/.028（要求 .033/.019）、HCS .144/.161（要求 .007/.005），都大于标准差。对候选 1：HCS +.007/+.008 小于候选 1 ROC 标准差 .016，只写"持平"；HateMM −.025 AP 大于标准差，如实写"低于候选 1"。
- (c) 两语料同架构、同损失、同流程、同搜索空间；只有标量超参不同。
- (d) checkpoint 由 validation 选；v2 搜索空间、20 trial、目标定义在 v2 搜索前写入第 6 节；搜索目标为 test；validation 选 trial 的 test 数字已报（第 9、12 节）。
- (e) 无推理后处理、无 ensemble、无按语料分支。train-only 单一 teacher = 冻结 Qwen2.5-VL-7B 裁定 + 由 train 视频标签 EM 估计的 HMM；去掉 teacher 的数字已报（no_verdict）。
- (f) 骨干特征与 MACIL-SD 相同（I3D + VGGish），另加 BERT 与裁定四列。"最强 baseline + 同样特征" = no_verdict 臂（MACIL-SD + BERT，裁定全去）：HateMM .526/.777、HCS .605/.576。注意该臂用的是 full 的 best 超参，不是为它单独搜的；HateMM 上它低于 MACIL-SD 官方三 seed .573/.807，说明这组超参对无裁定的骨干不合适，不能反过来解读为"加文本有害"。
- (g) 见上表：8/9 成立，flat_coarse 不成立。
- (h) 评测器、split、GT、1 fps 未改。
- (i) 两语料三项指标全报。

**未完成项（如实）**：
1. HateMM 三 seed 比候选 1 低 .025 AP（第 12 节），候选原因 w_fine / within 剪枝，等用户裁定是否加回 w_fine。
2. 用户 09-03 三模块计划里的"骨干结构 novelty"（模块 2 要有可 claim 的新结构）仍然没有：本版骨干是 MACIL-SD 原样，我们在骨干侧的贡献只有块级 MIL 这一项监督。
3. 规则 4 novelty 复核沿用候选 1 的（7/10 GO），本版是它的子集，未重新复核。
