...
as amended 2026-09-06: no within-video pruning.

    python experiments/20260906_hier_evidence_clean/search.py \
        --corpus hatemm --seed 234 --out-root runs/20260906_hier_evidence_clean
...
Optuna's objective is the scalar (test AP + test ROC) / 2; within is recorded
only. Trial budget: after the first trial, 20 trials if it took <= 1 h, else 5;
written to budget.json and never changed. Re-running resumes the sqlite study.
"""
...
def main(argv=None):
    ...
    def objective(trial):
        ...
        t0 = time.time()
        cmd = [sys.executable, TRAIN, "--corpus", args.corpus, "--seed", str(args.seed),
               "--out-dir", out_dir, "--config", cfg_path, "--device", args.device,
        ...
        say("trial %d | %.0fs | test AP %.4f ROC %.4f within %.4f | obj %.4f"
            % (trial.number, elapsed, t["pooled_ap"], t["pooled_roc"], t["within_roc"], obj))
        return obj
    ...
    rows = [{"number": t.number, "state": str(t.state).split(".")[-1], "value": t.value,
    ...
    with open(os.path.join(root, "SEARCH_DONE"), "w") as fh:
