"""Candidate 3 revision 4 (2026-09-08): revision-2 backbone + interval evidence HMM.

Revision 3 (experiments/20260907_c3_rev3_interval_evidence) showed on three
seeds that its two backbone changes hurt HateMM (query gating -.010 AP,
calibration on the logit -.023 AP vs the revision-2 placement) while the
interval evidence HMM fusion helps on both corpora. Revision 4 therefore keeps
the revision-2 backbone exactly (per-head KEY bias, g == 1; video-level
evidence context c (hid-dim) added to both streams before the head, so it
enters CMAL and the block MIL) and keeps revision 3's evidence encoding and
fusion (interval HMM, normalized time, positive constraint, w_fine fixed 1).
The full model here equals revision 3's `key_bias` arm with `ctx_in_rep`
placement (both numerically verified against revision 2 in REVIEW_RULE6.md of
revision 3).

  A  evidence encoding e_t = Emb[cell(b_fine_t, b_coarse_t)] + W [ell_t /
     ELL_SCALE, P(s_t)], added to the attention queries and keys only;
  B  per-head key bias: score_h(i, j) += beta_h(e_j), beta_h = Linear(hid -> nhead);
  C  video-level evidence context c = Linear(hid -> hid)(mean_t e_t), added to
     v_out and a_out before the shared head (revision-2 placement).
Arms (train.py --ablation, structure; kept in the code, NOT run in revision 4
by user ruling 2026-09-08 -- only total performance is judged):
  full         A + B + C
  avce         candidate 1 backbone: verdict columns concatenated into the audio stream
  no_qk_enc    e_t not added to q/k
  no_cell      Emb[cell] replaced by a linear map of all four columns
  no_lin       e_t = Emb[cell] only (no linear map of ell, P(s))
  no_bias      no evidence bias
  gated_bias   revision-3 query gating g_h(e_i) * beta_h(e_j)
  shared_bias  beta = Linear(hid -> 1) shared by all heads
  no_context   no C
  ctx_on_logit revision-3 placement: c scalar added to the per-second logit only
...
STRUCT_ARMS = ("full", "avce", "no_qk_enc", "no_cell", "no_lin", "no_bias", "gated_bias",
               "shared_bias", "no_context", "ctx_on_logit")
...
class ERCA(nn.Module):
    ...
    def __init__(self, cfg, prior_scale, arm="full", no_verdict=False):
        ...
        self.enc = None if self.concat else EvidenceEncoder(hid, cell=(arm != "no_cell"),
                                                            lin=(arm != "no_lin"))
        bias_mode = {"avce": "none", "no_bias": "none", "gated_bias": "gated",
                     "shared_bias": "shared"}.get(arm, "key")            # revision 2: key bias
        self.cma = EvidenceRoutedCMA(hid, nhead, ffn, dropout, bias_mode=bias_mode,
                                     qk_enc=(arm not in ("avce", "no_qk_enc")))
        self.ctx_mode = {"avce": None, "no_context": None, "ctx_on_logit": "logit"}.get(arm, "rep")
