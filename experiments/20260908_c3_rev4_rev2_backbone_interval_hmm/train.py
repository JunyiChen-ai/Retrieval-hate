"""One training trial of candidate 3 revision 4 (revision-2 backbone + interval
evidence HMM). Training = revision 2: video-level top-k MIL on
z = fc(a_out + c) + fc(v_out + c) + prior_scale * ell / ELL_SCALE, MACIL-SD's
cross-modal contrastive loss CMAL on a_out + c / v_out + c (the video-level
evidence context c enters both streams, revision-2 placement), the
verdict-block MIL on the logit without the prior, checkpoint on the official
validation split, test scored through the shared evaluator.

Fusion (module 3) = src/interval_evidence_hmm.IntervalEvidenceHMM fitted on
train video labels: true verdict intervals, transitions over normalized video
time, positive-video constraint in EM (config fusion / normalized_time /
positive_constraint; gate A1 of the README fixed these before the search).

    python experiments/20260908_c3_rev4_rev2_backbone_interval_hmm/train.py \
        --corpus hatemm --seed 234 --out-dir runs/.../trial0 --config cfg.json

--ablation (README section 3):
  structure  full | avce | no_qk_enc | no_cell | no_lin | no_bias | gated_bias | shared_bias | no_context | ctx_on_logit
  training   mean_prior | mean_prior_all | no_block | no_prior | no_cmal | no_verdict
  fusion     index_hmm (src/verdict_hmm index hierarchy) | no_constraint | seconds_time
"""
...
DEFAULTS = {
    ...
    # module 3 prior (w_fine removed in revision 3: fixed 1); module 2 block MIL
    "prior_scale": 2.0, "w_fine": 1.0, "lambda_block": 0.5, "topk_div": 16,
    # module 3 fusion, fixed by gate A1 (README section 4) before the search
    "fusion": "interval", "normalized_time": True, "positive_constraint": True,
}
TRAIN_ARMS = ("mean_prior", "mean_prior_all", "no_block", "no_prior", "no_cmal", "no_verdict")
FUSION_ARMS = ("index_hmm", "no_constraint", "seconds_time")
ABLATIONS = STRUCT_ARMS + TRAIN_ARMS + FUSION_ARMS
...
def train(corpus, seed, out_dir, cfg, ablation, device, num_workers):
    ...
    fusion = "index" if ablation == "index_hmm" else a.fusion
    fopts = {}
    if fusion == "interval":
        fopts = {"normalized_time": bool(a.normalized_time) and ablation != "seconds_time",
                 "positive_constraint": bool(a.positive_constraint) and ablation != "no_constraint"}
    hmm, n_pos, n_neg = hc.fit_hmm(corpus, train_ids, labels, binary, model=fusion, **fopts)
    hmm.save(os.path.join(out_dir, "hmm_params.json"))
    say("verdict HMM (%s %s) fitted on %d positive / %d negative train videos: %s"
        % (fusion, json.dumps(fopts), n_pos, n_neg,
           json.dumps({k: (round(v, 4) if isinstance(v, float) else v)
                       for k, v in hmm.params().items() if k not in ("k", "j", "A")})))
    ...
    model = ERCA(a, prior_scale, arm=arm, no_verdict=(ablation == "no_verdict")).to(device)
