...
**结果（三 seed，2026-09-17 21:55；来源 `runs/20260910_online_query_within_it5/<corpus>/seed<s>/trial<best>/summary.json`，判定 `screen.py --suffix _it5 --seeds 234 2025 3407`）**

| 语料 | best trial | 8 次点 AP / ROC / within（三 seed 均值 ± std） | 停止点（均调用） | 4b 8 次点 | P1 | W1 | E1 |
|---|---|---|---|---|---|---|---|
| HateMM | 5 / 10 / 2（12 有效 trial） | .6444 ± .0067 / .8500 ± .0018 / .6492 ± .0069 | 5.33 次 .6387 / .8460 / .6486 | .6448 / .8624 / .6420 | AP −.0100 **FAIL**，ROC +.0080 | +.0113 PASS | 过 |
| HCS | 11 / 0 / 12（15 有效 trial） | .6837 ± .0016 / .6809 ± .0037 / .5597 ± .0081 | 7.26 次 .6842 / .6796 / .5604 | .6893 / .6809 / .5802 | AP −.0016，ROC −.0014 **FAIL**（噪声内） | −.0023 **FAIL**（对 .562） | 过 |

可证伪预期：(1) a_f < a_v 六个 seed 全部成立：HateMM (a_f, a_x, a_v) = (3.509, 3.714, 3.618) / (2.487, 2.668, 2.604) / (0.917, 1.373, 1.263)，初始 α = 3.870 / 2.704 / 1.182；HCS (2.759, 2.818, 2.821) / (2.759, 2.771, 2.773) / (2.170, 2.193, 2.193)，初始 2.785 / 2.766 / 2.177。HateMM 上三个标量明显分开（细窗项压低 9–22%），HCS 上几乎停在初始值（变化 < 1%），所以 B 在 HCS 上没有起作用。(2) 34 次点对 8 次点：HateMM .6296 / .6418 / .6504 对 .6378 / .6418 / .6535，均值 −.0038（seed 234 单独 −.008），按均值成立；HCS 34 次点 .7047 / .7094 / .7027 全部高于 8 次点。4b 的"多问反而掉"在均值上消失。

读法：对 4b，HateMM AP 持平（−.0004）、ROC −.012、within +.007；HCS AP −.006、ROC 持平、within −.021（预注册时已预期 within 回落到第 3 轮水平 .565，实际 .560）。P1 的 HateMM AP 缺口（−.010）与第 3、4b 轮相同，本轮机制改动没有补回；审稿人指向 AP 的建议（有界累积）在门 T5 与臂 `temper_icc` 上都掉分。

**臂（full − arm，正 = 部件有用；规则 14(g) 两语料三 seed 均值 AP 或 ROC ≥ .01 才可主张；两语料三 seed 全部跑完，2026-09-18 00:52；来源 `runs/20260910_online_query_within_it5/arms_summary.json`，`arms_summary.py _it5`；每个臂用该 seed best trial 的超参，输出 `runs/20260910_online_query_within_it5/diag/<corpus>/seed<s>/<arm>/summary.json`）**

| 臂 | 去掉 / 换成 | HateMM AP / ROC / within | HCS AP / ROC / within | 可主张 |
|---|---|---|---|---|
| `text_prior_off` | 文本分数 x_t 移出先验项，只作编码器输入列 | +.042 / +.024 / +.049 | +.013 / +.018 / +.041 | **是**：两语料 AP、ROC 都 ≥ .01。审稿人第 2 条"文本项 = 外部分类器 ensemble"属实，且它是仅次于 VLM 裁定的第二大部件；论文如实写成外部有监督分类器分数进先验 |
| `eoc_model_raw` | EOC 权重回到内容头概率（不经 r_f + (q_f − r_f)p 校准） | +.022 / +.010 / +.024 | −.000 / +.002 / +.005 | 否（HateMM 单语料 AP、ROC 都 ≥ .01；HCS 无差别）。校准在 HateMM 上有用，是本轮唯一在 HateMM 上明确正向的改动 |
| `temper_icc` | 细窗证据按 ICC 折减（审稿人建议的有界累积；ρ 按 seed .52 / .54 / .49，HCS .61 / .74 / .53） | +.026 / +.008 / +.012 | +.001 / +.004 / −.014 | 否，且 HateMM 上有害（seed 3407 把 a_f 学到 .04，等于关掉细窗证据）。与门 T5 一致，不进默认 |
| `global_alloc` | 回到 4b 的跨视频分配（训练上限 8） | +.009 / +.003 / +.016 | +.001 / +.005 / −.012 | 否。每视频分配在 HateMM 上略好（AP +.009、within +.016），在 HCS 上 pooled 不变、within 掉 .012（回到 .5714；4b 的 .5802 还含更多 trial 的搜索差）：HCS within 的回落确实来自分配方式 |
| `single_prior` | 三个标量回到单个 α | +.008 / +.007 / +.017 | +.002 / +.001 / +.002 | 否（HateMM AP +.008 差 .002 到门槛，within +.017；HCS 上标量不动、无差别） |

归因：it5 对 4b 的 HCS AP −.006 不能归到单个改动（换回分配 .6828、换回旧 EOC .6840、换回单 α .6821 都不高于默认 .6837），只能是搜索差异（15 trial 热启动对 20 trial）或噪声（std .002–.004）。HateMM 上三项默认改动各自方向都是正的（校准 +.022、每视频分配 +.009、三标量 +.008 AP），但叠加后对 4b 只是持平（AP −.000，ROC −.012），因为 4b 的 best trial 也是在各自配置下搜出来的。
...
### 12c. 文本管线 error analysis（2026-09-22，用户要求；`text_error_analysis.py` → `runs/20260910_online_query_within_it5/text_analysis/<corpus>.json`；全部离线、test 集、同一评测器）

**A. 文本分类器分数单独当定位分数（test，8 次调用点无关）**

| 语料 | x_t 的转录 | x_t 单独 AP / ROC / within | 视频级 ROC（每视频最大值） | 负例视频语音秒 p>.5 占比 | 正例视频：仇恨秒 p>.5 / 非仇恨秒 p>.5 |
|---|---|---|---|---|---|
| HateMM | `data/ASR` | .511 / .763 / .626 | .914 | 4.1% | 41.4% / 31.2% |
| HateMM | 句子级 | .526 / .762 / .600 | .907 | 4.3% | 45.9% / 38.1% |
| HCS | `data/ASR` | .662 / .618 / .577 | .804 | 9.4% | 20.9% / 13.7% |
| HCS | 句子级 | .676 / .634 / .554 | .820 | 8.1% | 26.0% / 20.9% |

读法：这个推特上训练的分类器在 HateMM 上匹配得好（仇恨秒 41–46% 判高，负例只有 4%），在 HCS 上匹配得差（仇恨秒只有 21–26% 判高，负例 8–9%，正例里非仇恨秒和仇恨秒的判高率只差 5–7 个点）。两语料上句子级转录的 x_t 单独 pooled 略高、within 都更低。HateMM 上 `xt_chunks` 训练后掉 .021 pooled，不能由分类器单独的数字解释（单独看句子级略好），掉分发生在 HMM / 先验 / 训练交互里，本分析没有定位到。

**B. 冻结文本行上的逐秒线性探针（test 视频 5 折按视频分组，out-of-fold）**

| 行 | HateMM AP / ROC / within | HCS AP / ROC / within |
|---|---|---|
| bert（句子级，通用 BERT） | .344 / .607 / .523 | .589 / .566 / .490 |
| hate_chunks（句子级，hate-RoBERTa） | .391 / .666 / .551 | .614 / .576 / .499 |
| bert_utterance（`data/ASR` 话语，通用 BERT） | .414 / .662 / .534 | .580 / .589 / .525 |
| hate_roberta（`data/ASR` 话语，hate-RoBERTa） | .432 / .724 / .608 | .568 / .539 / .515 |

读法：HateMM 上 hate 行带的逐秒信息远多于 BERT 行（三项都高，within .608 对 .523）。HCS 上同一转录下 hate_chunks 行的线性可读信息不低于 BERT 行（.614 / .576 对 .589 / .566），所以 HCS 的掉分**不是**行里没有信息。

**C. 训练模型 pooled AP 的分解（8 次调用点，三 seed 均值）**

| 语料 | 配置 | AP | 视频内排序理想化后 AP（增量） | 视频间分离理想化后 AP（增量） | 视频级 ROC（每视频最大值） | 只算正例视频的 AP |
|---|---|---|---|---|---|---|
| HateMM | 默认 | .6444 | .7071 (+.063) | .7258 (+.081) | .923 | .726 |
| HateMM | unified_chunks | .6613 | .7354 (+.074) | .7361 (+.075) | .923 | .736 |
| HateMM | xt_chunks | .6232 | .6893 (+.066) | .7110 (+.088) | .917 | .711 |
| HCS | 默认 | .6837 | .7787 (+.095) | .6975 (+.014) | .881 | .698 |
| HCS | unified_chunks | .6658 | .7723 (+.107) | .6876 (+.022) | .840 | .688 |
| HCS | xt_chunks | .6910 | .8014 (+.110) | .7050 (+.014) | .877 | .705 |
| HCS | text_feat_hate | .6615 | .7575 (+.096) | .6863 (+.025) | .811 | .686 |

读法：HateMM 上 hate 行的 +.017 不来自视频级（视频级 ROC 不变 .923），来自正例视频内的排序（只算正例 AP +.010）。HCS 上 hate 行的 −.018 主要是视频级：视频级 ROC .881 → .840（`text_feat_hate` .811），只算正例 AP 只掉 .010；即带 hate 行的模型把负例视频推高了。HCS test 只有 10 个负例视频，视频级 ROC 由这 10 个决定，噪声大但方向与 A 一致（分类器在 HCS 负例上判高 8–9%，两倍于 HateMM）。x_t 这条路经 HMM 按语料校准（分类表、中心化），流这条路没有任何校准，且 hate 行范数 21 对 BERT 行 15、音频行 3，直接进线性投影。
...
## 15. 离线检验：内容作为 HMM 观测 + 视频级可靠性变量（2026-09-24，方向 1 的前提检验，用户批准"先做看看能不能推进"）

**起因（test 集，三 seed，`no_verdict` 臂与默认 best trial 的分数；只读分析）**：VLM 成片的错误主要是对整个仇恨视频"沉默"，不是对负例整段乱报。30 个细窗报仇恨比例 < .1 的正例视频：HateMM 21/85（其仇恨秒占全部 14%），HCS 24/69（占 29%）；fine-rate ≥ .3 的负例视频只有 HateMM 4 个、HCS 1 个。在"VLM 沉默"组（fine-rate < .1）内的帧级 AP：HateMM 默认 .371 对不用 VLM 的模型 .275；**HCS 默认 .495 对不用 VLM 的模型 .573**——HCS 上 VLM 的沉默把内容与文本的正确判断压了下去。现 HMM 假设裁定错误独立，30 个"没有"被当成 30 份独立证据。第 4 节门 D1 的三档可靠性混合只看裁定时 pooled AP 掉 .074 / .051，原因是"真没有"与"VLM 漏报"不可分；本节检验内容进入同一个模型后是否可分。

**做法（不训练神经网络，0 次新调用；`content_regime_check.py`）**：
- 内容观测：各 seed `no_verdict` 臂的模型（内容 + 文本 x_t，不含任何 VLM 信息）在 train / test 上的逐秒分数 p_c(t)，作为区间 HMM 的一个观测族（沿用文本族实现：10 档分类发射表，EM 用视频标签拟合；逐秒权重 w_t = W / T，W = 30 为主设置，即整段内容总权重等于 30 个细窗；W = 8 作敏感性）。先核对重建的分数与该臂 `scores_test_coarse4.jsonl` 一致。
- 裁定：训练与测试都是 8 次状态（4 粗块 + 均匀 4 细窗 0/15/7/22）；HMM 在 train 上按视频标签 EM 拟合（正例约束、归一化时间，同默认）。
- 变体：V0 只裁定 R = 1；V1 只裁定 R = 3（第 4 节的三档：常规 / 过度触发 / 漏报）；V2 裁定 + 内容 R = 1；**V3 裁定 + 内容 R = 3（主变体）**；V2b / V3b 同 V2 / V3 但 W = 8。分数 = HMM 逐秒后验对数几率，统一评测器。参照：不用 VLM 的模型本身、默认方法 best trial。
- 指标：test pooled AP / ROC / within（三 seed 均值；V0、V1 与 seed 无关）；分组帧级 AP（"VLM 沉默"组 fine-rate < .1 与其余视频，分析用，非官方指标）；V3 下沉默正例视频的"漏报档"后验。

**判定（开跑前写定）**：
1. 主问题（z 在内容进入模型后是否有用）：V3 − V2 在 HCS 沉默组帧级 AP ≥ +.03，且两语料 pooled AP、ROC 都 ≥ V2 − .005 → 成立，方向 1 的"视频级可靠性"主线有数据支撑，推进端到端版本。
2. 次问题（内容作为观测本身）：V2 对 V0 的 pooled 与沉默组 AP。
3. 对照：V1 对 V0 预期复现门 D1 的掉分（z 无内容不可识别）。
4. 1 不成立而 V2 明显好于 V0 → 方向 1 只剩"内容进同一模型"一条动机；两者都不成立 → 方向 1 缺少数据动机，停止。

### 15.1 结果（2026-09-24 06:50；来源 `runs/20260910_online_query_within_it5/content_regime/<corpus>/seed<s>/{summary.json,metrics_test_*.json}`；内容分数重建与原臂 test 分数逐秒完全一致，EM 对数似然全部单调）

test，三 seed 均值（V0、V1 与 seed 无关）；AP / ROC / within 来自统一评测器，分组 AP 为分析用。

| 变体 | HateMM | 沉默组 / 其余 AP | HCS | 沉默组 / 其余 AP |
|---|---|---|---|---|
| 不用 VLM 的模型（内容 + 文本） | .528 / .776 / .620 | .275 / .639 | .614 / .594 / .548 | .573 / .653 |
| V0 只裁定 R=1 | .561 / .841 / .563 | .216 / .591 | .670 / .651 / .496 | .389 / .720 |
| V1 只裁定 R=3 | .448 / .767 / .615 | .129 / .542 | .618 / .604 / .495 | .403 / .667 |
| V2 裁定 + 内容 R=1 | .530 / .796 / .623 | .266 / .640 | .621 / .594 / .524 | .561 / .662 |
| **V3 裁定 + 内容 R=3** | .525 / .785 / .624 | .263 / .640 | .599 / .582 / .521 | .510 / .657 |
| V2b / V3b（W = 8） | .545 / .800 / .665；.534 / .788 / .663 | .256 / .655；.254 / .652 | .592 / .586 / .525；.590 / .582 / .522 | .485 / .659；.484 / .656 |
| 参照：默认方法 best trial | .644 / .850 / .649 | .371 / — | .684 / .681 / .560 | .495 / — |

**判定（按 15 节预注册）**：
1. 主问题不成立：V3 − V2 在 HCS 沉默组 AP −.051（要求 ≥ +.03），pooled AP HCS −.022、HateMM −.005。可靠性变量确实把沉默正例视频分到"漏报档"（后验 HCS .91、HateMM .64），但分数没有因此变好。
2. 内容作为观测（V2 对 V0）：沉默组 AP 大幅上升（HCS +.172，HateMM +.050），within 上升（+.028 / +.060），但 pooled AP、ROC 两语料都下降（HCS −.050 / −.057，HateMM −.031 / −.045），不算"明显更好"。
3. V1 对 V0 复现门 D1 的掉分（HCS −.053，HateMM −.112）。
4. **结论：方向 1 在这个形式下没有数据支撑，按预注册停止。**

**原因（拟合参数，seed 234）**：内容族加入后 EM 改写了隐状态的含义——HateMM 细窗命中率 q_fine .695 → .387、仇恨段退出速率 λ10 2.09 → .044（隐状态变成"内容判为仇恨的长片段"）；HCS q_fine .485 → .255、r_fine .066 → .168，裁定几乎不带信息；V3 在 HCS 的常规档 r_fine .745 > q_fine .31（退化）。这与第 7 节第 2 轮"文本作 HMM 观测族"的失败同一机制：稠密的逐秒观测在 EM 下主导稀疏的裁定，隐状态从"VLM 意义上的仇恨"漂移成"内容分数高"。端到端版本的内容发射更灵活，预期同样面临这个问题。

**保留的事实**：HCS 上 VLM 对 35% 的正例视频沉默（占 29% 仇恨秒），这组视频里默认方法（.495）低于不用 VLM 的模型（.573）——这是一个真实、未解决的失败模式，但"把内容放进 HMM"不是它的解法。
