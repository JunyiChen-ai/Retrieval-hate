...
**起因**：4b 完成后请外部审稿（GPT-6-astra，口述方法、不读文件）对方法弱点做了一轮审阅与一轮追问（记录见本节末）。用户裁定"先解决审稿人的 concern"。本轮只改机制、不改评测；第 3 轮的 P1 / W1 / E1 与搜索目标不变；输出 `runs/20260910_online_query_within_it5/`。
