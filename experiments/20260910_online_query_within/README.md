...
| 轮 | 改动 | 结果 | 判定与下一步 |
|---|---|---|---|
| 4 | 机制 B'（跨视频分配，lookahead 2），其余同第 3 轮 | seed 234 HCS（lab1，20 trial，best trial 16）8 次固定点 **.6939 / .6754 / .5991**，停止点 6.77 次 .6902 / .6690 / .5955；34 条 .7104 / .6974 / .6039；训练分配：正例 4.12 / 负例 3.16 个细窗。HateMM（lab3）进行中（分配正例 5.66 / 负例 2.89）。 | HCS 单 seed：W1 过（+.037，比第 3 轮 within 高 .022）；P1 AP 过（+.009），ROC −.007（比第 3 轮低 .005）；E1 过。已起 HCS seed 2025 / 3407（lab1 并行）。 |
