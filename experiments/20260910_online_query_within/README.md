...
| 4（+ 训练预算跨视频分配） | .691 / .678 / .593 | .637 / .850 / .646（两 seed 只 5 trial） | W1 过；HCS ROC −.004；HateMM AP −.017 |
...
3. 论文里"跨视频分配"（第 4 轮）只对 within 有用、对 AP 无用，是否保留。**2026-09-14 更新**：第 4 轮首版结果因训练期记账缺陷作废（第 9 节），用户裁定按修正版重跑（`_it4b`），此项待重跑结果。
