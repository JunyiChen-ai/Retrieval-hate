...
**臂（full − arm，正 = 部件有用；规则 14(g) 两语料三 seed 均值 ≥ .01 才可主张；来源 `runs/20260910_online_query_within_it5/arms_summary.json`，`arms_summary.py _it5`；下表 HateMM 列为 seed 2025 / 3407 两 seed（seed 234 的五个臂与 seed 2025 的两个补充臂在 uoa-lab3 跑，跑完更新）**

| 臂 | 去掉 / 换成 | HateMM AP / ROC / within | HCS AP / ROC / within（三 seed） | 可主张 |
|---|---|---|---|---|
| `text_prior_off` | 文本分数 x_t 移出先验项，只作编码器输入列 | +.058 / +.033 / +.057（n=2） | +.013 / +.018 / +.041 | **是**：两语料 AP、ROC 都 ≥ .01。审稿人第 2 条"文本项 = 外部分类器 ensemble"属实，但它是仅次于 VLM 裁定的第二大部件，论文如实写 |
| `temper_icc` | 细窗证据按 ICC 折减（审稿人建议的有界累积） | +.028 / +.010 / +.020（n=2；模型把 a_f 学到接近 0） | +.001 / +.004 / −.014 | 否：HateMM 上有害、HCS 上 pooled 持平 within 反升 .014。与门 T5 一致，不进默认 |
| `single_prior` | 三个标量回到单个 α | +.007 / +.008 / +.020（n=2） | +.002 / +.001 / +.002 | 否（HateMM 单独 within +.02、AP +.007；HCS 无差别） |
| `eoc_model_raw` | EOC 权重回到内容头概率（不经 r + (q − r)p 校准） | +.013 / +.005 / +.043（n=1，seed 3407） | −.000 / +.002 / +.005 | 待 seed 234、2025；HCS 无差别 |
| `global_alloc` | 回到 4b 的跨视频分配（训练上限 8） | +.002 / −.004 / +.038（n=1，seed 3407） | +.001 / +.005 / −.012 | 待补；HCS within 回到 .5714（4b .5802），pooled 不变：HCS within 的回落确实来自分配方式；HateMM 上每视频分配 within 反而高 .038 |

归因：it5 对 4b 的 HCS AP −.006 不能归到单个改动（换回分配 .6828、换回旧 EOC .6840、换回单 α .6821 都不高于 .6837），只能归到三项叠加或搜索差异（15 trial 热启动对 20 trial）。
