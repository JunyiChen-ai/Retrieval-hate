"""Fixed Optuna search for one (corpus, seed), rule 7 of RESEARCH_ITERATION_RULES.md.
    ...
    python experiments/20260910_online_query_within/search.py \
        --corpus hatemm --seed 234 --out-root runs/20260910_online_query_within

Each trial trains once (train.py), selects its checkpoint on the official
validation split, scores test, and reports the test pooled AP/ROC/within at
the operating point (eoc, 4 fine picks, tau 0: 8 calls per video).
Optuna's objective is the scalar (test AP + test ROC + test within) / 3
(module-1 iteration 2: within is a target, plan of 2026-09-10). Nothing is
pruned on within (--no-within-prune is the default, kept for the record).
...
def sample(trial):
    # README section 2 (declared before any search, 2026-09-10, same five scalars as iteration 1);
    # w_fine removed (fixed 1), dropout (.2) and lamda_cof (.05) fixed at
    # MACIL-SD's values. Method-level scalars: prior_scale, lambda_block.
    return {
...
def main(argv=None):
    ...
    ap.add_argument("--no-within-prune", action="store_true", default=True,
                    help="rule 7 as amended 2026-09-06: record within, never prune on it (default)")
    ...
    def objective(trial):
        ...
        obj = (t["pooled_ap"] + t["pooled_roc"] + t["within_roc"]) / 3.0
    ...
    if scored:
        v = max(scored, key=lambda t: (t.user_attrs["val_pooled_ap"] + t.user_attrs["val_pooled_roc"]
                                       + t.user_attrs["val_within_roc"]) / 3)
