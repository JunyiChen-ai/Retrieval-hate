...
| **P3-HateMM** same | " | probe pass (**PASS, k-consistent +0.0108**) → train | **SLOT — training in flight** (ASR job 12393 running). Probe is the one k-consistent positive of the three; densest evidence signal (var 1.28/0.71) | EXP_p3_evidence_pooling · `15f5f08` |
...
- **[SLOT] P3-HateMM training verdict.** Probe PASSED, k-consistent (+0.0108 @k20; densest evidence
  signal) — the campaign's single cleanest probe positive. Training pending (ASR job 12393 running →
  scoring → 3-seed floor/wsoftT1/wmild, both protocols). *If* HateMM clears >1pt/≥2/3/both-protocols
  it is the one place the pooling intervention earns a role (an explicit-hate-speech dataset where
  the localized signal is densest); the EN/ZH pattern predicts within-noise. **Update §1 P3-HateMM
  row + §3 when it lands.**
