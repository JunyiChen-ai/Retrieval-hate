...
截至 **2026-09-06 07:56 NZST**。依据：候选9 HCS完整20trial回传审计，单语料筛选通过；HateMM仍运行固定20trial；HCS锁定配置消融链已启动。权威数字均引用本机runs原评测。
...
**C9 HCS seed234通过单语料数值筛选，但明显弱于C1，三个模块贡献未知。** HateMM尚未结束，不提前补确认seed；利用HCS已锁定配置并行检验主消融及no_vlm，不把单seed结果当三模块成立。
...
| C9 HateMM 首trial0（非最终） | .597334/.804517/.646860，epoch4 | 首trial完整50epoch+val109/test214核对；20trial仍运行，不能提前判完整方法；[原评测](../runs/20260906_interval_evidence_transport/hatemm/seed234/trial0/metrics.json) |
...
| 任务 | 当前状态 | 位置 |
|---|---|---|
| C9 HateMM seed234，lab1 | PID/PGID3125429；07:53已完成9/20，trial9运行；首trial212.884198秒固定20trial，monitor1734844存活 | [预算](../runs/20260906_interval_evidence_transport/hatemm/seed234/budget.json)、[monitor](../runs/20260906_interval_evidence_transport/hatemm/seed234/monitor/run.log) |
...
| C9 HCS seed234消融，lab3 | PGID3575434；锁定trial18，三个主替换并行后接no_vlm；monitor1745899首次RUNNING成功，三臂epoch输出正常 | [输出](../runs/20260906_interval_evidence_transport/ablations/hateclipseg/seed234/)、[monitor](../runs/20260906_interval_evidence_transport/ablations/hateclipseg/seed234/monitor/run.log) |
...
| 资源 | lab1搜索、lab3三个主消融并行；本机他人GPU任务；lab-server无HateVideo环境/项目 | 仅调度已就绪独立任务，不启动未满足双语料筛选条件的确认seed |
