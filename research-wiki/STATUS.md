...
- **迭代终态**:LOCO-ST(A1)= 最终方法。上限证据(三种特征栈一致):HateMM ~.76 / EN ~.77 / HCS ~.60——EN 已打到上限,HateMM 差 .08,HCS 上限本身仅比 VERA 高 .04。**廉价机制已穷尽;进一步需要资源级投入**(TANDEM 式 VLM LoRA 数天 GPU / 补 MHC 原始视频 / 新数据),属用户裁量。
