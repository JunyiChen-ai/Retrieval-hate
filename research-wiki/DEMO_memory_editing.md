...
> **⚠ 多 seed 更正(2026-07-26,F88 / ERRPAT_MHC-EN_2026-07-26.md §6.5,commit `ad56a62`)——本文件全部
> EN 编辑数字按此读。** 本演示所有数字为 **seed 0**(job 12210 val-selected ckpt)。在 4 个部署 seed 的
> banked top-60 邻居表上做的**精确重放**(未编辑重放逐 seed 复现 floor 到 <1e-12)给出:
> **2-entry 删除 = seed 0 +0.0124,seed 1/2/3 各 0 次投票翻转,4-seed 均值 +0.0031**;seed 0 翻的两项
> 首次被点名——`cYQyH7hbNnw`(vote +0.008649 → −0.018971)与 `xqilG4oMvvI`(+0.046778 → −0.027381)——
> **均为低 margin FP(C8 seed-flip 噪声带),都不是硬错误**。**14-id 规则表严格更强**:+0.0093 acc /
> +0.0089 mF1,**3/4 seed 正,6 修 0 坏**——即下文结论 4 的「其余 12 条无影响」**只在 seed 0 成立**
> (seed 1/2 上它们值 +0.0186 与 +0.0062)。规则表仍 3× 低于 +0.030 门、落 ±0.014 带内、且已 test-consumed,
> 合法的 dev 侧 pregate 在算术上不可能(dev n=80 ⇒ 1 项 = 0.0125)。
> **全项目统一引用措辞:"human-in-the-loop capability demonstration, single-seed; not an accuracy claim."**
> 「EN 全项目最高单点」这一措辞**已撤回**(PAPER_MASTER_TABLES T3 / T6.5、方法章 §5、实验章 §5、分析章 §4
> 均已同步)。本演示要主张的性质是**可定向编辑本身**(语义寻址 + 外科删除 + 零重训),该性质不依赖上述数字。
>
...
4. **误标下架:删 2 条 W2 噪声条目,整体不降反升**:EN 删除 14 条低置信条目后 acc 0.8075→0.8199、macro-F1 0.7626→0.7748,超过全部 5 个随机对照种子 (acc max 0.8137);且 (b') 只删 `XScP1AiMkNM`+`QvPp8Q7QhWE` 两条即可复现全部增益 (0.8199/0.7748),其余 12 条规则命中条目增删无影响。**两条被人工标记的疑似噪声条目确实是有害记忆,删除即修复,零训练。** —— **⚠ F88 更正(见文首横幅):本条全部为 seed-0 读数。**「2 条即复现全部增益」与「其余 12 条无影响」**只在 seed 0 成立**;跨 4 个部署 seed,2-entry 删除的均值是 **+0.0031**(seed 1/2/3 零翻转),而 14-id 规则表反而是**更强**的编辑集(**+0.0093 acc / +0.0089 mF1,3/4 seed,6 修 0 坏**)。本条应读作 **capability demonstration, single-seed**,不是精度主张。
...
6. **对可审计性 claim 的支撑**:本演示证明了 (i) 记忆条目可被档案字段语义寻址;(ii) 删除是外科手术式的(EN 定向、2 条噪声条目精确摘除);(iii) 全程 CPU、秒级、零训练——这是 trained-MoE 头结构上做不到的操作。未证明的:切片级效应的统计显著性(n 太小)、ZH 的组级编辑(受档案 target 召回率限制)、**以及(F88 新增)删除带来的精度改善在部署 seed 上的可复制性——多 seed 重放证明它不可复制(4-seed 均值 +0.0031,3/4 seed 零翻转),故 (i)–(iii) 的可编辑性性质成立而「删除即涨点」不成立**。
